...
- ☐ Осталось: реальные Apple/Telegram social providers после выдачи credentials/callback URLs.
...
- ✅ **Долги/рассрочка** (`debts/` модуль): продажа в долг/рассрочку по заказу, лимит
  ...
  срока → Outbox (`debt_due_soon`/`debt_overdue`) + ledger `debt.reminder_queued`.
...
**Осталось:**
- **Unblocked polish:** открытых пунктов без внешних блокеров нет; schema/trade-in IMEI intake
  вынесен в `CODEX-HANDOFF.md` B10 как отдельный координационный пункт.
