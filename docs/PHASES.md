...
`/staff` → Сотрудник App 2.0 ✅ (мобильное прил. сотрудника). ☐ Витрина+кабинет →
**Клиент App 2.0** (1000 строк, ещё не выровнено). Консоли `/warehouse /warranty /approvals`
покрывают функции Сотрудник App, но при желании их точку входа даёт `/staff`.
