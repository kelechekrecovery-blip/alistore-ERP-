...
даёт `/staff`.
Осталось по прототипу (мелочи): избранное ✅ + **сравнение ✅** (`/compare` + `lib/compare.tsx`,
макс 4, ⇄-переключатель на карточке товара, бейдж «ЛУЧШАЯ ЦЕНА»); «Мои устройства» ✅ +
**гарантийный талон ✅** (`/account/warranty/[imei]` + `WarrantyCertificate`, warrantyUntil/daysLeft
из devices-API = дата покупки + 12 мес). Осталось: статус-заказа timeline как отдельный экран.
