...
- ☐ ERP-дашборд: деньги (Payment Ledger), продажи, маржа, остатки, KPI сотрудников.
- ☐ **Risk Center** (тревоги: касса≠, COD не сдан >24ч, зависший резерв, повтор IMEI).
- ☐ **Command Center** + **Order Timeline** + **Event Ledger** просмотр.
**Проверка:** цифры дашборда = агрегаты из AuditEvent (расхождений нет); тревоги ловятся на подсеянных данных.

## Phase 9 — Мультисклад, склад-операции, гарантия (v1) ☐
- ☐ Мультифилиал + перемещения (stock.moved), инвентаризация со сканером.
- ☐ **WarrantyCase** с SLA, **Supplier RMA** + Scorecard.
...
**Проверка:** перемещение сходится по остаткам; гарантийный кейс проходит SLA; импорт идемпотентен.
