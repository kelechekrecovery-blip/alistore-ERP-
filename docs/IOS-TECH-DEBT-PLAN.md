...
> **Актуализация 05.08.2026.** Перечисленные в историческом аудите блокеры
> закрыты в коде и проверены: OTP-модели больше не требуют отсутствующий
> `expiresIn`; iOS/Android courier-команды передают обязательный Evidence key;
> PIN lockout, POS `activeSaleId`, split-tender и округление скидки покрыты
> тестами. Android courier дополнительно разделяет Evidence для успешной и
> неуспешной доставки (`Подтверждение доставки` / `Неуспешная доставка`).
> Ниже остаются рекомендации по техническому долгу и обязательная физическая
> сертификация устройств — этот файл больше не является доказательством того,
> что указанные дефекты всё ещё присутствуют.
>
