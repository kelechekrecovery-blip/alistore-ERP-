...
- **40 backend-модулей** (NestJS) · **32 веб-роута** (Next.js) · **9 миграций**
- **88 API тест-сьютов / 311 тестов — зелёные** (`jest`)
- **8 Playwright smoke-flow — зелёные** (`npm run e2e`)
...
| **3** Аккаунт+Auth | OTP-вход, «Мои заказы», адреса, настройки, уведомления+consent, бонусы | ✅ |
| **4** POS 2.0 | Тёмный терминал, полная продажа, скидка-approval, **offline-очередь**, печать | ✅ |
...
Похожие товары/отзывы · восстановление пароля/соцсети · Dispute Center UI · произвольные
периоды в KPI · приёмка партий UI · зарплаты · долг-напоминания · подарочные карты · B2B/опт.

## Проверка (гейт готовности)
```bash
cd apps/api && npx jest                # 88 сьютов / 311 тестов ✓
