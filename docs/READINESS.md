...
- **114 API test suites / 453 теста — зелёные** (`jest`)
...
Finance 2.0 cashflow/инкассация/валюта · предзаказы (варианты SKU и виртуальные наборы готовы; возврат компонентов выделен отдельно) · quantity-consignment и возврат комиссионного товара (серийная приёмка/начисление/выплата готовы) · HR-графики ·
...
cd apps/api && npx jest                 # 114 suites / 453 теста ✓
npm run api:build                     # ✓
