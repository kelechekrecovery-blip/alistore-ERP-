...
- **53 backend-модуля** (NestJS) · **39 веб-роутов** (Next.js) · **65 миграций**
- **126 committed API test suites / 498 тестов** для текущего продуктового baseline; detached regression на зафиксированном commit прошёл полностью.
- **42 Playwright flow подтверждены**, включая полный Marketing CMS, ERP-controlled storefront composition, checkout points, owner Finance settlement и Service Center.
...
| **2** Витрина | Каталог, карточка, корзина, checkout, поиск, избранное, сравнение, промо/бонусы; ERP-owned точки, scoped guest recovery и полный Marketing CMS handoff подключены: упорядоченные desktop/mobile блоки, подборки, approved-only отзывы и серверные промокоды | 🟡 |
...
exact detail pixel-pass сервис-центра и физический UAT (полный web/API цикл, включая подменный фонд, уже готов) · оптимизация маршрутов/live tracking (зоны, слоты, capacity, checkout и dispatch готовы) · store checklists · campaign attribution/ROAS (Marketing CMS handoff уже закрыт) · расширенная
