...
- **51 backend-модуль** (NestJS) · **39 веб-роутов** (Next.js) · **62 миграции**
- **124 API test suites / 493 теста — зелёные** (`jest`)
- **39 Playwright flow подтверждены**, включая ERP-controlled storefront collection, checkout points, owner Finance settlement и Service Center. В последнем широком прогоне 36 прошли сразу, три Next dev navigation timeout прошли отдельным повтором 3/3.
...
| **2** Витрина | Каталог, карточка, корзина, checkout, поиск, избранное, сравнение, промо/бонусы; ERP-owned точки, scoped guest recovery и CMS-контент с упорядоченной/запланированной товарной подборкой подключены; banner/block CMS и moderation ещё требуют завершения | 🟡 |
...
exact detail pixel-pass сервис-центра и физический UAT (полный web/API цикл, включая подменный фонд, уже готов) · оптимизация маршрутов/live tracking (зоны, слоты, capacity, checkout и dispatch готовы) · store checklists · CMS banner/block ordering, review approval и moderation (product collection scheduling уже готов) · расширенная
