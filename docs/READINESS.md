...
| **2** Витрина | Каталог, карточка, корзина, checkout, поиск, избранное, сравнение, промо/бонусы; ERP-owned точки, scoped guest recovery и полный Marketing CMS подключены: блоки, подборки, approved-only отзывы, серверные промокоды, first/last UTM, privacy-safe funnel и refund-adjusted net campaign ROAS | 🟡 |
...
exact detail pixel-pass сервис-центра и физический UAT (полный web/API цикл, включая подменный фонд, уже готов) · оптимизация маршрутов/live tracking (зоны, слоты, capacity, checkout и dispatch готовы) · store checklists · advertiser cabinet lifecycle, creative/channel publication и production spend import (net campaign ROI и privacy-safe funnel уже закрыты) · расширенная
