...
- **45 backend-модулей** (NestJS) · **33 веб-роута** (Next.js) · **21 миграция**
- **98 API тест-сьютов / 350 тестов — зелёные** (`jest`)
- **11 Playwright smoke-flow — зелёные** (`npm run e2e`)
...
периоды в KPI · приёмка партий UI · зарплаты · долг-напоминания.
...
cd apps/api && npx jest                # 98 сьютов / 350 тестов ✓
npm run api:build                     # ✓
