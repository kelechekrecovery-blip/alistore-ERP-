...
- **50 backend-модулей** (NestJS) · **37 веб-роутов** (Next.js) · **53 миграции**
- **119 API test suites / 471 тест — зелёные** (`jest`)
- **33 Playwright smoke-flow — зелёные**, включая quantity/consignment warehouse, Purchase Order, Finance/HR/logistics, Service Center diagnostics/customer estimate approval, product variants/bundles, customer account synchronization, server-authoritative loyalty checkout и exact desktop customer routes (`npm run e2e`)
...
| **10A** Сервис-центр | Warranty work-order, приёмка, диагностика/смета, подтверждение клиентом, RBAC, replay и Ledger готовы; платный ремонт, запчасти и подменный фонд остаются | 🟡 |
...
полный цикл платного сервиса, запчасти и подменный фонд (гарантийная приёмка, диагностика, смета и клиентское подтверждение уже готовы) · оптимизация маршрутов/live tracking (зоны, слоты, capacity, checkout и dispatch готовы) · store checklists · CMS · расширенная
...
cd apps/api && npx jest                 # 119 suites / 471 тест ✓
npm run api:build                     # ✓
