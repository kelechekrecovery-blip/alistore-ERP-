...
- **40 backend-модулей** (NestJS) · **29 веб-роутов** (Next.js) · **9 миграций**
- **62 тест-сьюта / 201 тест — зелёные** (`jest --runInBand`)
...
| **10** Уведомления+CRM | Support Inbox, Customer 360, consent, CRM-UI, **outbox/Novu-доставка** | ✅ |
...
`/support` · `/trade-in` · API: `/ai/insights`, `/ai/assess`, `/reports/*`, `/pos/sale` …
...
| **Telegram Mini App / WhatsApp-магазин** | бизнес-аккаунты + токены каналов |
...
```bash
cd apps/api && npx jest --runInBand   # 62 сьюта / 201 тест ✓
cd apps/api && npx nest build         # ✓
cd apps/web && npx next build         # ✓ (29 роутов)
```
