...
- **47 backend-модулей** (NestJS) · **37 веб-роутов** (Next.js) · **41 миграция**
- **114 API test suites / 455 тестов — зелёные** (`jest`)
- **27 Playwright smoke-flow — зелёные**, включая quantity/consignment warehouse, Purchase Order, Finance expense lifecycle, product variants/bundles, customer account synchronization, server-authoritative loyalty checkout и exact desktop customer routes (`npm run e2e`)
...
| **8** ERP владельца | Дашборд, Risk Center, Event Ledger, **Маржа/KPI**, KPI продавцов, Command Center, период-фильтр, **Finance 2.0 расходы, выплаты, бюджеты и план-факт** | ✅ |
...
Finance 2.0 cashflow/инкассация/валюта · предзаказы (варианты SKU и виртуальные наборы готовы; возврат компонентов выделен отдельно) · quantity-consignment и возврат комиссионного товара (серийная приёмка/начисление/выплата и quantity transfer/adjustment готовы) · HR attendance-derived payroll posting (cash handover готов) ·
платный сервис и подменный фонд · оптимизация маршрутов/live tracking (зоны, слоты, capacity, checkout и dispatch готовы) · store checklists · CMS · расширенная
...
cd apps/api && npx jest                 # 118 suites / 468 тестов ✓
npm run api:build                     # ✓
