...
Finance 2.0 cashflow/инкассация/валюта · предзаказы (варианты SKU и виртуальные наборы готовы; возврат компонентов выделен отдельно) · quantity-consignment и возврат комиссионного товара (серийная приёмка/начисление/выплата и quantity transfer/adjustment готовы) · HR handover/payroll posting ·
...
cd apps/api && npx jest                 # 118 suites / 467 тестов ✓
npm run api:build                     # ✓
