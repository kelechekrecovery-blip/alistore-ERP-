...
- **50 backend-модулей** (NestJS) · **37 веб-роутов** (Next.js) · **54 миграции**
- **119 API test suites / 472 теста — зелёные** (`jest`)
- **34/34 Playwright flow — зелёные**, включая warranty и paid Service Center intake/diagnostics/customer estimate approval
...
| **1** Деньги | Payment, «нет paid без резерва», CashShift, Courier COD и provider-neutral intents готовы; live provider/fiscal reconciliation ещё внешние | 🟡 |
...
| **4** POS 2.0 | Тёмный терминал, продажа, approval, offline replay, catalog delta-sync и print abstraction; packaged-app E2E и железо не сертифицированы | 🟡 |
...
| **10A** Сервис-центр | Warranty и внешний платный intake, диагностика/смета, клиентское подтверждение, RBAC, replay и Ledger готовы; оплата/POS, запчасти, lifecycle, гарантия ремонта и подменный фонд остаются | 🟡 |
...
полный цикл платного сервиса, запчасти и подменный фонд (гарантийный и внешний платный intake, диагностика, смета и клиентское подтверждение уже готовы) · оптимизация маршрутов/live tracking (зоны, слоты, capacity, checkout и dispatch готовы) · store checklists · CMS · расширенная
...
cd apps/api && npx jest                 # 119 suites / 472 теста ✓
npm run api:build                     # ✓
