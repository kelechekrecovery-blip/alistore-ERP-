...
- **53 backend-модуля** (NestJS) · **39 веб-роутов** (Next.js) · **66 миграций**
...
- **43 Playwright flow подтверждены**, включая campaign → storefront → paid checkout → ERP ROAS, полный Marketing CMS, owner Finance settlement и Service Center.
...
| **2** Витрина | Каталог, карточка, корзина, checkout, поиск, избранное, сравнение, промо/бонусы; ERP-owned точки, scoped guest recovery и полный Marketing CMS подключены: блоки, подборки, approved-only отзывы, серверные промокоды и first/last UTM до paid campaign ROAS | 🟡 |
...
exact detail pixel-pass сервис-центра и физический UAT (полный web/API цикл, включая подменный фонд, уже готов) · оптимизация маршрутов/live tracking (зоны, слоты, capacity, checkout и dispatch готовы) · store checklists · net campaign ROI после refund и privacy-safe funnel metrics (paid gross attribution уже закрыт) · расширенная
