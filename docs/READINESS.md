...
- **95 API тест-сьютов / 336 тестов — зелёные** (`jest`)
...
| **12** Каналы и рост | Подарочные карты, Telegram shell, click&collect — готовы; франшиза/B2B/страховка — каркас | 🟡 |
...
`/support` · `/trade-in` · click&collect checkout с pickup-кодом · API: `/pos/sale` … Owner API `/reports/*` и `/ai/*` работают
...
периоды в KPI · приёмка партий UI · зарплаты · долг-напоминания · B2B/опт · страховка устройств.
...
cd apps/api && npx jest                # 95 сьютов / 336 тестов ✓
npm run api:build                     # ✓
