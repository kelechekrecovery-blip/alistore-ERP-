...
- Native iOS foundation: **4 SwiftUI application targets + AliStoreCore build**, API tests **2/2**, Client launched on iPhone 17 Pro Simulator. Feature parity and Android Kotlin remain in progress. Expo is retained only as a legacy behavior reference.
...
| **Native iOS/Android release** | Swift iOS foundation is now real; Kotlin Android and full feature parity are still required. Store release also needs Apple/Google signing, APNs/FCM credentials and physical-device QA. |
