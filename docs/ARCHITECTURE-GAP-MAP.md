...
| POS | Native target builds; staff login and sale/offline shell | Separate Compose APK with cashier-only Keystore session, live catalog/cart, server-canonical price/SKU checks, cash/card/MBank split tender, discount approval retry and isolated SQLite/WorkManager sale replay | scanner/IMEI, explicit shift UI, queued-approval recovery, receipt/hardware, refund/exchange and physical certification |
...
- All four Debug APKs build; unit tests, Android Lint and 23 Client/Staff/Courier/POS Compose UI tests pass.
...
- POS login, catalog/cart, stock-capped quantity, cash/card/MBank split tender, approval parking/retry and exact offline command retention are verified through API integration, Android JVM/Lint and API 36 Compose gates. Scanner/IMEI, shift controls, conflict recovery after queued approval, receipt printing, refund/exchange and physical hardware certification remain open.
...
1. Native parity wave: Client final provider/device smoke; Staff iOS task/push parity and Android physical certification; Courier iOS parity plus Android Evidence/push/device certification;
   POS scanner/shift/receipt/refund completion. Each flow must run against the existing Nest contracts.
