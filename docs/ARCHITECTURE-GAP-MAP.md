...
|---|---|---|---|
| Client | Native target builds and runs; live catalog, OTP account with Keychain refresh rotation, protected order history, typed API and SwiftData queue foundation | Native Compose APK builds; live catalog, typed API, Keystore and SQLite/WorkManager queue foundation | iOS cart/checkout/payment, warranty/devices, push and offline replay; Android full client parity |
