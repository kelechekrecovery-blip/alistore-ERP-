...
|---|---|---|---|
| Client | Native target builds and runs; live catalog, cart/quantity, pickup/courier checkout, JWT-owned order and payment-intent creation, card/MBank/O!Деньги/installment handoff, OTP/Keychain refresh, protected order history, owned-device warranty status and case opening, typed API and SwiftData queue foundation | Native Compose APK builds; live catalog, typed API, Keystore and SQLite/WorkManager queue foundation | iOS post-payment reconciliation UI, push and offline replay; Android full client parity |
