...
|---|---|---|---|
| Client | Native target builds and runs; live catalog, cart/quantity, pickup/courier checkout, JWT-owned order/payment intents, card/MBank/O!Деньги/installment handoff, payment-return deep link plus foreground order reconciliation, OTP/Keychain refresh, protected orders, owned-device warranty, typed API and SwiftData queue foundation | Native Compose APK builds; live catalog, typed API, Keystore and SQLite/WorkManager queue foundation | iOS push and offline replay; Android full client parity |
