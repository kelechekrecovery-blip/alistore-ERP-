...
- All four Debug APKs build; unit tests, Android Lint and Client Compose auth/checkout/order tests pass.
  Client OTP/login, process-restart session restore, cart/checkout, payment-return routing and server order status UI are verified on Android API 36.

## Execution order

1. Native parity wave: Client remaining account/self-service data; Staff operations; Courier delivery/COD;
