...
- [ ] Custom domains (каждый сервис → Settings → Custom Domains):
      `api-staging.alistore.kg` → api, `staging.alistore.kg` и
      `admin-staging.alistore.kg` → web. DNS-записи — в пункте 3 (Cloudflare).
...
- [ ] Проверка с локальной машины:
  ...
  ```bash
  WEB_BASE_URL=https://staging.alistore.kg \
  API_BASE_URL=https://api-staging.alistore.kg \
...
- [ ] dash.cloudflare.com → Add site → `alistore.kg` → выдать два NS-сервера →
...
Готово, когда: в Sentry видно тестовое событие со staging, `dig staging.alistore.kg`
...
## 6. Регистратор alistore.kg (~3 мин + ожидание делегирования)

- [ ] В панели регистратора `.kg` проверить, что домен `alistore.kg` активен и
      продлён.
- [ ] Заменить NS на два сервера из пункта 3 (Cloudflare).
- [ ] Проверить с локальной машины:

  ```bash
  dig NS alistore.kg +short        # должны вернуться NS Cloudflare
  dig staging.alistore.kg +short   # после настройки DNS — CNAME на Render
  ```
