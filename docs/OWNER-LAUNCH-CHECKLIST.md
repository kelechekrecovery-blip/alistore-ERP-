...
**Бюджет времени:** пункты 1, 2, 3, 4, 6, 7 — примерно **45–55 минут** суммарно.
Пункт 5 (Apple/Google) — отдельный трек на дни: оплата, проверка документов и
...
## 0. СЕЙЧАС: публичный контур отвечает; Render ownership ещё не подтверждён

Проверено 2026-07-23 запросами к живым адресам:

| Адрес | Ответ | Что это значит |
|---|---|---|
| `https://ali.kg/` | `200` | витрина развёрнута и отдаётся |
| `https://ali.kg/catalog` | `200` | маршрут витрины отвечает; состав каталога зависит от текущего demo-датасета |
| `https://admin.ali.kg/` | `200` | ERP entrypoint отвечает |
| `https://api.ali.kg/api/health/live` | `200` | API process отвечает |
| `https://api.ali.kg/api/health/ready` | `200` | API и обязательные runtime-зависимости отвечают |

Дополнительно подтверждено: `/api/docs` и `/api/docs-json` публично возвращают
`404`, HTTPS/HSTS, security headers и Cloudflare proxy активны. Это означает,
что текущий публичный sandbox отвечает по основным адресам. Однако локальная
проверка не доказывает, что origin уже обслуживается Render: DNS скрыт
Cloudflare proxy, поэтому ownership origin нужно подтвердить в Render и
Cloudflare Dashboard.

**Что нужно сделать владельцу для окончательной сетевой сертификации:**

- [ ] Render → сервис `alistore-api-prod` → Logs/Events: подтвердить активный
      сервис, последний SHA и отсутствие restart loop.
- [ ] Render → сервис `alistore-web-prod` → подтвердить тот же release SHA,
      что и у API; `NEXT_PUBLIC_API_BASE` должен быть production URL на этапе
      сборки.
- [ ] Render → Custom Domains: подтвердить `api.ali.kg`, `ali.kg` и
      `admin.ali.kg`; Cloudflare DNS должен указывать на выданные Render
      CNAME-targets через proxied CNAME.
- [ ] После подтверждения origin выполнить `node scripts/deployment-smoke.mjs`
      с production URL и сохранить лог в release evidence.
- [ ] Catalog data, цены и остатки загрузить владельцем в ERP; наличие HTTP
      200 само по себе не означает, что production-каталог заполнен.

**Важно:** публичный sandbox не считается production-ready. Реальные платежи,
SMS, ОФД, push, private Evidence и физическое оборудование требуют отдельной
сертификации из разделов 3–7 и не должны включаться только по результату
браузерного smoke.
...
- [ ] dash.cloudflare.com → Add site → `ali.kg` → выдать два NS-сервера →
      вписать их у регистратора (пункт 7).
...
## 4. Первый вход в ERP и учётки для ревью (~15 мин, нужен поднятый API)

Демо-сидера в проекте больше нет (удалён по вашему решению «убрать все тестовые
данные»), поэтому в свежей базе **ноль сотрудников** и войти нечем. Первый
владелец создаётся одноразовым bootstrap-эндпоинтом, дальше — только через ERP.

- [ ] Проверить, что база пустая и bootstrap ещё доступен:

  ```bash
  curl -s https://<ваш-api>/api/staff-auth/bootstrap-status
  ```

  Ожидается `{"needsBootstrap":true}`. Если `false` — персонал уже есть,
  эндпоинт закрыт навсегда, входите существующим владельцем.

- [ ] Создать первого владельца. Пароль — **минимум 8 символов**, логин — до 80;
      придумайте их в менеджере паролей и никуда не коммитьте.

  ```bash
  curl -s -X POST https://<ваш-api>/api/staff-auth/bootstrap \
    -H 'content-type: application/json' \
    -d '{"username":"<логин>","password":"<пароль>"}'
  ```

  Эндпоинт работает ровно один раз: при непустой таблице персонала он отвечает
  `staff_already_bootstrapped`. Ограничение — 5 попыток в минуту.

- [ ] Войти владельцем в ERP и завести остальных сотрудников (роли `seller`,
      `senior_seller`, `cashier`, `warehouse`, `service`, `technician`,
      `courier`, `marketer`, `admin`, `owner`). Bootstrap для них не нужен и не
      сработает.
- [ ] Завести отдельные учётки **для ревью сторов** — Staff, Courier и POS
      помечены `demoAccountRequired: true` в `apps/ios/store/*-metadata.json`,
      без рабочего логина Apple отклонит сборку. Логин/пароль вписываются в App
      Store Connect → App Review Information; в репозиторий они не попадают.

> **Отдельный блокер: клиентское приложение.** Вход покупателя — по SMS-коду, а
> в продакшене эхо кода принудительно выключено (`auth.service.ts`: `devCode`
> возвращается только при `AUTH_OTP_DEV_ECHO=true` **и** `NODE_ENV !== production`).
> Значит ревьюер Apple не сможет войти сам. Варианты, между которыми надо
> выбрать до отправки: (а) дать ревьюеру номер, SMS с которого вы читаете, и
> держать связь через App Review Notes — хрупко, ревью идёт в чужом часовом
> поясе; (б) реализовать review-only учётку с фиксированным кодом, ограниченную
> одним номером — это изменение в `apps/api`, отдельная задача с ревью
> безопасности; (в) отправить клиента с опорой на гостевой режим («Продолжить
> как гость») и явно описать в notes, что покупка требует номера KG — риск
> отказа по Guideline 2.1, поскольку часть функций ревьюеру недоступна.

Готово, когда: `bootstrap-status` отдаёт `needsBootstrap:false`, владелец входит
в ERP, учётки ревью созданы и проверены живым входом в соответствующее приложение.

---

## 5. Apple Developer ($99) и Google Play ($25) — отдельный трек на дни
...
## 6. Push-уведомления: Firebase (~10 мин)
...
Bundle/package id перечислены в пункте 5.
...
## 7. Регистратор ali.kg (~3 мин + ожидание делегирования)
