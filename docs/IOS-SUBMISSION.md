...
1. **Review-учётка через env — РЕАЛИЗОВАНО ✅.** Один заранее заданный номер
   ...
   ```
   AUTH_REVIEW_PHONE=+996XXXXXXXXX     # номер, который укажете в App Review Notes
   AUTH_REVIEW_OTP=NNNNNN              # фиксированный код для входа ревьюера
   ```
   ...
   покрыто `apps/api/test/auth-review-login.e2e-spec.ts` (вход работает только при
   точном совпадении и заданных env; иначе — обычный отказ).
