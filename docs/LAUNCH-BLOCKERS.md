...
| — каркас `FiscalModule` + честный гейт | 🟦 | P0 | M | **не начато.** `launch:check` сейчас зелёный БЕЗ фискализации — врёт |
...
| **Вход ревьюера сторов** | 🟨+🟦 | P0 | S | эхо OTP в проде выключено — ревьюер не войдёт; решить схему входа |
...
(`external-readiness.ts`). **Фискализации среди них НЕТ** — пройдёт зелёным без неё
(это и чинит каркас из Слоя 1).
