...
| Marketing CMS | `/erp`, storefront | revisioned content, ordered product collection, non-overlapping schedules, approved-only customer reviews, campaign/consent/channel primitives | collection/review API + ERP-to-storefront Playwright | Partial: collection scheduling and review moderation complete; managed promo codes and banner/block ordering remain |
