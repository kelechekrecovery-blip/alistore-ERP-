...
| Marketing CMS | `/erp`, storefront | revisioned content; ordered hero/promo/info/collection blocks; device targeting; non-overlapping hero schedules; approved-only reviews; managed promotion/redemption lifecycle; campaign/consent/channel primitives | content/block/review/promotion API + RBAC/Ledger + ERP-to-desktop/mobile/checkout Playwright | Accepted for committed Marketing CMS handoff; production media population, channel certification and campaign attribution/ROAS are launch/analytics follow-ups |
