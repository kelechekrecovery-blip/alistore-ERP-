...
| Marketing CMS | `/erp`, storefront | revisioned content, ordered product collection, non-overlapping schedules, approved-only customer reviews, managed promotion/redemption lifecycle, campaign/consent/channel primitives | collection/review/promotion API + ERP-to-storefront/checkout Playwright | Partial: collection scheduling, review moderation and server-authoritative promo codes complete; banner/block ordering remains |
