...
| Service center | `/erp`, `/pos`, `/account/devices` | warranty/paid intake, service and technician roles, diagnosis/approval, POS settlement, store-owned parts reserve/release/consume, assigned execution, SLA escalation, closure and 30-day repair warranty | concurrency/RBAC/IDOR API tests + full ERP repair browser journey | Partial: loaner custody; exact linked case-detail handoffs are absent, so detail pixel acceptance is not claimable |
