...
- `MER-001` Make the customer storefront commercially truthful and complete: remove synthetic ratings and guessed product media, add real CMS-owned merchandising/assets, implement `/about` and `/delivery`, reconcile store hours/location copy, remove the 100-product discovery ceiling and enforce server stock caps in cart. Gate: API-backed content, link crawl, catalog pagination and purchase-path visual/E2E evidence pass without fabricated claims.
