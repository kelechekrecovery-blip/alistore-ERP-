...
- `FUL-001` Make first-store fulfillment authoritative across ERP, storefront and native checkout: persisted active pickup/store point with inventory location, server checkout options, explicit guest delivery address, immutable order snapshot and point-local serialized/quantity/bundle reservation without cross-location fallback. Gate: ERP activation/deactivation immediately changes all clients; unknown point fails; guest address is exact; pickup stock is allocated only at the selected point.
