...
- `FIN-001` Add authoritative first-store Finance settlement and reconciliation across provider payments, POS shifts, courier COD and refunds: durable runs/lines, owner/admin RBAC, replay/concurrency safety, disputed variance, `payment.reconciled` Ledger events, ERP workspace and cross-surface browser/API acceptance. Gate: exact/negative/disputed/replay/rollback cases plus owner Finance E2E close with zero unexplained variance.
- `FUL-002` Persist the guest order capability and expose a scoped receipt/status link after web and Telegram checkout so a guest can reopen only their own order without OTP or an unscoped order ID. Gate: refresh/restart recovery works, expired/tampered/cross-order capabilities fail and no capability value appears in logs or analytics.
