...
Файлы в пакете — **дизайн-референсы, созданные в HTML** (`.dc.html` — самодостаточные прототипы, показывающие внешний вид и поведение). Это **не production-код для копирования**. Задача — **воссоздать эти дизайны в целевой кодовой базе** её средствами и паттернами. Рекомендованный стек (см. мастер-документ): Next.js/React (веб), React Native/Flutter (моб.), Node/NestJS или Python/Django (бэкенд), PostgreSQL + Redis + Elasticsearch, LLM/Vision API (AI-слой). Если кодовой базы нет — выбрать этот стек.
