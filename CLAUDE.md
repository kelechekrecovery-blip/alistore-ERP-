...
- **Worktree: `node_modules` симлинком, не копией.** В `git worktree` своих зависимостей нет.
  Симлинк на `node_modules` главного дерева — рабочий вариант для jest. **Копировать нельзя:**
  `cp -R node_modules` даёт дерево, на котором Nest перестаёт резолвить `Reflector`
  («can't resolve dependencies of the PermissionGuard (?, AuthzService)») и разом краснеют
  ~65 сьютов — при том что на симлинке тот же коммит даёт `210/210`. Замерено 23.07 в обе
  стороны; точный механизм не установлен, но копия воспроизводимо ломает DI, а симлинк
  воспроизводимо работает. Не принимай такой красный за регрессию кода.
  **Исключение — сборка web:** Turbopack отказывается от симлинка наружу проекта
  («Symlink … points out of the filesystem root»), и для `npm run build -w @alistore/web`
  копия как раз нужна. То есть API-тесты и web-сборку в worktree проверяй по очереди,
  в разных конфигурациях, а не одной.
