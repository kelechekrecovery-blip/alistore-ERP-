...
**Итог заходов ревью (сессия).** Четыре адверсариальных ревью денежных/auth-поверхностей iOS
→ пять реальных багов починено (CRITICAL false-success оплаты `6198b5fe`; nonce+ввод `822ff9a7`;
HIGH атрибуция POS `f444836e`; HIGH зависшая COD `476fffb7`) + три находки в бэклоге. Прод-аудиты
раньше это пропускали — ложный успех и POS-атрибуция были **pre-existing**.
