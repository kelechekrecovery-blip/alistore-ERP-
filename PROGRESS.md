...
## 2026-07-31 — витрина перестала врать про входы: способы приходят с сервера

**Что не работало.** В боевом контуре нельзя было ни войти, ни зарегистрироваться —
ни одним способом. `render.yaml` отдаёт `SMS_PROVIDER=disabled` (:244) и
`AUTH_EMAIL_LOGIN_ENABLED=false` (:207), а `APPLE_CLIENT_ID` и `TELEGRAM_BOT_TOKEN` в
блюпринте не объявлены вовсе. При этом `/login` всё равно рисовал форму телефона:
свои кнопки витрина включала по `NEXT_PUBLIC_*`, вшитым в бандл Next на этапе сборки,
и о содержимом дашборда хостинга не знала в принципе. Покупатель вводил номер, жал
«Получить код» и получал 503 — вход выглядел сломанным, хотя это конфигурация.

**Корень, который важнее флагов.** Регистрация упирается в SMS не только на телефонном
пути: `completeSocialEnrollment` (`auth.service.ts:793-799`) ищет погашенный
SMS-challenge, поэтому Apple и Telegram при мёртвом SMS остаются входом **только для
тех, у кого CustomerIdentity уже есть в базе**. Это же — причина отказа App Store
2.1(a): у ревьюера identity не было.

**Сделано.** `GET /api/auth/methods` (`auth-methods.ts`) — единственный источник правды
о живых каналах, считается тем же процессом, который потом обслужит вход. Витрина
`/login` рендерится из него: кнопки Apple/Telegram/почты появляются, когда владелец
задаёт переменную в дашборде, **без пересборки образа**; при мёртвых каналах вместо
формы-обманки показывается прямой текст и путь к гостевому заказу. Эндпоинт честно
отделяет `enabled` от `registers` и знает про мину `SMS_PROVIDER=production` —
режим проходит селектор и рапортует `ready` в preflight, но `ProductionOtpSender` не
содержит ни одного сетевого вызова.

**Починен живой баг Apple-входа на вебе.** Клиент бил в legacy `/auth/social/apple`
**без nonce**: сервер отвечал `apple_nonce_required`, а с nonce — `social_enrollment_required`
без пути дальше. Переведён на `/auth/v2/social/apple` с nonce; неизвестный Apple-аккаунт
теперь ведёт на привязку телефона тем же двухшаговым экраном, что и Telegram.

**Блюпринт.** В `render.yaml` объявлены слоты `APPLE_CLIENT_ID`, `APPLE_WEB_CLIENT_ID`,
`TELEGRAM_BOT_TOKEN`, `SMTP_*`, `STAFF_BOOTSTRAP_ENABLED` (`sync: false` — пустой слот
ничего не включает). В `.env.production.example` описаны переменные, которые код читал,
а файл, обещающий полное покрытие, умалчивал: `AUTH_EMAIL_LOGIN_ENABLED`,
`AUTH_RECOVERY_OTP_ENABLED`, `AUTH_REFRESH_ROTATION_GRACE_ENABLED`,
`AUTH_REFRESH_DERIVATION_SECRET`, `AUTH_REVIEW_*`, `STAFF_BOOTSTRAP_ENABLED`,
`TELEGRAM_ENROLLMENT_MAX_AGE_SECONDS`.

**Проверка.** Тесты написаны до реализации. `auth-methods` 17/17 (14 юнит + 3 через
реальный HTTP-стек Nest, включая проверку, что наружу не уходит ни один секрет);
веб-юниты **182/182**; полный изолированный API-гейт **256/256 сьютов, 1604/1604 теста**;
typecheck API и web; production-сборка витрины.

**Проверено в живом браузере** (сборка на свободном порту, `/auth/methods` подменялся на
клиенте): при недоступном справочнике остаётся рабочая форма телефона — деградация без
падения; при выключённых каналах вместо формы-обманки отрисовывается прямой текст; при
всех включённых видны вкладка «Почта», переключатель «Восстановить» и кнопка Apple, а
сам режим восстановления меняет заголовок и CTA на «Получить код восстановления».
**Не проверен глазами** Telegram Login Widget: `telegram.org` из окружения недоступен,
виджет всё равно не отрисовался бы. Его логика покрыта юнит-тестами сериализации.

**Playwright по затронутым поверхностям** (своя БД `alistore_authslice_test`, порты
4310/3310): `web-email-login` + `web-telegram-enrollment` 10/10, `web-route-audit`,
`return-refund`, `staff-ui` — 54 прошли, `web-checkout` 7/7, `tg-mini-app` +
`storefront-offline` 7/7.

**E2E поймал две вещи, которых не видели юнит-тесты.**
1. `web-telegram-enrollment` краснел: сценарий подменяет сервер целиком, но не подменял
   справочник входов, а витрина теперь по нему и решает, показывать ли кнопку Telegram.
   Это изменение контракта, а не подгонка теста: раньше кнопка рисовалась при любом
   `initData` и вела в отказ `social_provider_not_configured`. В сценарий добавлена
   подмена `/auth/methods`.
2. **Регрессия в чужой незакоммиченной правке чекаута.** Начальное значение поля телефона
   сменили с `''` на `'+996'`, а префилл остался `setPhone((p) => p || user.phone)` —
   непустая заготовка всегда выигрывала, и авторизованный покупатель видел «+996» вместо
   своего номера, вводя его заново. Ловилось детерминированно
   (`web-checkout` «authenticated checkout…»), воспроизведено в одиночном прогоне.
   Починено с сохранением замысла: заготовка вынесена в `PHONE_PREFIX` и уступает данным
   аккаунта, всё реально набранное неприкосновенно. Правка сделана в файле параллельной
   работы — если она конфликтует с чужим замыслом, откатывается одной строкой.
Промежуточный прогон дал одно падение `observability-status`, которое в изоляции даёт
5/5, а следующий полный прогон прошёл целиком — это известная флакующая природа общего
гейта, а не регрессия.

**Вход ревьюера сторов стал достижим (P0 из `docs/LAUNCH-BLOCKERS.md:36`).** Механизм
фиксированного кода жил только в `verifyOtp`, но дойти до него было нельзя: `requestOtp`
первым делом звал `otpSender.assertOperational()`, а при `SMS_PROVIDER=disabled` это
503 до создания вызова — при том что и iOS-клиент, и витрина показывают поле кода
только после успешного запроса. Ревьюеру физически некуда было ввести согласованный
код; это и есть отказ 2.1(a). Теперь для согласованного номера отправка и проверка
канала пропускаются, вызов создаётся. Обход опирается на прежний `reviewOtpForPhone`
(все три переменные, точное совпадение номера, окно ≤7 дней) и дополнительно ограничен
`purpose === 'login'`: восстановление доступа отзывает чужие сессии и этим ключом не
открывается. Пять тестов закрывают ровно границы обхода — чужой номер, отсутствие
переменных, истёкшее окно, recovery — и то, что реальная отправка не происходит.

**Ревью безопасности нашло регрессию, которую внесла эта же правка, — и она закрыта.**
Обход в `requestOtp` создавал ОБЫЧНЫЙ вызов: `channel` и `purpose` не задавались явно и
брали дефолты схемы (`sms`, `login`, `schema.prisma:3489,3491`). `verifyOtp` для
согласованного номера идёт своей веткой и его не трогает — но он не единственный
потребитель: `completeSocialEnrollment` (`auth.service.ts:823-839`) ищет ровно такой
непогашенный вызов и на верном коде зовёт `customerByCanonicalPhoneOnTx(..., true)`,
то есть создаёт покупателя и привязывает `CustomerIdentity`. Посторонний, знающий номер
ревьюера, мог перебирать шестизначный код (5 попыток на вызов, вызовы по 3/мин) и,
попав, привязать СВОЙ Apple-аккаунт к этому номеру — доступ пережил бы закрытие окна.
До обхода такой строки не существовало ни для одного номера. Теперь вызов обхода
рождается уже погашенным (`consumedAt` при создании): форма ответа для клиентов
сохранена, но ни один потребитель, требующий `consumedAt IS NULL`, его не возьмёт.
Тест написан как атака и **воспроизвёл эксплуатацию до правки**: привязка отвечала
`otp_invalid` (строка найдена, попытка списана — перебор имеет смысл), после правки —
`otp_not_found`.

**Оттуда же — две правки поменьше.** `APPLE_WEB_CLIENT_ID` стал единственным источником
веб-идентификатора: прежний вывод «список из одного элемента считаем веб-овым»
противоречил собственному правилу модуля, а рекомендованное одиночное значение —
`kg.alistore.client`, то есть bundle id приложения; браузерный SDK с ним получает отказ
Apple уже после клика. И витрина больше не заводит нового Apple-пользователя в тупик:
раньше после успешного входа у Apple экран переключался на ввод номера, submit которого
навсегда заблокирован при мёртвом SMS, — теперь показывается прямой текст, что первая
привязка требует SMS.

**Восстановление доступа получило интерфейс.** `authRequestRecoveryOtp` /
`authVerifyRecoveryOtp` были обёрнуты в контекст и не вызывались ниоткуда, а переводы под
них уже лежали готовыми и мёртвыми (`login.title.recovery`, `login.mode.recover`,
`login.cta.recovery`, `login.code.recover` — обе локали). Добавлен переключатель
«Войти / Восстановить» на телефонном канале, видимый только когда сервер говорит
`recovery.enabled` (флаг rollout И живой SMS одновременно). Причина отказа больше не
глушится общей фразой — `recovery_temporarily_unavailable` доходит до человека.

**Закрыты ещё два тупика витрины.** `/approvals` был единственным из 14 staff-экранов со
своей копией формы входа и потому без bootstrap-ветки: на пустой базе он показывал вечную
«Ошибка входа», тогда как остальные 13 предлагают создать первого владельца. Переведён на
общий `StaffSessionLogin`. Мобильный шелл `components/mobile/**` не содержал НИ ОДНОЙ
точки входа — добавлена ссылка «Войти» в общий хедер `MobileFrame` с безопасным `next`
(проверка на `//` вынесена в чистую функцию и покрыта 11 тестами).

**Хвосты ревью безопасности закрыты (четыре штуки).** `GET /auth/methods` отдаёт
`Cache-Control: no-store` — это не гигиена: между витриной и API стоит CDN, и
закешированный ответ пережил бы включение канала владельцем, оставив экран входа со
словами «входов нет» до истечения чужого кеша. Код ревьюера сверяется
`timingSafeEqual` (единственное место, где секрет сравнивался в открытую — у обычных
OTP сравнение прячет argon2). Выдача вызова обходом пишется в леджер
(`auth.review_login_challenge_issued`, номер — хешем): раньше факт использования обхода
не оставлял следа вообще. И эхо кода для согласованного номера подавлено даже вне
production — там генерируется код, который заведомо не сработает, потому что `verifyOtp`
уходит в ветку ревью; тестировщик ловил бы «код не подходит» на технически валидном коде.

**Telegram-вход появился в обычном браузере.** Сервер умел `source: 'login_widget'` и
проверял его подпись (SHA-256 от токена вместо HMAC от `'WebAppData'`), но витрина всегда
слала `'mini_app'`, а кнопка рендерилась только при наличии `window.Telegram.WebApp.initData` —
то есть из десктопного или мобильного браузера вход был недостижим. Добавлен Login Widget:
`/auth/methods` отдаёт `telegram.botUsername` (публичный хендл, нужен атрибуту
`data-telegram-login`; токен наружу не выходит), витрина монтирует скрипт Telegram и
сериализует его ответ в ту же query-строку, что присылает Mini App. Сериализация вынесена
в `lib/telegram-widget.ts` и покрыта тестами на то, что ломает подпись: потерянный ключ,
добавленный ключ, `null` вместо отсутствия поля, пустая строка как значение. По тому же
правилу, что и с Apple, имя бота не выводится ниоткуда: нет `TELEGRAM_BOT_USERNAME` — нет
виджета, при этом вход из Mini App продолжает работать.

**Telegram Mini App перестала быть анонимной.** `/tg` читала только `initDataUnsafe` —
данные для предзаполнения, которые клиент волен подделать, — и всегда шла гостевым путём:
телефон оставался введённым руками, а заказ — заказом без сессии. Теперь страница берёт
подписанный `initData` и входит тем же маршрутом, что `/login`; при успехе телефон
подставляется из аккаунта (он проверен кодом), а заказ и платёжное намерение создаются
сессией покупателя (`createMyOrder` / `createMyPaymentIntent`) — то есть видны в кабинете
с любого устройства, а не только по гостевой ссылке в этом браузере. Отказ ничего не
ломает: бот может быть не настроен, а у нового человека нет привязанного телефона —
гостевая ветка работает как раньше. Попутно исправлен тип `createMyOrder`: он объявлял
`'web' | 'mobile'`, хотя сервер принимает те же каналы, что и гостевой `createOrder`
(`CreateMyOrderDto` наследует ту же валидацию) — из-за этого недосмотра Mini App не могла
оформить заказ сессией, хотя API это разрешал.

**Лимит `/auth/methods` теперь действительно проверяется.** Соседний сьют поднимал
троттлер с `limit: 100`, поэтому декоратор `@Throttle({limit: 60})` был в нём
ненаблюдаем — удали его, и всё осталось бы зелёным. Новый `auth-methods-throttle.e2e-spec.ts`
держит дефолт щедрым и проверяет 60 проходов и 429 на 61-м. Тест прогнан мутационно:
с удалённым декоратором он краснеет, с восстановленным — зелёный.

**Принятый и незакрытый риск.** `/auth/otp/request` стал оракулом на `AUTH_REVIEW_PHONE`:
согласованный номер отвечает 200, любой другой — 503. Убрать нельзя, не убрав сам смысл
фикса. Более дешёвый оракул на тот же секрет существовал и раньше в `/auth/otp/verify`
(10/мин против 3/мин, `otp_invalid` против `otp_not_found`). Практическое следствие —
не захват (он закрыт выше), а отказ в обслуживании: найдя номер, посторонний держит вход
ревьюера в локе 15 минут циклически. Отсюда правило: `AUTH_REVIEW_PHONE` — одноразовый
номер, выдаётся перед самым Submit и гасится сразу после ревью.

**Осталось за владельцем — без этого вход в проде не оживёт ничем в коде.** SMS-канал:
либо договор с оператором KG, либо мост через Android-телефон
(`SMS_PROVIDER=android_gateway` + четыре `SMS_GATEWAY_*`). Всё остальное — Apple,
Telegram, почта — регистрацию нового человека без него не открывает. Исключение ровно
одно и временное: `AUTH_REVIEW_*` пускает один согласованный номер на срок до семи
дней — это путь для ревью сторов, а не для покупателей.

## 2026-07-26 (7) — второй заход охоты: N+1 на первом экране главной

**Что проверил и нашёл здоровым.** Продолжил охоту за классом «интерфейс сообщает о
состоянии, которого нет», уже на невеб-поверхностях. Swift-клиент оказался аккуратным:
отказ каталога не подменяется выдумкой — `remoteProducts ?? localFallbackProducts` берёт
**уже загруженный** каталог приложения (не фикстуры) и рядом показывает
«Офлайн-каталог: <причина>»; самовывоз различает «точек нет» и «не смогли спросить».
Типы веб-хелперов под 202-эндпоинтами тоже честные: `RefundAggregate` моделирует заявку
полем `approvalId`, `schedule`/`cancel-schedule` действительно возвращают ревизию.
Это результат, а не пустая проверка: класс, давший три дефекта в вебе, на клиентах не
воспроизвёлся.

**Найденный дефект (`32377843`).** `/storefront-blocks/public` рендерит **первый экран
главной** и звал `catalog.curated()` по разу на каждую подборку. `curated()` — это два
запроса (товары + агрегат отзывов), то есть N подборок стоили 2N round-trip'ов на каждый
показ витрины, на странице, где задержка виднее всего. Теперь один батч на объединение всех
id, с раскладкой обратно по блокам.

**Батч тут легко испортить незаметно, поэтому оба способа закрыты тестом, написанным до
правки и зелёным на старом коде:** у каждой подборки **свой** порядок товаров (он живёт в
её `productIds`, и общий батч не вправе его усреднить) и товары не протекают между блоками.
Тест берёт две подборки с разным порядком, пересекающиеся по одному товару, — это ровно тот
случай, на котором наивная общая сортировка падает.

**Проверка.** Полный изолированный API-гейт **221/221 сьютов, 1314/1314 тестов**;
`storefront-blocks` 4/4 до и после; e2e `storefront-cms-ui` 5/5; typecheck.

## 2026-07-26 (6) — охота на дефекты: два найденных, один инвариант закрыт тестом

**Что искал.** Самый урожайный класс этой сессии — «интерфейс сообщает о состоянии, которого
нет». Прошёл по нему системно, а не наугад.

**Проверено и оказалось здоровым (важный результат сам по себе).** Все семь маршрутов,
паркующих заявку на согласование, и их UI: цена (`'applied' in result` — различает применённое
и заявку), архивация товара («Архивация ждёт approval»), долг (`isDebtApproval`), склад
(`'approvalId' in result`), блоки витрины (публикуются напрямую, не через согласование).
То есть баг публикации витрины из прошлого захода был выбросом, а не системой.

**Инвариант, который держался на внимательности (`95444bac`).** Каждое опасное действие
обязано что-то делать при одобрении. Иначе худший исход: одобряющий подтверждает списание или
удаление, заявка становится `approved`, событие уходит в леджер — а эффекта нет. Все следы
говорят, что операция была. Проверки не существовало, и это легко пропустить: `delete`
регистрируется как `delete: del` (имя зарезервировано), поэтому поиск по сокращённой записи
его теряет — я сам сначала прочитал список как «четыре действия без исполнителя».
Теперь три действительно безысполнительных действия — явный список с причиной каждого
(`discount` потребляет POS-продажа через `consumedAt`, `pii` — сам грант, `exchange` идёт
отдельной веткой), и тест держит список честным в обе стороны. **Проверено, что тест кусается:**
добавил гейтед-действие без исполнителя — сьют падает и называет его; откатил.

**Найденный дефект (`016f074a`).** `catalog.ts` подробно описывает правило: вызывающий обязан
отличать «ничего нет» от «не смогли спросить» — там записано, что покупатель однажды видел
пустой магазин вместо сообщения о сбое. Загрузка **категорий** это правило нарушала, а страница
каталога добавляла сверху второй `.catch(() => [])` — в двух строках под комментарием,
запрещающим подменять отказ пустотой. При недоступном эндпоинте покупатель получал каталог без
фильтров, без признака неполадки и без записи в лог. Теперь `null` + лог; рендер не тронут
(ронять каталог из-за фильтров нельзя), но различие есть в типе, и три места обязаны были
выбрать. Клиентские два теперь не затирают уже показанные категории пустым списком.

**Проверка.** API-сьюты 14/14 по затронутым, web 120/120, typecheck обеих сборок,
production build, `check-no-fixtures`, e2e по каталогу/витрине/чекауту **18/18**.

## 2026-07-26 (5) — Android проверен, и один предикат вместо двадцати одного

**Android — единственная поверхность экосистемы, которую я в этой сессии не трогал.**
`npm run android:test` (юниты + lint по четырём модулям): **BUILD SUCCESSFUL**, exit 0.
То есть Android-часть экосистемы здорова на текущем дереве.

**Гонка на денежных маршрутах решалась двадцатью одним разным предикатом (`04fa7974`).**
Проверка «это нарушение уникального индекса?» была написана заново в 21 месте — payments,
procurement, finance, orders, courier, auth, promotions, tradeins, warranty, service-center,
customers, approvals — в двух несовместимых формах: `instanceof PrismaClientKnownRequestError`
в большинстве и утиная типизация по `code` в пяти. От этой проверки зависит, станет ли гонка
двух одинаковых запросов **успешным реплеем** или ошибкой пользователю. Расхождение означало,
что один маршрут восстанавливается после гонки, а соседний нет, и увидеть это можно было
только под нагрузкой.

**Объединил формы, а не выбрал одну — и это важно.** `instanceof` точнее, но ломается, если
ошибка пересекла границу модульного realm (две копии `@prisma/client`): прототип теряется,
`code` остаётся. Утиная типизация это переживает, но поймала бы любой объект с таким полем.
Общий предикат принимает обе формы и строго сверяет значение — поэтому **ни одно место не
сузило покрытие**. Выбери я «правильную» форму, часть маршрутов молча потеряла бы защиту.

**Проверка.** Новый сьют 4/4 (настоящая ошибка Prisma, ошибка без прототипа, другой код
Prisma, мусор), полный изолированный API-гейт **220/220 сьютов, 1309/1309 тестов**, typecheck
чистый, сырых сравнений с `P2002` вне хелпера не осталось.

## 2026-07-26 (4) — витрина: маркетологу больше не сообщают о публикации, которой не было

**Настоящий дефект продукта (`2ee06da6`).** Публикация витрины закрыта правилом четырёх
глаз намеренно — в контроллере записано, почему: одиночный POST маркетолога менял главную
всем покупателям. Эндпоинт отвечает `202` и **паркует заявку**, ревизия остаётся `draft`.
А ERP писала «Версия vN опубликована», потому что её `run()` печатал фиксированную строку
успеха на любой не-бросивший ответ. Маркетолог правил главную, получал «опубликовано» и
уходил — витрина не менялась.

Доказано по базе, а не по рассуждению: после прогона ревизия `draft` с верным заголовком и
двумя товарами, а витрина отдаёт дефолтные «Популярное» и ноль товаров.
Заодно `publishStorefrontRevision` был типизирован `Promise<StorefrontContent>` — он никогда
такого не возвращал. Теперь возвращает конверт заявки, а текст решает `storefrontPublishNotice`
по фактическому ответу и называет номер заявки, чтобы её можно было найти. Тип — то, что не
даст вернуть эту ложь молча.

**Метод важнее фикса: первая гипотеза была неверной.** Я предполагал, что `HomeClient`
перетирает серверные блоки. Временный серверный лог опроверг это за один прогон — блоков
ноль, и дело вообще не в блоках, а в контенте витрины. Без инструментирования я бы «чинил»
не то место.

**Второе падение — дефект теста, не продукта (`1de2b0b5`).** `getByText` по имени кампании
ловил strict mode: имя законно рендерится дважды — в карточке списка и в `<option>` списка
сверки расходов. Ассерт привязан к карточке через уже существующий `campaign-link-<id>`.

**Устаревший тикет снят (`e8f3e02b`).** `UI-SHIFT-HANDOVER-001` утверждал, что экран передачи
смены завести нельзя: кассир без `staff:manage` не резолвит получателя. Блокер снят
отдельным эндпоинтом `GET /shifts/handover-targets` под правом `shift:handover`, и UI на нём
уже работает. Открытый тикет сбивал бы с толку следующего.

**Проверка.** Полный `npm run e2e` на изолированной БД и портах: **140/141**. Оба
исправленных спека зелёные (`storefront-cms-ui` 5/5, `web-checkout` 7/7). Единственное
падение — `storefront-compare-ui`, и оно **изолированно проходит 1/1**, то есть флак, а не
дефект: ровно то различие, которое CLAUDE.md велит проверять повтором. Web-юниты 116/116,
оба typecheck, публичный сайт 11/11 HTTP 200, `api.ali.kg` health и каталог 200.

## 2026-07-26 (3) — идемпотентность списания, дедуп очереди заказов, предупреждение курьеру

**Двойное списание со склада (`c31ef04a`).** `POST /inventory/movements` не был защищён от
повтора. Списание одобряется, а не применяется сразу, поэтому дубль не двигал сток сам — он
парковал **вторую заявку на одно физическое списание**. Заявки идентичны по построению, и
одобривший обе списывал сток дважды, причём второе списание неотличимо от настоящего. Между
двойным тапом и этим стоял только клиентский `isSubmitting`.

`Approval.idempotencyKey` уже был объявлен уникальным в схеме — его просто никогда не
заполняли. Теперь `approvals.request()` принимает необязательный ключ: повтор переигрывает
первую заявку, а гонку двух одновременных нажатий решает **уникальный индекс, а не чтение**.
Заявку под тем же ключом, которую уже решили, сознательно **не** выдаём за `requested` —
это была бы та же ложь о состоянии, которую я весь день чиню; она даёт
`approval_already_decided`. Ключ оставлен необязательным намеренно: сборка 4 сейчас на ревью
и заголовок не шлёт, обязательность сломала бы версию под ревью. iOS-сторона шлёт ключ от
содержимого (паттерн сдачи COD).

**`POST /inventory/count` не трогал — и это осознанно.** Он перечитывает `expected` перед
расчётом diff, поэтому повтор даёт diff 0 и охранник пропускает запись баланса: он
самокорректируется. Дубль добавляет только нулевое движение — шум, а не ошибка стока.

**Очередь заказов (`319bd1e5`).** Кассовая и курьерская очереди делят коллизию ключа на два
случая: точный повтор — идемпотентный no-op, другое тело под тем же ключом — громкий отказ.
Покупательская вставляла безусловно, то есть была единственной очередью, где
переиспользованный ключ молча давал две записи — а запись там это покупка. Приведена к тому
же поведению.

**Курьер терял правку суммы (`a2f8f06b`).** Дедуп двойного тапа был закрыт раньше, но остаток
жил: курьер, правящий COD, пока по заказу висит неотправленная команда, терял правку — сервер
разрешает статус через CAS, первая (возможно устаревшая) сумма выигрывает, а поздняя верная
умирает конфликтом, которого курьер не видит. Карточка доставки теперь предупреждает до
отправки. Сверка по сегментам пути, не подстрокой: `order-1` не должен совпасть с `order-10`.
**Поймано сборкой приложения, а не сьютом:** `ios:test` собирает только `AliStoreCore`,
поэтому `pending` вне области видимости в `CourierRouteView` там компилировалось молча.

**FLAKE-001 не воспроизвёлся.** Пять подряд изолированных прогонов: четыре чистых
(`218/218`+`1299`, затем трижды `219/219`+`1304`). Пятый «красный» — моя же правка на лету,
не флак. При заявленных ~40% вероятность четырёх чистых подряд ≈ 13%: это **умеренное**
свидетельство снижения частоты, не доказательство отсутствия. Тикет оставлен открытым.

**Проверка.** Полный изолированный API-гейт **219/219 сьютов, 1304/1304 тестов**;
iOS **143/143**; `AliStoreCourier` собирается и стартует на iPhone 17 Pro; API typecheck
чистый. Экран предупреждения курьера глазами не проверен — нужен аутентифицированный курьер
с командой в очереди.

**Про App Store.** Эти правки поедут в **сборку 5**, после того как завершится ревью сборки 4.
Трогать версию под ревью новой заливкой нельзя — она сбросит ревью.
...
## AUDIT-WAVE2-106-2026-07-27
- Task: волна 2 аудита (`docs/AUDIT-CONTINUATION-PROMPT.md` + SUPER PROMPT wave2). Ветка `fix/audit-wave2` от `fix/audit-2026-07` (несёт 9 фиксов прошлой сессии). Порядок: verified-баги первыми, тест до фикса.
- Правило снова оправдалось: два корня из SUPER PROMPT (F-03, F-15*) точны и подтверждены по коду — это настоящие баги, которых прошлый проход не нашёл, потому что не бил по нужному пути.
- Закрыто фиксом + зелёным тестом:
  - **F-03** (`376d3ad9`) — payroll 500. `Payment.receivedBy` несёт `staff:<id>` (POS, `payments.controller.ts:113`) или `<customerId>` (`payForCustomer`); оба текли в `HrPayrollLine.staffId` (FK→StaffUser) → 500. Фикс: `soldBy()` снимает префикс `staff:`, а consumers (`hr.service`, `reports.service`) держат только реальных StaffUser — призрак не начисляется. Заменяет прошлый pin `07bc92eb`. Тест `hr-payroll-actor-prefix.e2e-spec.ts` + правка фикстуры `reports.e2e`.
  - **F-15*** (`7884ef46`) — generic `transition()` ставил `reserved` мимо `reserve()`, помечая заказ зарезервированным без стока (взрыв позже на picking). Фикс как у `paid`: ребро в state machine остаётся (нужно `reserve()`), но generic-переход отказывает `reserved` → 422 `order_reserve_requires_service`. Заменяет прошлый message-only `d8d054c5`. Тест `order-reserve-guard.e2e-spec.ts`.
  - **F-17** (`9439c4b1`) — наличный возврат авто-резолвит открытую смену исполнителя при пустом `shiftId`; ноль/несколько → 422 `cash_refund_shift_required`. Текст «инициатора» → «исполнителя», OpenAPI обновлён. Тесты в `refund-aggregate.e2e`.
  - **F-19** (`58cb9ab2`) — политика паролей персонала: ≥12, ≥3 класса, deny-list; 422 `password_too_weak`. В контроллере (bootstrap/create/reset), не в сервисе — чтобы прямые сервис-вызовы в тестах не ломались и политика жила на границе API. Существующие учётки не затронуты. Тест `password-policy.spec.ts` + HTTP-кейс; укреплены 4 фикстуры с слабыми паролями.
- Уже сделано в прошлую сессию (не переделывалось): S-04 (`1e3f9baf`, флаг назван `STAFF_BOOTSTRAP_ENABLED`, не `BOOTSTRAP_ENABLED` — имя яснее), S-05, S-07, S-08, F-07, F-08, F-01a, F-02, F-14.
- Отклонено/отложено с обоснованием:
  - **F-14b** (enforce-2FA) — отложено. Промпт сам ставит его ПОСЛЕ деплоя D-02; меняет контракт логина сотрудника (POS/мобайл должны понимать enrollment-challenge — кэшируют схемы) и трогает глобальную JWT-стратегию. Env-gated-off, включить нельзя без клиентских правок. Спешно делать security-чувствительный auth-путь хуже честного «не сделано». План: guard-подход (обычный staff-токен + Enforce2faGuard, блокирующий все staff-маршруты кроме `/2fa/*` и `/me` при `STAFF_2FA_ENFORCE=true` и непривилегированном enrolled), без нового typ токена.
  - **D-03** (production-preflight fail-fast) — не трогал: `apps/api/src/health/production-preflight.ts` в рабочем дереве имеет чужие незакоммиченные правки (Codex-зона), правка сметёт их (COMMIT-HYGIENE-182).
- OWNER-BLOCKER (код без них не закрыть):
  - **D-01** — remote УЖЕ есть (`origin git@github.com:kelechekrecovery-blip/alistore-ERP-.git`), `.gitleaks.toml` есть; премисса промпта «remote отсутствует» неверна. Push веток + gitleaks по истории + launchd-бандл — за владельцем (push = внешняя публикация; при находке секретов — стоп по правилу).
  - **D-02** — деплой F-02/F-14 в прод. Прод в не-production режиме (перепроверено: `/api/metrics` отдаёт 200 без токена, `/api/docs` закрыт). Флип прода + план TOTP-секретов — за владельцем.
  - **S-03** (email-провайдер + DNS), **F-06** (миграция serialized, dry-run), **F-20** (роли, dry-run), **C-01** (чистка тест-данных, DELETE на проде) — миграции/данные/внешние учётки, только с подтверждением владельца.
- Гейт: полный изолированный API — **234 сьюта, 1378 тестов зелёные, без флаков в этом прогоне**. e2e — прогон на изолированной БД `alistore_e2e_wave2_test` (порты 4320/3320).

## IOS-SUBMISSION-PREP-107-2026-07-27
- Task: «выпустите обновление в appstore». Снял живое состояние ASC по API (не по докам — они врали).
- Факт на 27.07: все четыре (Client/Staff/Courier/POS) — `1.0.0 PREPARE_FOR_SUBMISSION`; сборки 4 VALID и НЕ истекли, но были НЕ привязаны к версии; reviewSubmission ни у одного нет. Нового iOS-бинарника собирать нечего — правки аудита были бэкендом, не кодом приложений.
- Сделал (обратимая подготовка, ревью НЕ запускал): прикрепил сборку 4 к версии 1.0.0 у всех четырёх (ASC API, read-back подтвердил). Метаданные (категория + content rights) уже совпадали — `ios:store-metadata` dry-run показал «No changes needed».
- Submit НЕ нажимал — обоснованно:
...
## IOS-REVIEW-CATALOG-109-2026-07-27
- Task: продолжение «сделайте» — довёл витрину до пригодной для ревью (Guideline 2.1) и убрал аудит-мусор из вида ревьюера. Всё на живой `alistore_dev` (обслуживает публичный api.ali.kg через туннель).
- Мусор аудита скрыт: `TEST-AUDIT-1`, `TEST-SER-1`, `XSS-AUDIT` (в т.ч. товар с именем `<script>alert(1)</script>…`) переведены в `archived=true` — designed soft-delete, обратимо (не DELETE, FK не тронуты). Каталог DB-only (Meili не настроен), поэтому фильтр `archived:false` скрыл их сразу; проверено — в `/catalog/products` их нет.
- Витрина заполнена штатно: `scripts/seed-review-data.mjs --apply` через реальный `POST /inventory/receive-quantity` (Event Ledger, idempotency) добил остатки трём реальным товарам (MacBook Air, iPhone 17, iPhone 17 Pro Max) до 3 шт. Итог: **4/4 товара покупаемы** (APP3 уже был). Ревьюер сможет пройти покупку.
- Для сида создал временную учётку `prov_warehouse_*` (роль warehouse — единственная с `inventory:receive` из безопасных), после сида деактивировал (`active=false`). Пароль нигде не сохранён.
- Ранее (108): три учётки ревью seller/courier/cashier активны и проверены входом.
- Что ТЕПЕРЬ реально закрыто для ревью: демо-входы (staff/courier/pos), витрина непустая, аудит-мусор скрыт, сборка 4 привязана, метаданные применены.
- Остаётся только владельцу (мне запрещено/недоступно): NODE_ENV=production+деплой (рестарт публичного процесса — риск простоя, нужен полный prod-env); в ASC вставить демо-пары + Client env перед Submit + App Privacy (docs/APP-STORE-REVIEW-HANDOFF.md); нажать Submit.

## SUPPLY-TO-ORDER-SLICE-3-2026-07-29 — заказ поставщику под клиентскую строку
- Task: срез 3 `docs/SUPPLY-TO-ORDER-PLAN.md` — связка клиентской строки заказа с закупкой у поставщика + автомат её поставки + re-key гварда `assertNoToOrderLine`.
- Схема: новая модель `OrderLineSupply` (миграция `20260729090000_order_line_supply`) — `orderItemId` уникален (одна запись на строку, это и есть unique-constraint из 3a, а не check-then-insert), `purchaseOrderItemId` уникален и NULL до размещения PO, CHECK в БД запрещает статус дальше `awaiting_supplier`/`cancelled` без `purchaseOrderItemId`. Прямого FK `Order → PurchaseOrder` нет намеренно — один PO поставщику аггрегирует строки нескольких клиентских заказов.
- Автомат `src/procurement/order-line-supply-state.ts` (чистый модуль, зеркало `orders/order-state-machine.ts`): `awaiting_supplier → ordered → in_transit → received → handed_over`, `cancelled` из любого нетерминального состояния.
- `OrderLineSupplyService` (`src/procurement/order-line-supply.service.ts`): `pg_advisory_xact_lock` по customer orderId (паттерн `store-operations.service.ts:111`) + compare-and-swap `updateMany({where:{orderItemId, status: previous}})` на переход — двойной клик двух менеджеров даёт один `PurchaseOrderItem` (проверено конкурентным тестом). RBAC — переиспользованы существующие `procurement:{create,send,receive,cancel}`, отдельный ресурс не заводился. Роуты `POST /orders/:orderId/items/:orderItemId/supply/{order,ship,receive,handover,cancel}`, контроллер и сервис зарегистрированы в `procurement.module.ts` (не в `app.module.ts` — он уже импортирует `ProcurementModule`).
- Re-key нагруженной части: `assertNoToOrderLine` (ключ — `Product.supplyMode`) → `assertOrderLineSupplyReceived` (ключ — статус `OrderLineSupply` конкретной строки; `received`/`handed_over` пропускают). Все четыре входа обновлены синхронно в одном коммите: `orders.service.ts` (reserve, fulfill), `payments.service.ts` (pay), `courier.service.ts` (COD при доставке — гвард здесь был непроверяем ранее, добавлен тест, путь стал реально достижим).
- `OrdersService.create`: заказ `to_order` заводит `OrderLineSupply` на каждую строку сразу при создании (`awaiting_supplier`) — гейт «не смешивать to_order и own_stock в одном заказе» (срез 2) гарантирует, что весь список строк либо целиком to_order, либо нет.
- Тесты: `test/order-line-supply-state.spec.ts` (чистый автомат), `test/order-line-supply.e2e-spec.ts` (17 кейсов: авто-создание записи, идемпотентность размещения PO под конкуренцией, нелегальный переход, отмена из ordered/in_transit, блок продажи на awaiting_supplier/ordered/in_transit, полный путь reserve→pay→списание после received, COD-гвард в courier.service.ts). Побочный фикс: `test/order-to-order-request.e2e-spec.ts` и мой файл чистили `orderItem` до `orderLineSupply` (новый FK Restrict) — поймано и исправлено кросс-файловой контаминацией в общем прогоне, не в одиночном.
- Гейт: `tsc --noEmit` чисто (exit 0). `api:test:isolated` по `orders|payments|courier|quantity-inventory|purchase|procurement|inventory|order-to-order` — 24 сьюта / 151 тест, дважды подряд зелёные. Полный `api:test:isolated`: первый прогон 2 красных в несвязанных сьютах (`accounting-ar-aging` — FK-гонка в общем прогоне, `outbox.e2e-spec` — таймингова флака в файле, который правит другой инструмент, не тронут мной), повтор в одиночку — оба зелёные; второй полный прогон — **239/239 сьютов, 1421/1421 тестов**, без красных. Не трогал `app.module.ts`/`production-preflight.ts`/`src/outbox/**`/`runtime-env-files.ts` (зона другого инструмента).
- Осталось за пределами среза (не делал, не просили): контроллер/UI для просмотра очереди «под заказ» персоналом, автоматическое создание `OrderLineSupply` через `receive()` в `ProcurementService` (сейчас размещение PO и обычная приёмка склада — независимые действия, реальный сток на `received` должен появиться через штатный `inventory.receiveQuantity`/`procurement.receive`, как в e2e-тесте).

## OUTAGE-WEB-500-2026-07-30 — витрина отдавала 500 на каждый маршрут шесть часов
- Симптом: `https://ali.kg/` и все её маршруты — HTTP 500 с телом `Internal Server Error` (21 байт, без `content-type`), включая `/healthz`. `api.ali.kg/api/health` при этом 200: API был жив, лежала только витрина.
- Причина: Turbopack кладёт внешние пакеты Sentry в `apps/web/.next/node_modules` симлинками на **раскладку пакетного менеджера, с которым шла сборка** — `../../../../node_modules/.pnpm/<пакет>@<версия>/node_modules/<пакет>`. Сборка сделана в 16:52, в 21:05 зависимости переустановили через npm, `node_modules/.pnpm` исчез, обе ссылки повисли. Next падает на `instrumentation.js` до обработки запроса, поэтому 500 отдаётся на любой путь, а `/_next/static/*` продолжает отвечать 404 с edge — отсюда ложное впечатление, что «часть сайта работает».
- Диагностика на будущее: `ls -la apps/web/.next/node_modules` (битые ссылки видно сразу) и `/tmp/alistore-web-prod.err.log` (`Failed to load external module require-in-the-middle-<хэш>`). В репозитории лежат **оба** лок-файла (`package-lock.json` и `pnpm-lock.yaml`) — это и есть источник расхождения.
- Почему не поднялось само: сторож умел только `launchctl kickstart`. Против сломанного артефакта рестарт бессилен — в журнале 256 перезапусков витрины подряд без единой строки диагноза.
- Fix: ссылки перепривязаны к плоской npm-раскладке, витрина перезапущена; `ali.kg`, `www`, `admin`, `api` и `/catalog` отвечают 200.
- Сторож (`scripts/keep-site-up.mjs`, коммиты `b2f40a52` + `7b358e37`): `repairDanglingExternals()` чинит такие ссылки по имени пакета (basename цели), проверка идёт **каждый цикл** первым делом и при успешном ремонте сразу перезапускает витрину; после 10 неудачных рестартов подряд — ЭСКАЛАЦИЯ в журнал и уведомление вместо молчаливого цикла; проба витрины переведена с `/` на `/healthz` (по `/` сторож перезапускал исправную витрину при мёртвом API); напоминание про sleep пишется раз в час (две строки в минуту занимали 7778 строк журнала из 9015). Тесты `scripts/__tests__/keep-site-up.test.mjs` — 4/4, `npm run site:keepalive:test`.
- Учение на боевом контуре 23:11: ссылки сломаны ровно так же, витрина перезапущена и легла; один цикл сторожа вернул её за 2 секунды (`РЕМОНТ: … перепривязаны` → `витрина перезапускается после ремонта сборки`), после чего `/healthz` и публичный `ali.kg` — 200. Полная длительность учения 44 с.
- Замер доступности за 25–30.07 (журнал `/tmp/alistore-keepalive.log`, окно 131ч47м): публичный `ali.kg` — **57,6 %** (55ч50м простоя, 117 падений, максимум 12ч04м), `api.ali.kg` — 54,6 %; при этом локальные процессы лежали лишь 13ч43м и 7ч22м. Разница ≈42ч — сон и сеть машины, а не отказы кода.
- Открытые блокеры владельца (не мои): `sudo pmset -a disablesleep 1` (нужен пароль; `caffeinate` держит только idle-sleep, от закрытой крышки не спасает — 3889 предупреждений сторожа за окно); создать сервисы из `render.yaml` (`*.onrender.com` отвечает `no-server` — production так и не создан) и увести DNS с туннеля. Пока боевой origin — этот ноутбук, потолок доступности задаётся им, а не сторожем.
- Причина простоев уточнена по `pmset -g log` (25–30.07): машина спала **72ч03м из 131ч** — это и есть недостающая доступность. В разбивке по «причинам» лидируют `Maintenance Sleep` (622 эпизода, 37ч25м) и `Sleep Service Back to Sleep` (182, 32ч07м), но это НЕ самостоятельные причины: так дышит уже спящая машина между dark wake. Настоящий триггер — `Clamshell Sleep`, **19 закрытий крышки** за окно. Прослежено на самом долгом простое: 27.07 19:27:10 `Clamshell Sleep` → далее всю ночь `DarkWake → Maintenance Sleep` по кругу `Using Batt (Charge:52%)` → простой закончился 28.07 10:54, когда крышку открыли. Отсюда же второй фактор: в тот момент ноутбук был НЕ в зарядке. `caffeinate` (агент `com.alistore.nosleep`, держит ассерты 131ч подряд) от закрытой крышки не спасает — это подтверждено: ассерты были активны всё окно, а машина всё равно спала 72 часа.
- Функциональная проверка после починки: `E2E_BROWSERS=chromium npx playwright test --config playwright.prod-smoke.config.ts` — **51/51 зелёных**, включая `catalog lists products and a product page loads`, гейтинг приватных маршрутов, `/healthz`, `sitemap.xml`, apple-app-site-association и a11y-пробы шести страниц. То есть сайт не просто отвечает 200, а работает.

## DEPLOY-2026-08-06 — AliStore Business выкачен в прод
- Что выкачено на `codex/production-auth-activation` (боевой чекаут `~/.codex/deployments/alistore-production-auth`): кабинет партнёра AliStore Business целиком (модели `Seller`/`SellerUser`, `Product.sellerId`, отдельный контур авторизации `typ:'seller'` со своим `BusinessAuthGuard`, онбординг магазина из ERP), публичная оферта и текстовые настройки, фиксы контраста и доступности, фолбэк фото товара.
- Перенос патчами, а не мержем: main и ветка прода расходятся на содержимое самой `production-auth-activation` (auth/telegram/staff-auth), поэтому `git format-patch | git am --3way` по границе последнего переноса. Два конфликта, оба разрешены по фактам, а не по «своё/чужое»:
  - `e2e/global-setup.ts` — входящий патч менял `migrate reset` на `db push`. Оставлена сторона ветки: `db push` создаёт схему с 0 из 33 триггеров, которые ставят 7 миграций кастомным SQL (замерено, CLAUDE.md).
  - `ProductCard.tsx` — `text-white/48` против `/45`. Взята входящая: `/48` нет в шкале Tailwind, правило не компилируется вовсе. Следующий патч довёл до `/50`. Проверено в собранном бандле: `/45` и `/48` — 0 правил, `/50` — 84.
- Миграции на проде уже были применены ранее, `migrate status` — «up to date». Данные целы: 7 товаров, 9 заказов до и после.
- Дымовой прогон боевой сборки шёл на **копии** `alistore_dev` (`pg_dump | psql`, 24 МБ): production-preflight запрещает старт с выключенными воркерами, а гонять реле очереди вторым процессом по живой базе — задвоить отправки. 15/15 проверок кабинета: вход, изоляция ассортимента, смена цены, отказ на чужом товаре (404, не 403), событие в леджере с прежней ценой и актором `seller:…:user:…`, отказ партнёрскому токену в ERP, доезд цены до витрины.
- Ключевая проверка на регресс: набор ключей публичной проекции каталога у новой сборки и у работающего прода **совпал побайтово** — это тот самый инвариант, который трижды ломался утечкой `sellerId` и разъезжался на `undefined`-ключах в `enrichAll`.
- Подмена `dist`/`.next` каталогами рядом + `launchctl kickstart -k`; прежние сборки оставлены в `dist-old`/`.next-old` для отката.
- Найдено и починено уже на живом проде: кнопка «Показать» в поле пароля кабинета наезжала на текст — поле резервировало 64px, кнопке нужно 74.33px. Замер, а не глаз; тест `e2e/business-cabinet.spec.ts` меряет оба числа в обоих состояниях кнопки и краснел ровно на этих цифрах до правки. `pr-16 → pr-24`, 4/4 зелёных.
- Ложная проверка, которую поймал: дымовой web на порту 3100 не поднялся (`EADDRINUSE`), а 200 отдавал **предыдущий** инстанс на том же порту. Порт освобождён, проверка переделана — иначе выкатка была бы «проверена» на старой сборке.
- Побочно: `.gitignore` не знал про `dist-new/` — прошлая выкатка затащила в историю 1301 файл скомпилированного вывода. Убрано из индекса, закрыты все четыре каталога подмены.
- **Требует решения владельца, не агента:** имя кабинета оскциллирует между двумя инструментами — `e6f2e257` назвал его «Savio», `99d50041` переименовал в «AliStore», `4c9a4440` откатил обратно в «Savio». Прод выровнен по main (сейчас «Savio Business»), но само решение за владельцем.

## BUSINESS-HARDENING-2026-08-06 — два пункта долга кабинета закрыты
- **Advisory-лок на смену цены.** Без него итоговая цена выходила верной (её держит блокировка строки в `update`), ломался леджер: чтение прежней цены ничем не защищено, параллельные правки читали одно значение и все писали его в `previousPrice`. Замерено восемью параллельными правками — событие заявляло переход с 9500, когда реальная предыдущая цена была 8500. Цепочку «с какой на какую» по такому леджеру не собрать, а ведётся он ровно ради этого. Ключ `business-product-price:<productId>` — по товару, не по магазину.
- **Первая версия теста проходила и без фикса**, то есть не доказывала ничего: на двух параллельных правках гонка не воспроизводилась. Поднял конкуренцию до восьми — краснеет 3 из 3, после фикса зеленеет 3 из 3. Второй тест держит лок снаружи своей транзакцией и проверяет область ключа: с ключом по магазину он краснеет.
- **HTTP-слой кабинета закрыт supertest'ом** (`apps/api/test/business-http.e2e-spec.ts`, 13 случаев). Юнит-тесты зовут сервис напрямую и потому не видят ни `BusinessAuthGuard` (он разбирает заголовок и тип токена до вызова метода), ни `@Throttle` (лимитер живёт в конвейере). Подбор пароля и подделка токена в CI не проверялись вовсе.
- Ключевой случай — **staff-токен, подписанный тем же секретом**: подпись верна, отличается только `typ`. Без проверки типа он открыл бы кабинет. Проверено, что тесты умеют краснеть: снятие проверки типа роняет 3 теста, снятие `ThrottlerGuard` — 2.
- Попутная находка: `BusinessModule` **не** импортирует `RateLimitModule`, и `ThrottlerGuard` резолвится только потому, что `ThrottlerModule.forRoot` регистрируется глобально. На живом API замерено — 11-й вход подряд отвечает 429, защита работает; но держалась она ни на чём, кроме этого резолва. Уберут глобальность в библиотеке — декоратор станет украшением; теперь это поймает тест.
- Ещё одна ошибка в собственном тесте, стоившая прогона: кириллица в значении HTTP-заголовка непередаваема, Node падает с `Invalid character in header content` ещё до отправки — тест краснел, ничего не проверив. Значения заголовков в тестах только латиницей.
