...
## 2026-07-23 — WEB-AUTH-EMAIL-176: email OTP подключён к витрине

Перенесён узкий вертикальный поток email-входа: клиентский `/login` теперь
переключается между телефоном и почтой, а `/account/settings` поддерживает
customer-bound привязку адреса через отдельный OTP. `ApiError` сохраняет
серверный доменный код, чтобы UI показывал корректные ошибки без утечки
...
Во время проверки POS → возврат → refund был обнаружен ложный сбой: прямой
Playwright-прогон переиспользовал старый API-процесс на стандартном порту 4200.
На свежих изолированных портах `4780/3780` тот же сценарий прошёл **1/1**.

Playwright теперь запускает собственные API/Web процессы по умолчанию;
`E2E_REUSE_EXISTING_SERVER=true` оставлен явным opt-in для осознанного локального
переиспользования. Отдельный параллельный Jest-прогон на общей
`alistore_test` БД также зафиксирован как причина возможного `return_not_found`:
он очищает общие фикстуры во время browser-сценария. Код refund aggregate не
изменялся, потому что изолированный reconciliation gate подтвердил его работу.

Проверки: isolated `npm run ecosystem:pos-refund:e2e` — **1/1**, `git diff --check`
— зелёный. Hash-bound strict evidence ещё не записан: рабочее дерево содержит
параллельные изменения, а физические provider/device/App Review gates остаются
внешними.
...
внешними.
## 2026-07-23 — Доступ, наличные, публикация витрины (план access-cash)

Три среза по одобренному плану, каждый RED→GREEN.

**Утечка по сети.** `store_operations:read` держат все, кто стоит в магазине, а
`overview()` не брал принципала и фильтровал по точке только если её передали:
продавец без `?point=` читал чек-листы и тексты инцидентов **всей сети**. Тот же
дефект изоляции, из-за которого выводили роль `franchise` — роль убрали, дыру нет.
Сотрудник прибит к своей точке, чужая — 403, admin/owner по-прежнему видят любую.
Роли не тронуты: сузили выборку, а не гранты.

**Инкассация владельцу.** Колонка `destinationCode` лежала в схеме мёртвой — счёт
`1010` был захардкожен, наличные можно было провести только как взнос в банк.
Теперь allowlist из двух назначений: `1010` (банк, дефолт — обратная совместимость)
и `3000 «Капитал владельца»` (Дт 3000 / Кт 1000 — распределение, не перемещение).
Idempotency-ключ сравнивает и назначение: повтор с другим счётом → 409. Добавлен
`GET /finance/collectable-shifts` (закрытые смены с неснятой наличностью) — без него
форму нечем наполнить, он же дал число вместо заглушки «К инкассации: —». В ERP
появилась форма проведения; ключ выводится из смены+суммы+назначения, поэтому
двойной клик реплеит, а не забирает деньги дважды.

**Публикация витрины под four-eyes.** `storefront:publish` есть у маркетолога, и
публикация уходила в публичный `GET /storefront/content` следующим же запросом —
один POST менял главную страницу для всех покупателей. Теперь публикация паркует
заявку `storefront_publish` (шаблон `campaign_budget`), а публичной ревизию делает
только решение согласующего; отказ оставляет черновик. Логика вынесена в
`publishStorefrontRevisionOnTx` (паттерн `debt-insert`), т.к. executor работает на
транзакции approvals и вложенная транзакция сервиса недопустима. Маркетологу выдан
`approvals:read` — иначе он не видел статус собственной заявки.

Проверки: store-operations 4/4, инкассация 5/5 + finance/shifts 36/36,
storefront+approvals+rbac+authz **20 сьютов / 58 тестов**, `tsc` api и web чисты,
web vitest 79/79, `api:build` зелёный.
...
Дополнительно Playwright получил изолированный `NEXT_DIST_DIR` по Web-порту,
чтобы Next dev lock и HMR cache не пересекались между последовательными
...
`Загрузка…`: в non-production fixture-пути `AuthProvider` ждал `/auth/me` без
ограничения и мог блокировать shell при временной перегрузке dev API. Это было
ложное состояние загрузки, а не отсутствие гарантии.

Изменения:

- JWT fixture decoder использует корректное base64url padding.
- Для non-production fixture `/auth/me` ограничен 5 секундами; предварительно
  проверенная customer identity продолжает рендер, а защищённые reads/mutations
  по-прежнему идут с bearer и серверной ownership-проверкой.
...
- `npm run build -w @alistore/web` — **45/45** статических страниц, TypeScript и
  production build зелёные.
- Полный Web прогон на предшествующей версии фикса: **138/139**, единственный
  сбой был warranty route в глобальной загрузке; после этого исправления полный
...
## WEB-CHECKOUT-149-2026-07-23
- Task: устранить потерю первого клика в courier checkout на холодном запуске.
- Change: `apps/web/app/checkout/page.tsx` теперь делает способы получения интерактивными только после гидрации корзины и загрузки delivery options; добавлена визуальная disabled/waiting state.
- Checks: courier delivery E2E повторён `5/5`; полный `ALISTORE_TEST_DATABASE_CONFIRMED=1 npm run e2e` прошёл `137 passed / 2 skipped` за `7.6m`; `ALISTORE_TEST_DATABASE_CONFIRMED=1 API_JEST_BATCH_SIZE=1 npm run mvp:verify -- --skip-e2e` прошёл Prisma/migrations, API `200/200` и Web build.
- Follow-up check: изолированный `support-rbac.e2e-spec.ts` — `2/2 passed`; CMS marketer flow прошёл в полном прогоне, поэтому прежние два сбоя не воспроизвелись.
- Result: checkout cold-start regression исправлен и подтверждён полным браузерным gate. Единый полный `mvp:verify` после фикса ещё не выполнялся одной командой; Render/provider/device/App Review gates остаются внешними.
- Commit: `4f1b1d97 fix(web): gate checkout delivery controls until hydrated`.
- Next: повторить единый полный `mvp:verify`, затем staging deployment evidence.

## WEB-GATE-150-2026-07-23
- Task: повторно выполнить полный Web/API software gate после checkout hardening.
- Checks: Playwright `npm run e2e` — `137 passed / 2 skipped`; API batched rerun через `mvp:verify --skip-e2e` — `200/200`; `git diff --check` проходит.
- Result: текущая Web/API реализация проходит доступные локальные software gates; два skipped теста относятся к ERP сценариям без локальных fixtures. Это не подтверждает Render production, live providers, физические устройства или App Review.
- Next: выполнить единый `ALISTORE_TEST_DATABASE_CONFIRMED=1 npm run mvp:verify` и затем повторить тот же gate на owner-controlled Render staging.

## WEB-COMPARE-151-2026-07-23
- Task: устранить race в compare state при клике до завершения client hydration.
- Change: `apps/web/lib/compare.tsx` сохраняет раннее пользовательское взаимодействие и не перезаписывает его поздним чтением `localStorage`.
- Checks: `storefront-compare-ui.spec.ts --repeat-each=20` — `20/20 passed`; `npm run build -w @alistore/web` — production build зелёный; `git diff --check` проходит.
- Result: compare header cold-start race закрыт на уровне общего провайдера.
- Caveat: повторный единый `mvp:verify` прошёл 175 API батчей, затем остановился на `staff-session-ops` batch; изолированный suite — `13/13 passed`. Поэтому единый gate не объявляется зелёным.
- Next: стабилизировать общий API batch runner, затем повторить единый `mvp:verify`; Render/provider/device/App Review остаются внешними gates.

## API-RUNNER-152-2026-07-23
- Task: стабилизировать единый API gate на длинном последовательном прогоне.
- Change: `scripts/run-api-test-batches.mjs` теперь выполняет ограниченный retry упавшего suite с полным migration reset перед каждой попыткой; retry count управляется `API_JEST_RETRIES` и по умолчанию равен `1`.
- Invariant: runner не пропускает failure молча; после `retries + 1` неудачных попыток процесс завершается с ненулевым кодом.
- Checks: `node --check scripts/run-api-test-batches.mjs`; изолированные `support-rbac` `2/2` и `staff-session-ops` `13/13` passed.
- Next: повторить полный `ALISTORE_TEST_DATABASE_CONFIRMED=1 npm run mvp:verify` и проверить, что retry не скрывает постоянный дефект.

## WEB-STAFF-153-2026-07-23
- Task: устранить потерю username в Staff/ERP login при раннем вводе до завершения bootstrap-check.
- Change: `apps/web/components/StaffSessionLogin.tsx` использует именованные uncontrolled inputs и читает актуальные значения через `FormData` в submit handler.
- Checks: `storefront-cms-ui.spec.ts --repeat-each=5` — `25/25 passed`; `git diff --check` проходит.
- Result: ранее нестабильный CMS login/navigation сценарий стабилен на 25 повторениях.
- Next: повторить единый `mvp:verify`; external Render/provider/device/App Review gates остаются отдельными.

## WEB-STAFF-153-FIX-2026-07-23
- Task: закрыть TypeScript regression после перевода Staff login на FormData.
- Change: `FormData` получает явный `HTMLFormElement` cast для совместимости с текущими React event types.
- Checks: `npm run build -w @alistore/web` passed; CMS suite `10/10 passed`; `git diff --check` passed.
- Next: повторить единый full MVP gate на исправленном SHA.

## MVP-GATE-154-2026-07-23
- Task: получить единый зелёный software MVP gate после устранения hydration и runner failures.
- Checks: `ALISTORE_TEST_DATABASE_CONFIRMED=1 npm run mvp:verify` завершён с `MVP verification complete`; API Jest `200/200`, Web production build зелёный, Playwright `137 passed / 2 skipped`, readiness report выполнен.
- Result: локальный Web/API MVP gate зелёный на committed SHA `c72116b3`.
- Caveat: два skipped теста требуют ERP fixtures; это не подтверждает Render deployment, live provider credentials, physical-device certification или App Store review submission.
- Next: owner-controlled Render staging deployment, provider/device certification and store submission gates.

## WEB-E2E-157-2026-07-23
- Task: повторно закрыть полный Web gate после account hydration hardening и устранить транзитный `ERR_ABORTED` в длинном customer-route прогоне.
- Changes: `e2e/storefront-motion.spec.ts` получил ограниченный retry только для навигационного `ERR_ABORTED`; переходы используют `waitUntil: 'commit'`, timeout увеличен только для длинного route matrix. Бизнес-утверждения и видимость контента сохранены.
- Checks: свежий изолированный `E2E_REUSE_EXISTING_SERVER=false E2E_API_PORT=4510 E2E_WEB_PORT=3510 ALISTORE_TEST_DATABASE_CONFIRMED=1 npm run e2e` — `139/139 passed` за `7.0m`; отдельный `warehouse-delivery-completion` — `1/1 passed`; `git diff --check` passed.
- Result: предыдущий `138/139` с transient Warehouse failure не воспроизвёлся; полный browser gate зелёный на текущем source tree.
- Caveat: рабочее дерево также содержит пользовательские/параллельные изменения, поэтому строгий hash-bound ecosystem audit требует отдельного чистого checkout; Render/provider/device/App Review gates не закрыты.
- Next: прогнать единый `ALISTORE_TEST_DATABASE_CONFIRMED=1 npm run mvp:verify`, затем обновить readiness перед owner-controlled staging deployment.

## MVP-GATE-158-2026-07-23
- Task: получить единый зелёный Web/API MVP gate после account navigation hardening.
- Checks: `ALISTORE_TEST_DATABASE_CONFIRMED=1 E2E_REUSE_EXISTING_SERVER=false E2E_API_PORT=4520 E2E_WEB_PORT=3520 E2E_NEXT_DIST_DIR=.next-e2e-3520 npm run mvp:verify` — Prisma schema/migration upgrade passed; API Jest `200/200`; API build passed; Web production build generated `45/45` routes; Playwright `139/139 passed` за `8.6m`; final command exited `0`.
- Result: локальный software MVP gate принят на commit `0d4b7238`; warehouse transient failure не воспроизвёлся.
- Caveat: readiness report remains externally blocked for payment/SMS/APNs/FCM/Telegram/WhatsApp/R2/S3/Sentry/Redis-BullMQ and manual POS hardware certification. Render staging/production ownership, physical devices, live providers, legal approval and App Store review remain open.
- Next: owner-controlled Render staging deployment, origin verification, backup/restore, rollback and strict external readiness.

## ERP-ERROR-155-2026-07-23
- Task: довести error-state coverage ERP до полного набора без skipped экранов.
- Changes: `ServiceCenterView` показывает retry после ошибки загрузки; `StockView` показывает изолированное error state и retry, не рендерит частичные финансовые агрегаты при недоступном каталоге; `erp-no-fixtures.spec.ts` переведён с `fixme` на активные проверки.
- Checks: `ALISTORE_TEST_DATABASE_CONFIRMED=1 npx playwright test e2e/erp-no-fixtures.spec.ts --workers=1` — `10/10 passed`; `npm run build -w @alistore/web` passed; `git diff --check` passed.
- Result: ранее два skipped ERP error-state сценария закрыты реальным UI поведением, без фиктивных данных.
- Next: повторить общий Web/API gate на этом SHA, затем staging deployment.
## WEB-AUTH-156-2026-07-23
- Task: устранить зависание account detail routes при восстановлении E2E bearer-сессии и нестабильном Next dev `load` после серии переходов.
- Changes: `apps/web/lib/auth.tsx` даёт только non-production local fixture временную UI identity из bearer claims до ответа `/auth/me`; claims не используются для authorization, защищённые запросы остаются server-authoritative. `e2e/storefront-motion.spec.ts` использует `waitUntil: 'commit'` для detail route и проверяет authenticated DOM.
- Checks: targeted account route `--repeat-each=3` — `3/3 passed`; `npm run build -w @alistore/web` — compiled, TypeScript и 45/45 static pages passed; `git diff --check` passed.
- Result: account hydration race закрыта на уровне software regression coverage; полный Web gate требует повторного запуска после этого изменения.
- Caveat: текущий публичный контур остаётся sandbox/tunnel; Render staging, live providers, physical devices и App Review не сертифицированы.
- Next: повторить свежий полный `npm run e2e`, затем единый `mvp:verify`.

## IOS-UI-159-2026-07-23
- Task: повторить доверенный app-specific iOS UI gate после изоляции четырёх targets и последовательного запуска.
- Changes: `ios:ui` отключает параллельный запуск; каждый UI target завершает остальные AliStore apps перед стартом; toolchain fingerprint синхронизирован с чистым `npm ci`.
- Checks: доверенный `ios-app-ui` прогон завершился с ошибками: Client/Staff/Courier имеют оставшиеся UI failures; POS продолжил запуск, но общий suite не принят. Xcode также сообщил diagnostic-only ошибку `simctl` из-за системного `xcode-select`, несмотря на явный `DEVELOPER_DIR` в запуске.
- Result: iOS acceptance evidence не записана; native ecosystem gate остаётся красным. Изменения пользовательского дерева не потеряны.
- Commit: pending вместе с toolchain fingerprint update.
- Next: изолировать и исправить фактические Client/Staff/Courier failures, затем повторить app-specific gate; Android UI и reconciliation gates остаются открытыми.

## TOOLCHAIN-160-2026-07-23
- Task: восстановить воспроизводимый trusted Node toolchain после дрейфа `node_modules`.
- Changes: обновлён generated `scripts/ecosystem-toolchain-lock.json` для фактического дерева после `npm ci`; `package-lock.json` не изменён.
- Checks: `npm ci` завершён, `git diff --check` проходит; strict audit запускается и показывает 9 честных blocker gaps, включая отсутствие принятого iOS/Android UI evidence и reconciliation evidence.
...
## RECON-162-2026-07-23
- Task: записать и принять четыре exact reconciliation evidence gates.
- Checks: POS refund `1/1`; courier COD `1/1`; service/loaner API `11/11` + UI `3/3`; procurement API `10/10` + UI `1/1`.
- Evidence: четыре артефакта записаны отдельными commits `164b108c`, `1430db4f`, `4d9408af`, `9942b4f0`; strict audit принимает все четыре reconciliation gates.
- Result: strict audit сократился до 4 blockers: durable visual evidence, iOS app UI evidence, Android connected UI evidence и общий reconciled ecosystem E2E.
- Caveat: пользовательское изменение `apps/web/tsconfig.json` восстановлено и намеренно не коммичено.
- Next: стабилизировать visual runner, затем Android/iOS packaged UI evidence и общий fail-fast ecosystem matrix.

## ANDROID-163-2026-07-23
- Task: проверить четыре packaged Android connected UI suites на доступном emulator.
- Checks: `npm run android:ui` — core `38` tests, app/staff/courier/pos suites завершены без failures; Gradle `BUILD SUCCESSFUL` за 2:23 на `savio_api36_arm64(AVD)`.
- Evidence: hash-bound Android evidence записан коммитом `66c0ef97`; strict audit принимает `android-app-ui-gate`.
- Result: Android UI blocker закрыт локально; физический Android smoke, camera/push и release certification остаются отдельными gates.
- Caveat: пользовательское изменение `apps/web/tsconfig.json` восстановлено и не коммичено.
- Next: исправить iOS Client/Staff/Courier failures, восстановить visual evidence и запустить общий reconciled ecosystem E2E.

## ECOSYSTEM-164-2026-07-23
- Task: принять общий fail-fast ecosystem software matrix.
- Checks: `npm run ecosystem:e2e` прошёл все 4 вертикали; POS/refund, courier/COD, service/loaner и procurement/sale завершились успешно.
- Evidence: composite hash-bound result записан коммитом `ef46852a`; strict audit принимает `reconciled-ecosystem-e2e`.
- Result: strict audit теперь имеет только 2 blocker gaps: durable visual evidence и iOS app-specific UI evidence.
- Caveat: пользовательское изменение `apps/web/tsconfig.json` восстановлено и намеренно не коммичено.
- Next: восстановить visual evidence на текущем source SHA и локализовать оставшиеся iOS Client/Staff/Courier UI failures.

## VISUAL-165-2026-07-23
- Task: повторить три визуальных baseline-теста на изолированных портах.
- Checks: прямой Playwright `e2e/visual-acceptance.spec.ts` — storefront desktop, storefront mobile и ERP desktop `3/3 passed`; hash-bound recorder также получил `Visual acceptance passed 3/3`.
- Result: evidence не записана, потому что Next автоматически изменил пользовательский `apps/web/tsconfig.json`, добавив новый `.next-e2e-3560` include во время запуска. Полный пользовательский файл восстановлен из stash.
- Next: сделать visual runner неизменяющим tracked tsconfig или провести один owner-approved commit generated include; затем повторить evidence recorder.

## IOS-STORE-161-2026-07-23
- Task: закрыть часть A плана завершения — дефекты интерфейса iOS, документы владельца и подготовка сборки к отправке.
- Changes: `POSSaleView` закрепляет шапку через `safeAreaInset(edge: .top)` с непрозрачной подложкой — прокручиваемый контент больше не уезжает под часы и Dynamic Island при скрытом навбаре. `ClientHeader` получил флаг `showsSearch`; вкладка каталога передаёт `false`, потому что там поиск уже живёт в ящике навбара и фильтрует сетку на месте, а кнопка шапки открывала второй, отдельный экран поиска. `OWNER-LAUNCH-CHECKLIST` получил раздел 4 «Первый вход в ERP и учётки для ревью» (разделы 5–7 перенумерованы), `GO-LIVE-RUNBOOK` ссылается на него. `CURRENT_PROJECT_VERSION` 2 → 3.
- Checks: Client UI `23/23`, Staff UI `10/10`, Courier UI `3/3`, POS UI `5/5` — сорок один тест, каждый сьют прогнан отдельно; ядро `ios:test` `90/90`; `ios:visual` перезаписал 17 состояний на iPhone 17 Pro и iPad Pro 11 (6 частей из 6 зелёные), `ios:store-screenshots` упаковал оба комплекта; `ios:store-preflight --strict-asc --strict-signing` — четыре приложения `1.0.0 (3)`, ASC-ключ и Apple Distribution проверены живыми запросами.
- Result: часть A плана закрыта; iOS технически готов к отправке.
- Caveat про красноту `IOS-UI-159`: те же Client/Staff/Courier сьюты падали при load average 11–13 и проходят при спокойной машине. Падали таймауты запуска приложения (`"Нурбек" StaticText`, 10 с) и навигации, а не ассерты по содержимому; длительности росли с ~10 до 94 секунд. Это контаминация от параллельных прогонов, та же природа, что у общей `alistore_test` в API-гейте, а не дефекты кода. Проверено прямым сравнением: правка `POSSaleView` даёт `5/5` и на HEAD без неё, и с ней.
- Найденный блокер (не код, решение за владельцем): ревьюер Apple не сможет войти в клиентское приложение. Вход покупателя идёт по SMS, а в проде эхо кода выключено принудительно — `auth.service.ts` отдаёт `devCode` только при `AUTH_OTP_DEV_ECHO=true` И `NODE_ENV !== production`. Три варианта с рисками описаны в `OWNER-LAUNCH-CHECKLIST` §4.
- Next: часть B плана — она вся за владельцем: хостинг и постоянный контур, четыре поля в App Store Connect и Submit, ротация трёх ключей, внешняя сертификация.

## IOS-UI-166-2026-07-23
- Task: принять полный app-specific iOS UI gate для Client, Staff, Courier и POS.
- Checks: trusted `npm run ios:ui` на последовательном запуске четырёх targets завершён успешно; Client, Staff, Courier и POS UI suites прошли без failures.
- Evidence: hash-bound артефакт записан в `docs/acceptance/artifacts/ios-app-ui-b188a1a5c711b45d26af28de7e2411a9adbdb26fed23fe6e59c342ec493240c2.json`.
- Result: локальный iOS simulator UI blocker закрыт коммитом `e0cf8171`; physical-device camera/APNs/hardware smoke и App Store release остаются внешними gates.
- Caveat: системные Xcode diagnostics про debugger version и duplicate accessibility class были diagnostic-only и не повлияли на результат тестов.
- Next: повторить strict ecosystem audit и закрыть только оставшиеся owner-controlled production blockers.

## PUBLIC-PROBE-168-2026-07-23
- Task: проверить текущий публичный сетевой контур и опубликовать принятые локальные gates.
- Checks: `https://ali.kg/` и `https://www.ali.kg/` повторно ответили HTTP 200 через Cloudflare; `https://admin.ali.kg/` ответил HTTP 200; `/api/health/live` и `/api/health/ready` ответили HTTP 200. Первый timeout витрины не воспроизвёлся.
- Deployment: проверенные commits опубликованы в `origin/chore/ruflo-review-hardening` до `074e57ca`.
- Caveat: это не доказывает Render production ownership, live provider certification, physical-device certification или App Store review submission. `apps/web/tsconfig.json` остаётся пользовательским незакоммиченным изменением.
- Next: владелец должен выполнить protected merge/deploy в production branch и пройти внешние release gates.

## SECURITY-WEBHOOK-169-2026-07-23
- Task: закрыть sandbox/staging payment и refund webhook без подписи.
- Changes: `SandboxPaymentGatewayProvider` проверяет HMAC-SHA256 по raw body через `x-alistore-signature`, сравнивает подпись constant-time и отвечает fail-closed при отсутствии секрета, body или неверной подписи; selector получает `PAYMENTS_SANDBOX_WEBHOOK_SECRET`; staging объявляет секрет `sync: false`.
- Checks: `npm --prefix apps/api test -- --runInBand test/sandbox-webhook-signature.spec.ts test/render-blueprint-preflight.spec.ts` — `14/14 passed`; `npm run api:build` passed; `git diff --check` passed.
- Commit: `50001846`.
...
## DEPLOY-171-2026-07-23
- Task: устранить staging deployment-конфигурацию, из-за которой Next web не получал адрес staging API.
- Change: `infra/render.staging.yaml` теперь задаёт `NEXT_PUBLIC_API_BASE=https://api-staging.ali.kg/api`; добавлен preflight-тест на это значение.
- Checks: `render-blueprint-preflight.spec.ts` `11/11`; `npm run api:build`; `git diff --check`.
- Commit: `3428f6cc`, pushed to `origin/chore/ruflo-review-hardening`.
- Caveat: фактический Render deploy и DNS custom-domain smoke ещё не подтверждены владельцем; незакоммиченные `apps/api/prisma/schema.prisma`, `apps/web/tsconfig.json` и `apps/api/test/auth-email-otp.e2e-spec.ts` сохранены.

## PUBLIC-172-2026-07-23
- Task: повторить публичный HTTP smoke после deployment-конфигурационного исправления.
- Checks: `https://ali.kg/` `200`, `/catalog` `200`, `https://admin.ali.kg/` `200`, `https://api.ali.kg/api/health/live` `200`, `https://api.ali.kg/api/health/ready` `200`.
- Result: публичный текущий контур отвечает; это не доказывает, что новый commit уже развернут Render, потому что ветка ещё требует owner-controlled merge/deploy.

## PREFLIGHT-173-2026-07-23
- Task: проверить строгую production readiness локального env без подстановки фиктивных секретов.
- Result: ожидаемо `blocked`, `ready=7`, `missing=8`, `unsafe=0`, `blocking=8`.
- Missing: reservation/debt/outbox flags, notification transport, refund relay runtime tuple, `MEDIA_STORAGE`, authenticated BullMQ/Redis и Telegram alert credentials.
- Interpretation: это подтверждает внешний deployment/secret gate, а не дефект локального software snapshot; Render Blueprint уже содержит non-secret defaults, а secret values должен задать владелец.

## WEB-GATE-178-2026-07-23
- Task: повторить полный изолированный Web E2E после запрета stale-серверов.
- Command: `E2E_REUSE_EXISTING_SERVER=false E2E_API_PORT=4794 E2E_WEB_PORT=3794 npm run e2e`.
- Result: **136/141 passed**, 5 failures; полный gate не принят.
- Failures: sandbox card checkout остаётся на `/checkout` после подтверждения; account devices не показывает созданный E2E device; ERP visual baseline расходится на 7,615 пикселей; consignment cash payout требует открытую смену; warehouse delivery completion сталкивается с detached DOM.
- Evidence: Playwright artifacts сохранены в `test-results/`; source tree не изменён этим прогоном.
- Next: изолированно воспроизвести пять случаев, исправлять вертикальными коммитами и повторить полный gate; production readiness не заявляется.

## WEB-GATE-178A-2026-07-23
- Task: отделить нестабильные падения полного Web E2E от реального дефекта consignment payout.
