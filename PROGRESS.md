...
## 2026-07-26 (2) — витрина для ревью скриптом + починен красный гейт «нет фикстур»

**Сток для App Review — теперь скриптом (`22bab49d`).** Продовая витрина отдаёт четыре
товара с `availableUnits: 0`: ревьюер открывает AliStore KG и не может ничего купить —
прямой риск Guideline 2.1, и до сих пор это висело ручной задачей владельца без
безопасного способа сделать. `scripts/seed-review-data.mjs` (`npm run review:seed`)
пополняет **уже существующие** товары через `POST /inventory/receive-quantity` — то есть
каждая запись идёт через Event Ledger с детерминированным idempotency-ключом. Товары,
цены и категории **не выдумывает**: сериализованные позиции докладывает и не трогает
(фиктивные IMEI в реальном леджере хуже пустой витрины — ровно поэтому в репозитории живёт
`check-no-fixtures`). По умолчанию read-only, запись только `--apply`, а на любой
не-локальный API дополнительно нужен `--yes-production`; реквизиты — из окружения, не из argv.

**Проверено на живом, не на моках.** Одноразовая БД из шаблона + API на своём порту:
dry-run спланировал, `--apply` довёл товар с 0 до 3 и сделал покупаемым, повторный
`--apply` оставил `onHand` ровно 3 — ключ удержал. Read-only прогон против **прода** нашёл
два дефекта, которых юнит-тесты не видели: каталог режет `limit` на 100 (добавил
пагинацию) и повторный прогон по маленькому, но полностью укомплектованному каталогу винил
«пропущенные товары», которых не было (теперь сообщает реальную причину — заводите товары
в ERP). Юнит-тесты 8/8.

**API-гейт зелёный на изолированной БД.** Полный прогон `npm run api:test:isolated` на
текущем коммите: **218/218 сьютов, 1299/1299 тестов**, ни одного флака за прогон. Это
заметно лучше цифры в `docs/READINESS.md` (там записано «4 сьюта / 7 тестов красные»,
снято 23.07 на общей базе) — и подтверждает то, что CLAUDE.md и так утверждает: красный
на общей `alistore_test` при работающем рядом втором инструменте — это шум, а не дефект.
Один прогон по-прежнему ничего не доказывает про флаки (`FLAKE-001`), но состояние
«гейт красный» на этом коммите не воспроизводится.

**Починен `npm run api:test:isolated` (`ffd523a3`).** CLAUDE.md подробно объясняет, что
это **тот самый** способ гонять API-тесты, не ломая чужие прогоны, — а команда падала с
`Missing script`. Сам раннер (`scripts/run-isolated-api-tests.mjs`) всё это время лежал
в репозитории; npm-запись снесло попутно в `e20c0537`, iOS-релизном коммите. То есть
единственный работавший путь был ровно тот, от которого документация предостерегает.

**Гейт `mvp:verify` был красный, и этого никто не заметил (`3df1f997`).** Шаг «Нет новых
фикстур в ERP» падал с момента коммитов `54269a77` и `c6bfe1c3` — на закоммиченном коде,
не на моих правках. Оба места сбрасывали список в пустой на **любом** отказе. Опасный —
передача кассы: у кассира с открытым ящиком запрос отваливается, а экран пишет «передать
смену некому: на вашей точке нет других активных сотрудников». Сломанный бэкенд подан как
организационный факт, отличить и повторить нельзя. Теперь показывает ошибку и «Повторить»,
а «некому» осталось для случая, когда это правда. На доске задач пустой ростер у админа без
`staff:manage` **законен** (об этом и был комментарий), но `catch` глотал и 500 — теперь
глотает только настоящий отказ в доступе. Правило вынесено в `isPermissionDenied()`
(`lib/api/http.ts`) и покрыто тестами: в репозитории тестируется чистая `lib`-логика,
рендер-тестов компонентов нет, поэтому решение, которое стоит защитить, должно жить там,
где его можно проверить.

**Проверка.** `check-no-fixtures` зелёный, `tsc` зелёный, web-тесты **107/107**,
production web build проходит. Сам отрисованный экран ошибки в браузере не гонял —
до него нужна аутентифицированная открытая смена.

## 2026-07-26 — App Store: сняты два жёстких блокера Submit, метаданные ревью приведены к реальности

**Состояние снято живьём, не по докам.** Через ASC API, распаковку загруженных `.ipa` и
продовый API. Репозиторий описывал релиз неверно: `SUBMISSION-STATUS.md` утверждал сборку
`1.0.0 (2)` и «reviewSubmission не найден», рунбук — что загружена сборка 2. Фактически по
всем четырём приложениям в ASC лежит **сборка 4 `VALID` и привязана** к версии `1.0.0`
(Client загружен 25.07 — блокер `Sign in with Apple` снят), а `reviewSubmission` существуют
у всех четырёх в `READY_FOR_REVIEW` с **пустым `items`** — то есть отправлять нечего.
Пересборка и повторная выгрузка не нужны: всё оставшееся — метаданные ASC и ввод владельца.

**Блокер 1 — категория и content rights (`ed7995ef`).** У Staff, Courier и POS
`primaryCategory` и `contentRightsDeclaration` были `NULL`. Оба поля обязательны до Submit,
то есть три приложения из четырёх были физически неотправляемы, и ни один документ этого не
упоминал. Корень: `apps/ios/store/*-metadata.json` объявляют `BUSINESS` и
`DOES_NOT_USE_THIRD_PARTY_CONTENT`, но в репозитории были только валидаторы — **ничто
никогда не переносило эти значения в ASC**; Client настроили руками, три остальных забыли.
Новый `scripts/apply-ios-store-metadata.mjs` диффает метаданные против ASC и патчит только
расхождения (ES256/JWT переиспользован из `verify-app-store-connect.mjs`); dry-run по
умолчанию. Применено 6 изменений, Client не тронут. Идемпотентность доказана: повторный
прогон → «No changes needed». Тест первым, `node --test` 8/8.

**Блокер 2 — App Review Notes описывали несуществующий вход (`fef6f66e`).** Заметки
Staff/Courier/POS велели ревьюеру войти «по телефону и одноразовому коду» и **просить код
у нас**. Обе части неверны: эти три входят через `staff-auth/login`, а живой запуск на
симуляторе показывает ровно два поля — «Логин» и «Пароль», ни телефона, ни OTP. Ревьюер по
старой инструкции не вошёл бы вообще → Guideline 2.1; плюс демо-доступ через переписку с
разработчиком Apple отклоняет как шаблон. Заметки переписаны под реальный вход, с явным
«одноразового кода нет» и требованием выключенного TOTP на ревью-учётке (`staff-auth`
поддерживает пофакаунтный TOTP и иначе запер бы ревьюера). Заодно убрано «back-office app of
a single electronics retail company in Bishkek… not a consumer app» — это описание
приложения, которое Apple отправляет в Custom Apps, и оно противоречило соседнему
`review.notes` в тех же файлах. Все три приведены к одному multi-tenant позиционированию.

**Дефект конфига — сборка ушла бы на мёртвый хост (`26e3aabb`).** `apps/ios/.env.production.example`
и рунбук были переведены на `https://ali.kg/api`, который отдаёт HTML витрины и **404**;
работает `https://api.ali.kg/api` (200), и именно он зашит в загруженные бинарники
(`API_BASE_URL` в Info.plist). Cloudflare Functions за этим адресом не задеплоены. Вернул
рабочий хост; `validate-canonical-api-base.mjs` перестал требовать эти два iOS-файла — с
комментарием вернуть на cutover. Остальные четыре записи (зона Codex) не тронуты.

**Проверка.** `store-preflight --strict-asc --strict-signing` зелёный на `1.0.0 (5)`
(`project.yml` поднят 4→5: сборка 4 занята, конвенция «дерево на один впереди»
восстановлена); `ios:test` **134/134**; все четыре схемы собираются и **стартуют на
симуляторе iPhone 17 Pro**, экраны входа рисуются — юнит-тесты идут с
...
Client и только Client), для Staff/Courier/POS — три учётки сотрудников из ERP с
выключенным TOTP. Плюс App Privacy, подтверждение цены и сам Submit ×4. Отдельно:
витрина отдаёт `total: 4` и **0 позиций в наличии** — ревьюер не пройдёт покупку, риск
Guideline 2.1; остатки заводит владелец через ERP. Точный чек-лист —
`apps/ios/store/SUBMISSION-STATUS.md`.

## 2026-07-24 — клиентский чекаут: оплата подарочной картой + состояние релиза iOS

**Срез.** Оплата заказа подарочной картой в клиентском приложении (`f3bf6bf1`).
`GiftCardCheckout` (`apps/ios/Shared/GiftCardPayment.swift`) заворачивает
`GET /giftcards/:code` и `POST /payments` (`method=gift_card`, customer-scoped —
`payments.controller.ts:96` намеренно открывает этот POST покупателю только для
`gift_card`). Покрытие карты проверяется ДО создания заказа — полуоплаченный заказ не
создаём; списание уходит под конкретный `orderId` уже после того, как заказ существует.
Успех не оставляет payment intent — экран результата показывает успех, как при оплате
при получении; отказ карты оставляет заказ неоплаченным и говорит это прямо, а не ложный
успех. Только полная оплата картой; частичная карта + добор другим тендером (как в вебе)
сознательно не тянется в клиент.

**Проверка.** Контракт `GiftCardPaymentTests` 3/3 (маршрут, тело, Bearer-токен, бросок на
не-2xx); полная сборка `AliStoreClient` с интеграцией зелёная; Client UI-регрессия 27/27;
существующие пути checkout не тронуты — вся ветка под `paymentMethod == "gift_card"`.

**Ревью поймало CRITICAL — исправлено `6198b5fe`.** Адверсариальный код-ревью денежного
потока нашёл false-success, который я пропустил: `ClientPaymentResultView` читал `nil`
payment intent как успех и не смотрел на `retryErrorMessage`, поэтому отказ списания карты
(гонка баланса) показал бы зелёное «Заказ оформлен» при неоплаченном заказе — ровно то, что
срез обещал не делать. Тот же латентный баг был и на провале онлайн-интента. Состояние
экрана вынесено в чистую `paymentResultState` (Shared) + `PaymentResultStateTests`;
surfaced-ошибка при неоплаченном заказе теперь читается как провал. Core-сьют 122/122.
...
**Четвёртый ревью — курьерские деньги (pre-existing) — `476fffb7` + бэклог.** Нашёл
реальный money-баг HIGH: кнопка «Сдать COD» была безусловно выключена при
`collectedTotal != codTotal`, поэтому частичный рейс (в рейсе была неудачная доставка)
сдать было нельзя никогда — уже собранная наличность зависала. Логика вынесена в чистую
`CODHandover` + `CODHandoverTests`: расхождение требует причину, но не блокирует сдачу.
Ещё две находки — не деньги/ниже приоритетом, заведены в бэклог: `COURIER-OFFLINE-OWNER-001`
(очередь без владельца — но сервер ловит чужой реплей 403, это робастность, не мисатрибуция,
в отличие от POS) и `COURIER-IDEMPOTENCY-002` (ключ не от содержимого, устаревший амаунт может
выиграть CAS). Мат COD, округление, force-unwrap — чисто. Core 128/128, all-targets build
зелёный.

**Пятый ревью — покупательский платёжный путь (pre-existing) — `69e23bd5`.** Чекаут-математика
(`payable` не уходит в минус — бонус клампится; сервер пересчитывает total авторитетно; интент
берёт `order.total`), gift-card-сеттл и сплит оплаты — подтверждены чистыми. Нашёл CRITICAL потери
заказа: `OfflineOrderQueue` не переигрывал заказы, застрявшие в `syncing` (приложение убили посреди
отправки) — авто-повтор фильтровал только queued/failed, ручной был выключен, заказ терялся молча.
Тот же кейс уже чинили в POS (`replayable`), в покупательской очереди — нет. Добавлен
`OfflineOrderQueue.replayable` (+syncing) + тесты; авто- и ручной повтор его используют. Заведено
`OFFLINE-ORDER-DEDUP-003` (LOW: enqueue без дедупа по ключу — не воспроизвёлся, defense-in-depth).
Core 130/130, all-targets build зелёный.

**Шестой ревью — staff-авторизация/быстрый-вход (pre-existing) — `ba142e00`.** Гейт,
Keychain-хранение (`SecureTokenStore`), лок-аут с клампом после ребута, очистка PIN при
логауте (защита общего устройства), биометрия — CRITICAL/bypass НЕ нашёл. Две защиты-в-глубину
по брутфорсу PIN починены: `registerFailure` глотал ошибку записи счётчика (`try?`) → лок-аут
фейлился ОТКРЫТО; и лимит попыток сторожил только вью, а `matches` был публичным без проверки.
Теперь фейл-закрыто + store-энфорснутый `verify(pin:)`. Core 132/132.

**Итог заходов ревью (сессия).** Шесть адверсариальных ревью всех денежных и auth-поверхностей
iOS (gift-card, email/Apple-auth+списание, POS, курьер, покупательский путь, staff-PIN) →
**шесть фиксов**: два CRITICAL (false-success оплаты `6198b5fe`; потеря офлайн-заказа `69e23bd5`),
две HIGH денег (атрибуция POS `f444836e`; зависшая COD `476fffb7`), два hardening (nonce+ввод
`822ff9a7`; PIN-лок-аут `ba142e00`) — плюс четыре находки в бэклоге. Прод-аудиты раньше это
пропускали: ложный успех, POS-атрибуция и потеря заказа были **pre-existing**. Системный паттерн
(трижды): фикс применяли в одном пути (авто / POS / online-intent), а в парном (ручной UI /
курьер / покупатель / gift-card) — нет; и «энфорс во вью, не в сторе». Всё поедет в сборку 4.

**Довод по паттерну — `0d8a78b0`.** Закрыт `COURIER-OFFLINE-OWNER-001`: курьерская офлайн-очередь
теперь пишет владельца и фильтрует показ/реплей (`OfflineCourierQueue.owned`), как POS и заказ.
Все три офлайн-очереди единообразно owner-scoped — системный «фикс в одном пути, не в парном»
на офлайн-очередях закрыт целиком. Core 134/134.
...
24.07: `applesignin: 0` в обоих локальных профилях для `kg.alistore.client`). Разблокировка —
включить Sign in with Apple на App ID в Apple Developer и перевыпустить профиль. Пост-build-3
правки в клиенте (эта оплата подарочной картой) и в Staff (списание, `f2e4430b`) поедут в
сборку 4. Полные шаги владельца — memory `ios-appstore-submission-state`.
...
## IOS-RELEASE-MECHANICS-104-2026-07-23
- Task: make the iOS release mechanics cover all four apps instead of Client only.
- Changes: `apps/ios/scripts/store-preflight.sh` `--strict-signing` now requires an App Store provisioning profile per bundle id (`kg.alistore.client|staff|courier|pos`), ignores Development/Ad Hoc profiles (`get-task-allow=true` or a device list), names the missing profile in the failure and no longer prints a blanket "signing material verified"; `CURRENT_PROJECT_VERSION` 2 → 3 (build 2 is already in App Store Connect); new `npm run ios:archive` (`apps/ios/scripts/archive.sh`) archives four schemes into `apps/ios/build/`; new `npm run ios:export` (`apps/ios/scripts/export-archives.sh`) generates `exportOptions.plist` per scheme from environment variables into git-ignored `apps/ios/build/exportOptions/` and exports four `.ipa`; `apps/ios/store/release-runbook.md` rewritten for four apps (the stale "no local provisioning profile" paragraph is gone — all four App Store profiles are present locally); App Review Notes added as `review.appReviewNotes` in the four `apps/ios/store/*-metadata.json` and enforced by both metadata validators.
- Checks: `bash -n` on the three release scripts; `node scripts/validate-ios-store-metadata.mjs apps/ios/store/client-metadata.json` and `node scripts/validate-ios-ecosystem-store-metadata.mjs` pass; `npm run ios:store-preflight` green on all four schemes at `1.0.0 (3)`; `--strict-signing` (no `--strict-asc`, keys untouched) green with all four local App Store profiles resolved.
