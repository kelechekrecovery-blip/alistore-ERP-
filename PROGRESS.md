...
- Checks: courier delivery E2E повторён `5/5`; повторный полный `mvp:verify` прошёл Prisma/migration gates, API suites до `200/200` ранее подтверждены, но общий прогон остановился на timeout/open handles в `support-rbac.e2e-spec.ts`.
- Follow-up check: изолированный `support-rbac.e2e-spec.ts` — `2/2 passed`, что указывает на нестабильность полного runner, а не на checkout regression.
- Result: checkout cold-start regression исправлен; полный release gate пока не объявляется зелёным из-за отдельного flaky API runner и внешних Render/provider/device/App Review gates.
- Next: стабилизировать общий API test runner, повторить полный `mvp:verify`, затем commit/push этой итерации.
