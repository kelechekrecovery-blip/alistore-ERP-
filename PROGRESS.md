...
- **Checks:** фактический hash рассчитан через тот же `hashDependencyTree`, что использует trusted runner; strict audit повторяется после commit.
- **Next:** принять strict audit result и не считать экосистему release-ready при оставшихся GAP.
