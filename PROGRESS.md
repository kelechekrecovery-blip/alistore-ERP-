...
**Итог заходов ревью (сессия).** Пять адверсариальных ревью денежных/auth-поверхностей iOS
(gift-card, auth, POS, курьер, покупательский путь) → **шесть реальных багов починено**: два
CRITICAL (false-success оплаты `6198b5fe`; потеря офлайн-заказа `69e23bd5`), три HIGH (атрибуция
POS `f444836e`; зависшая COD `476fffb7`; nonce+ввод `822ff9a7` — MEDIUM+LOW) — плюс четыре находки
в бэклоге. Прод-аудиты раньше это пропускали: ложный успех, POS-атрибуция и потеря заказа были
**pre-existing**. Паттерн, повторившийся трижды: фикс применяли в одном пути (авто / POS), а в
парном (ручной UI / курьер / покупатель) — нет.
