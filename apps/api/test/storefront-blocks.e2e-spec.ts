...
describe('Storefront blocks CMS', () => {
  ...
  it('enforces RBAC, asset/product invariants and publishes device-scoped blocks with Ledger events', async () => {
    await request(app.getHttpServer()).post('/storefront-blocks').set(auth(sellerToken)).send(block('DENIED')).expect(403);
    const invalid = await request(app.getHttpServer()).post('/storefront-blocks').set(auth()).send(block('BAD', { imageUrl: 'http://unsafe.example/banner.jpg' })).expect(422);
    ...
    const collection = await request(app.getHttpServer()).post('/storefront-blocks').set(auth()).send(block('COLLECTION', { type: 'collection', device: 'desktop', productIds: [product.id], ctaHref: '/catalog?category=phones' })).expect(201);
    const hero = await request(app.getHttpServer()).post('/storefront-blocks').set(auth()).send(block('HERO', { type: 'hero', device: 'desktop', imageUrl: 'https://media.example/banner.jpg' })).expect(201);
    await request(app.getHttpServer()).post(`/storefront-blocks/${collection.body.id}/publish`).set(auth()).send({}).expect(201);
    await request(app.getHttpServer()).post(`/storefront-blocks/${hero.body.id}/publish`).set(auth()).send({}).expect(201);

    const desktop = await request(app.getHttpServer()).get('/storefront-blocks/public?device=desktop').expect(200);
    ...
    const mobile = await request(app.getHttpServer()).get('/storefront-blocks/public?device=mobile').expect(200);
  ...
  it('prevents overlapping hero schedules, reorders all live blocks and archives without stale publication', async () => {
    const first = await request(app.getHttpServer()).post('/storefront-blocks').set(auth()).send(block('SCHEDULE-A', { type: 'hero', device: 'desktop' })).expect(201);
    const second = await request(app.getHttpServer()).post('/storefront-blocks').set(auth()).send(block('SCHEDULE-B', { type: 'hero', device: 'all' })).expect(201);
    const promo = await request(app.getHttpServer()).post('/storefront-blocks').set(auth()).send(block('PROMO')).expect(201);
    const startsAt = new Date(Date.now() + 3_600_000).toISOString();
    const endsAt = new Date(Date.now() + 7_200_000).toISOString();
    await request(app.getHttpServer()).post(`/storefront-blocks/${first.body.id}/schedule`).set(auth()).send({ startsAt, endsAt }).expect(201);
    const overlap = await request(app.getHttpServer()).post(`/storefront-blocks/${second.body.id}/schedule`).set(auth()).send({ startsAt: new Date(Date.now() + 5_400_000).toISOString(), endsAt: new Date(Date.now() + 9_000_000).toISOString() }).expect(409);
    expect(overlap.body.code).toBe('storefront_block_schedule_overlap');

    await request(app.getHttpServer()).post('/storefront-blocks/reorder').set(auth()).send({ ids: [promo.body.id, second.body.id, first.body.id] }).expect(201);
    const rows = await request(app.getHttpServer()).get('/storefront-blocks').set(auth()).expect(200);
    ...
    await request(app.getHttpServer()).post(`/storefront-blocks/${first.body.id}/cancel-schedule`).set(auth()).send({}).expect(201);
    await request(app.getHttpServer()).post(`/storefront-blocks/${promo.body.id}/publish`).set(auth()).send({}).expect(201);
    await request(app.getHttpServer()).post(`/storefront-blocks/${promo.body.id}/archive`).set(auth()).send({}).expect(201);
    const publicRows = await request(app.getHttpServer()).get('/storefront-blocks/public?device=desktop').expect(200);
