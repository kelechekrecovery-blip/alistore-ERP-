...
describe('Reference data: chart of accounts', () => {
  ...
  it('устанавливает полный план счетов на базе, где его нет', async () => {
    // Счёт нельзя удалить, пока на него кто-то ссылается. «Заведомо не заняты»
    // по списку кодов — не работает: в полном прогоне соседние сьюты успевают
    // провести проводки по 6700/6900/6990, и удаление падало на
    // `AccountingJournalLine_accountCode_fkey` (в одиночку сьют проходил, потому
    // и выглядел зелёным). Спрашиваем базу, а не угадываем; пустыми должны быть
    // все четыре обратные связи, иначе упрёмся в другой внешний ключ.
    const untouched = await prisma.accountingAccount.findMany({
      where: {
        code: { in: ['6700', '6900', '6990'] },
        lines: { none: {} },
        openingBalanceLines: { none: {} },
        bankStatements: { none: {} },
        payments: { none: {} },
      },
      select: { code: true },
