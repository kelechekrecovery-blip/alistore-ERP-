...
describe('Service loaner custody (integration + ownership)', () => {
  ...
  it('keeps IMEI, custody, evidence, ownership, overdue and repair close consistent', async () => {
    ...
    await request(app.getHttpServer()).post('/evidence/images').set('Authorization', `Bearer ${token}`).set('Idempotency-Key', `evidence-denied-${run}`).attach('file', Buffer.from('not-an-image'), 'loaner.jpg').field('entityType', 'loaner').field('entityId', prepared.body.id).field('label', 'loaner_issue').expect(403);
