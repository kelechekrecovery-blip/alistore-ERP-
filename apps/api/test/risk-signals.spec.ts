...
describe('buildRiskSignals', () => {
  ...
  it('flags sold units with no order as a stock≠money mismatch (high)', () => {
    const s = buildRiskSignals({ ...base, soldWithoutOrder: 3 }, now);
  ...
  it('omits the mismatch signal when zero units are affected', () => {
    const s = buildRiskSignals({ ...base, soldWithoutOrder: 0 }, now);
    expect(s.some((x) => x.kind === 'stock_money_mismatch')).toBe(false);
  });

  it('ranks the high-severity mismatch before the medium leak', () => {
    const s = buildRiskSignals(
      { ...base, soldWithoutOrder: 1, marginLeaks: [{ sku: 'Y', name: 'Z', price: 1, cost: 2 }] },
      now,
    );
