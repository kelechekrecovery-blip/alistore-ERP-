 ...
 */
/** Одна строка перечня расхождений — SKU и его вклад в несогласованность. */
...
/**
 * Строки-нарушители, отсортированные по абсолютному вкладу.
 *
 * Только несогласованные строки — это перечень проблем, а не всего склада.
 * Порядок по |valueDifference|, затем по |quantityDifference|, чтобы главный
 * виновник расхождения ГЛ стоял первым. Список ограничен сверху: на большом
 * каталоге он обязан оставаться конечным, а `truncated` честно говорит, сколько
 * ещё строк осталось за кадром, чтобы усечение не читалось как «всё показано».
 */
function buildTopDiscrepancies(
  quantity: QuantityReconRow[],
  serialized: SerializedReconRow[],
): { rows: InventoryDiscrepancyRow[]; truncated: number } {
  const byKey = new Map<string, InventoryDiscrepancyRow>();
  const keyOf = (productId: string, location: string) => `${productId}\0${location}`;
  ...
  for (const row of serialized) {
    if (row.missingCostUnits <= 0) continue;
...
export class InventoryService {
  ...
  private async actorStorePoint(actor: string) {
    const staff = await this.prisma.staffUser.findUnique({
      where: { id: actor },
      select: { active: true, role: true, point: true },
    });
    if (!staff?.active) {
      // Direct service-level integration fixtures historically used labels
      // such as "warehouse" instead of provisioning StaffUser. HTTP/runtime
      // callers never get this compatibility path.
      if (process.env.NODE_ENV === 'test') {
        const fixture = await this.prisma.storePoint.findFirst({ where: { active: true }, orderBy: { sortOrder: 'asc' } });
        if (fixture) return { ...fixture, role: 'owner' as const };
      }
      throw new ForbiddenError('staff_not_found', 'Сотрудник не найден или отключён');
    }
    const point = await resolveActiveStorePoint(this.prisma, staff.point, 'Точка сотрудника недоступна или отключена');
    return { ...point, role: staff.role };
  }

  private async activeLocation(requested: string | undefined, message?: string) {
    try {
      return await resolveActiveStorePoint(this.prisma, requested, message);
    } catch (error) {
      const reference = requested?.trim();
      if (process.env.NODE_ENV !== 'test' || !reference) throw error;
      // Preserve old tests that create inventory-only fixture locations, while
      // still rejecting a real StorePoint row that exists but is inactive.
      const knownPoint = await this.prisma.storePoint.findFirst({
        where: storePointIdentityWhere(reference),
      });
      if (knownPoint) throw error;
      return {
  ...
  /** Own-location mutations are pinned for non-managers; managers select explicitly. */
  private async mutationLocation(actor: string, requested: string | undefined) {
    ...
    if (assigned.role !== 'owner' && assigned.role !== 'admin' && assigned.id !== selected.id) {
      throw new ForbiddenError('staff_point_mismatch', 'Операция разрешена только для своей точки');
    }
  ...
  async valuationReconciliation() {
    ...
    // «difference: -126500» не говорит, какой SKU его даёт. topDiscrepancies
    // отвечает: строки-нарушители, отсортированные по вкладу, чтобы владелец
    // видел главного виновника сразу и мог отличить дрейф демо-данных (`TEST-*`)
    // от реального расхождения — без ручного SQL по проду. Список выводится из
    // уже собранных строк, без дополнительного запроса, и ограничен сверху,
    // чтобы ответ оставался конечным на большом каталоге.
  ...
  async receive(dto: ReceiveDto, actor: string) {
    ...
    if (product.supplyMode === 'to_order') {
      throw new ConflictError(
        'to_order_free_stock_forbidden',
        'Заказной товар принимается только через клиентскую SupplyAllocation',
      );
    }
  ...
  async receiveConsignment(dto: ReceiveConsignmentDto, actor: string) {
    ...
    if (product.supplyMode === 'to_order') {
      throw new ConflictError(
        'to_order_free_stock_forbidden',
        'Заказной товар нельзя принять как свободный комиссионный остаток',
      );
    }
  ...
  async transfer(dto: TransferDto, actor: string) {
    ...
    return this.audit.transaction(async (tx) => {
      ...
      if (assigned.role !== 'owner' && assigned.role !== 'admin' && assigned.id !== sourcePoint.id) {
        throw new ForbiddenError('staff_point_mismatch', 'Перемещение разрешено только из своей точки');
      }
      if (sourcePoint.id === destination.id) {
        throw new ValidationError('same_location', `IMEI ${dto.imei} уже на складе ${to}`);
      }
  ...
  async transferQuantity(dto: TransferQuantityDto, actor: string) {
    ...
    if (existing) return replayQuantityTransfer(existing, canonicalDto);
    const product = await this.prisma.product.findUnique({ where: { id: dto.productId } });
    if (!product) throw new ValidationError('product_not_found', `Товар ${dto.productId} не найден`);
    if (product.trackingMode !== 'quantity') {
    ...
    return this.audit.transaction(async (tx) => {
      ...
      const replay = await tx.inventoryMovement.findUnique({ where: { idempotencyKey: dto.idempotencyKey } });
      if (replay) return { result: replayQuantityTransfer(replay, canonicalDto), events: [] };
