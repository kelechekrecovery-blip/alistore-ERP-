...
import { finalizeOrderItemInventorySaleOnTx } from '../inventory/order-inventory-sale';
import { UnitsService } from '../units/units.service';

export async function handOverReadyOrderItemOnTx(
  ...
  if (!item) throw new ValidationError('order_item_not_found', `Строка ${input.orderItemId} не найдена`);
  if (item.fulfillmentStatus === 'handed_over') {
    throw new ConflictError('order_item_already_handed_over', 'Строка уже выдана');
  }
  if (item.fulfillmentStatus !== 'ready') {
    throw new ConflictError('order_item_not_ready', `Строка ${item.id} не готова к выдаче`);
  }
  ...
  if (item.supplyModeSnapshot === 'own_stock' && receivables.length !== 1) {
    throw new ConflictError('stock_sale_receivable_missing', 'Для складской строки требуется одно начисление stock_sale');
  }
  if (item.supplyModeSnapshot === 'to_order') {
    if (item.orderLineSupply?.status !== 'ready') {
      throw new ConflictError('order_line_supply_not_ready', 'Поставочная строка не готова к выдаче');
    }
    const deposit = receivables.find((row) => row.kind === 'supply_deposit');
    if (!deposit || deposit.status !== 'settled') {
      throw new ConflictError('order_line_deposit_unpaid', 'Задаток поставочной строки не закрыт');
    }
  }
  if (input.paymentMode !== 'cod' && receivables.some((row) => row.status !== 'settled')) {
    throw new ConflictError('order_line_receivables_unpaid', 'Перед выдачей необходимо закрыть начисления строки');
  }

  const netSale = item.price * item.qty - item.discountAmount;
  if (item.taxBaseAmount + item.taxAmount !== netSale) {
    throw new ConflictError('order_line_financial_snapshot_invalid', 'Налоговый снимок строки не совпадает с суммой продажи');
  }
  const receivableTotal = receivables.reduce((sum, row) => sum + row.amount, 0);
  if (receivableTotal !== netSale) {
    throw new ConflictError('order_line_receivable_total_invalid', 'Начисления строки не совпадают с суммой продажи');
  }
  const liability = receivables.reduce(
    (sum, row) => sum + row.allocations.reduce((allocated, allocation) => allocated + allocation.amount, 0),
    0,
  );
  if (liability > netSale || (input.paymentMode !== 'cod' && liability !== netSale)) {
    throw new ConflictError('order_line_payment_allocation_incomplete', 'Платёжные аллокации строки повреждены');
  }
  const codReceivable = netSale - liability;
  const occurredAt = input.occurredAt ?? new Date();

  if (item.supplyModeSnapshot === 'own_stock') {
    await finalizeOrderItemInventorySaleOnTx(tx, {
    ...
    if (units.length !== item.qty) {
      throw new ConflictError('order_line_reserved_units_incomplete', 'Серийные единицы поставочной строки не совпадают с количеством');
    }
    for (const unit of units) {
      const valuation = await input.units.sellOnTx(tx, unit.imei, input.orderId, input.actor);
      if (valuation?.entry) input.events.push({
    ...
    if (allocations.reduce((sum, allocation) => sum + allocation.qty, 0) !== item.qty) {
      throw new ConflictError('supply_quantity_inventory_allocation_missing', 'Количественная поставка не имеет складской аллокации');
    }
    for (const allocation of allocations) {
      ...
      if (allocationCas.count !== 1) {
        throw new ConflictError('supply_quantity_allocation_race', 'Поставочная аллокация изменилась параллельно');
      }
      if (cogsEntry) {
        input.events.push({
          type: EventType.AccountingEntryPosted,
  ...
  if (itemCas.count !== 1) throw new ConflictError('order_item_handover_race', 'Строка изменилась параллельно');
  if (item.orderLineSupply) {
    ...
    if (supplyCas.count !== 1) throw new ConflictError('order_line_supply_race', 'Поставка изменилась параллельно');
  }
  input.events.push({
    type: EventType.AccountingEntryPosted,
    actor: input.actor,
    ...
    type: item.supplyModeSnapshot === 'to_order' ? EventType.OrderLineSupplyHandedOver : EventType.OrderItemHandedOver,
    actor: input.actor,
