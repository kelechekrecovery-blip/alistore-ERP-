...
const PUBLIC_SELECT = {
  ...
  createdAt: true,
  resolvedAt: true,
} satisfies Prisma.SupplyQuarantineResolutionSelect;

type PublicResolution = Prisma.SupplyQuarantineResolutionGetPayload<{ select: typeof PUBLIC_SELECT }>;
type ResolutionForMutation = Prisma.SupplyQuarantineResolutionGetPayload<{
  include: {
    orderLineSupply: {
      include: {
        orderItem: { select: { id: true; orderId: true; supplyModeSnapshot: true } };
        quantityAllocations: true;
      };
    };
  };
}>;
...
@Injectable()
export class SupplyQuarantineService {
  constructor(
  ...
  async propose(
    orderItemId: string,
    dto: ProposeSupplyQuarantineDto,
    actor: string,
    ...
    if (replay) return replayProposal(replay, proposalHash);

    return this.audit.transaction<PublicResolution & { idempotent: boolean }>(async (tx) => {
      ...
      if (lockedReplay) return { result: replayProposal(lockedReplay, proposalHash), events: [] };
      ...
      if (!item) throw new ValidationError('order_item_not_found', `Строка заказа ${orderItemId} не найдена`);
      if (item.supplyModeSnapshot !== 'to_order' || !item.orderLineSupply) {
        throw new ConflictError(
          'supply_quarantine_requires_to_order',
          'В quarantine можно поместить только физически принятый товар из строки «под заказ»',
        );
      }
      ...
      if (!poItem || supply.receivedQty <= 0 || poItem.receivedQty < supply.receivedQty) {
        throw new ConflictError(
          'supply_quarantine_not_physically_received',
          'Нельзя поместить товар в quarantine до его физической приёмки',
        );
      }
      if (!['received', 'quality_check', 'ready', 'customer_cancelled', 'quarantined'].includes(supply.status)) {
        throw new ConflictError(
          'supply_quarantine_invalid_state',
          `Поставка в статусе ${supply.status} ещё не готова к quarantine`,
        );
      }
      if (!item.product) throw new ConflictError('supply_quarantine_product_missing', 'У строки отсутствует товар');
      ...
      if (!storePoint?.active) {
        throw new ConflictError(
          'supply_quarantine_store_point_required',
          'Склад PO должен соответствовать активной торговой точке',
        );
      }
      ...
      if (item.product.trackingMode === 'serialized') {
        if (imeis.length !== supply.receivedQty) {
          throw new ValidationError(
            'supply_quarantine_imeis_required',
            'Для серийной поставки нужно передать все принятые IMEI',
          );
        }
        ...
        if (units !== imeis.length) {
          throw new ConflictError(
            'supply_quarantine_units_mismatch',
            'IMEI не совпадают с зарезервированными единицами клиентской поставки',
          );
        }
      } else if (imeis.length > 0) {
        throw new ValidationError('supply_quarantine_quantity_has_no_imei', 'Для количественного товара IMEI передавать нельзя');
      } else {
        const allocations = supply.quantityAllocations;
        if (
          allocations.reduce((sum, allocation) => sum + allocation.qty, 0) !== supply.receivedQty
          || allocations.some((allocation) => (
            allocation.productId !== item.product!.id
            || allocation.location !== location
            || allocation.unitCost !== poItem.unitCost
          ))
        ) {
          throw new ConflictError(
            'supply_quarantine_quantity_allocation_mismatch',
            'Количественная поставка не совпадает с клиентскими складскими аллокациями',
          );
        }
      }

      if (await tx.supplyQuarantineResolution.findUnique({ where: { orderLineSupplyId: supply.id } })) {
        throw new ConflictError('supply_quarantine_already_proposed', 'Для этой строки уже создано quarantine-решение');
      }
      const resolution = await tx.supplyQuarantineResolution.create({
        data: {
          ...
          imeis: imeis.length ? imeis : Prisma.DbNull,
          proposalReason: reason,
          proposalEvidence: evidence as Prisma.InputJsonValue,
          proposedBy: actor,
      ...
      if (item.product.trackingMode === 'serialized') {
        ...
        if (changed.count !== imeis.length) {
          throw new ConflictError('supply_quarantine_unit_race', 'Статус одной из серийных единиц изменился параллельно');
        }
      }
      if (supply.status !== 'quarantined') {
        ...
        if (changed.count !== 1) {
          throw new ConflictError('supply_quarantine_state_race', 'Статус поставки изменился параллельно');
        }
      }
      ...
      return {
        ...
        events: [{
          type: EventType.SupplyQuarantineProposed,
          actor,
          payload: {
  ...
  async resolve(
    resolutionId: string,
    dto: ResolveSupplyQuarantineDto,
    actor: string,
    ...
    if (role !== 'owner' && role !== 'admin') {
      throw new ForbiddenError(
        ...
        'Решение quarantine может принять только владелец или администратор',
      );
    }
    const reason = normalizeReason(dto.reason);
    const evidence = normalizeEvidence(dto.evidence);
    ...
    if (replay) return replayResolution(replay, resolutionHash);
    ...
    return this.audit.transaction<PublicResolution & { idempotent: boolean }>(async (tx) => {
      ...
      if (lockedReplay) return { result: replayResolution(lockedReplay, resolutionHash), events: [] };
      ...
      if (!resolution) throw new ValidationError('supply_quarantine_not_found', `Quarantine-решение ${resolutionId} не найдено`);
      if (resolution.status !== 'pending') {
        throw new ConflictError('supply_quarantine_already_resolved', 'Quarantine уже разрешён другой командой');
      }
      if (
        resolution.orderLineSupply.status !== 'quarantined'
        || resolution.orderLineSupply.orderItem.supplyModeSnapshot !== 'to_order'
      ) {
        throw new ConflictError(
          'supply_quarantine_invalid_resolution_state',
          'Поставка больше не находится в допустимом quarantine-состоянии',
        );
      }
      ...
      const changed = await tx.supplyQuarantineResolution.updateMany({
        ...
        data: {
          ...
          resolutionEvidence: evidence as Prisma.InputJsonValue,
          resolvedBy: actor,
      ...
      if (changed.count !== 1) {
        throw new ConflictError('supply_quarantine_resolution_race', 'Quarantine был разрешён параллельно');
      }
      ...
      const events: AuditInput[] = [{
        type: EventType.SupplyQuarantineResolved,
        actor,
        payload: {
          resolutionId: resolution.id,
  ...
  private async consumeQuantityAllocationsOnTx(
    tx: Prisma.TransactionClient,
    resolution: ResolutionForMutation,
  ): Promise<void> {
    if (resolution.trackingModeSnapshot !== 'quantity') return;
    ...
    if (allocations.reduce((sum, allocation) => sum + allocation.qty, 0) !== resolution.quarantinedQty) {
      throw new ConflictError(
        'supply_quarantine_quantity_allocation_mismatch',
        'Quarantine-количество не совпадает с активными клиентскими аллокациями',
      );
    }
    ...
    for (const allocation of allocations) {
      ...
      if (changed.count !== 1) {
        throw new ConflictError(
          'supply_quarantine_quantity_allocation_race',
          'Клиентская количественная аллокация изменилась параллельно',
        );
      }
    }
  }

  private async convertToOwnStock(
    tx: Prisma.TransactionClient,
    resolution: ResolutionForMutation,
    idempotencyKey: string,
  ): Promise<string> {
    ...
    if (!point?.active || point.inventoryLocation !== resolution.inventoryLocationSnapshot) {
      throw new ConflictError(
        'supply_quarantine_location_changed',
        'Торговая точка деактивирована или её складская идентичность изменилась',
      );
    }
    ...
    if (resolution.trackingModeSnapshot === 'serialized') {
      ...
      if (changed.count !== resolution.quarantinedQty) {
        throw new ConflictError('supply_quarantine_unit_race', 'Не все quarantined IMEI доступны для конвертации');
      }
    } else {
  ...
  private async returnToSupplier(
    tx: Prisma.TransactionClient,
    resolution: ResolutionForMutation,
  ): Promise<null> {
    if (resolution.trackingModeSnapshot === 'serialized') {
      ...
      if (changed.count !== resolution.quarantinedQty) {
        throw new ConflictError('supply_quarantine_unit_race', 'Не все quarantined IMEI доступны для возврата поставщику');
      }
    }
    return null;
  }
...
function normalizeReason(reason: string): string {
  ...
  if (normalized.length < 3) {
    throw new ValidationError('supply_quarantine_reason_required', 'Укажите содержательную причину минимум из трёх символов');
  }
  return normalized;
}
...
function normalizeEvidence(evidence: Record<string, unknown>): Record<string, unknown> {
  if (!evidence || Array.isArray(evidence) || typeof evidence !== 'object' || Object.keys(evidence).length === 0) {
    throw new ValidationError('supply_quarantine_evidence_required', 'Для quarantine требуется непустое подтверждение');
  }
  return evidence;
}

function normalizeImeis(input: string[] | undefined): string[] {
  const imeis = (input ?? []).map((imei) => imei.trim()).filter(Boolean).sort();
  if (new Set(imeis).size !== imeis.length) {
    throw new ValidationError('supply_quarantine_duplicate_imei', 'IMEI в quarantine-запросе не должны повторяться');
  }
  return imeis;
}
...
function replayProposal(
  replay: PublicResolution & { proposalHash: string },
  expectedHash: string,
): PublicResolution & { idempotent: true } {
  if (replay.proposalHash !== expectedHash) {
    throw new ConflictError('idempotency_key_reused', 'Idempotency-Key уже использован с другим quarantine-запросом');
  }
...
function replayResolution(
  replay: PublicResolution & { resolutionHash: string | null },
  expectedHash: string,
): PublicResolution & { idempotent: true } {
  if (replay.resolutionHash !== expectedHash) {
    throw new ConflictError('idempotency_key_reused', 'Idempotency-Key уже использован с другим решением quarantine');
  }
  const { resolutionHash: _resolutionHash, ...result } = replay;
  return { ...result, idempotent: true };
}

function jsonImeis(value: Prisma.JsonValue | null): string[] {
  if (!Array.isArray(value) || value.some((entry) => typeof entry !== 'string') || value.length === 0) {
    throw new ConflictError('supply_quarantine_imei_snapshot_invalid', 'Снимок IMEI quarantine повреждён или отсутствует');
  }
