...
exports.inventoryValuationRollForward = inventoryValuationRollForward;
const errors_1 = require("../common/errors");
const MAX_REPORT_SPAN_MS = 366 * 24 * 60 * 60 * 1000;
const amount = () => ({ quantity: 0, value: 0 });
const numeric = (value) => Number(value ?? 0);
async function inventoryValuationRollForward(prisma, fromInput, toInput) {
    const from = new Date(fromInput);
    const to = new Date(toInput);
    if (!Number.isFinite(from.getTime()) || !Number.isFinite(to.getTime()) || from >= to) {
        throw new errors_1.ValidationError('valuation_period_invalid', 'Начало периода должно быть раньше окончания');
    }
    if (to.getTime() - from.getTime() > MAX_REPORT_SPAN_MS) {
        throw new errors_1.ValidationError('valuation_period_too_large', 'Период отчёта не может превышать 366 дней');
    }
    const dbFrom = from.toISOString().slice(0, -1);
    const dbTo = to.toISOString().slice(0, -1);
    const [layerAggregates, issueAggregates, reversalAggregates, transferAggregates, serializedReceiptAggregates, incompleteTransferCount, incompleteSerializedReceiptCount, incompleteServiceConsumptionCount, missingReversalQuantityRows, unknownIssueLocationCount, unknownReversalLocationCount, legacyConsignmentIssueCount, incompleteQuantityBalanceCount, glTotals] = await Promise.all([
        prisma.$queryRaw `
    ...
    const rows = new Map();
    const getRow = (productId, sku, name, location) => {
    ...
    return {
        ...
        period: { from: from.toISOString(), to: to.toISOString(), semantics: '[from,to)' },
...
//# sourceMappingURL=inventory-roll-forward.js.map