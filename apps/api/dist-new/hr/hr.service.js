...
const seller_revenue_1 = require("../reports/seller-revenue");
const store_point_identity_1 = require("../common/store-point-identity");
const DAY_MS = 86_400_000;
function dateOnly(value, field) {
    const date = new Date(`${value.slice(0, 10)}T00:00:00.000Z`);
    if (Number.isNaN(date.getTime()))
        throw new errors_1.ValidationError('hr_invalid_date', `${field}: неверная дата`);
    return date;
}
...
function payrollBounds(period) {
    if (!/^\d{4}-(0[1-9]|1[0-2])$/.test(period))
        throw new errors_1.ValidationError('hr_invalid_payroll_period', 'Период должен быть в формате YYYY-MM');
    const [year, month] = period.split('-').map(Number);
    return { start: new Date(Date.UTC(year, month - 1, 1)), end: new Date(Date.UTC(year, month, 1)) };
}
function payrollPoint(point) {
    const value = point.trim();
    if (!value)
        throw new errors_1.ValidationError('hr_payroll_point_required', 'Укажите точку начисления');
    return value;
}
function requireKey(key) {
    if (!key?.trim())
        throw new errors_1.ValidationError('idempotency_key_required', 'Требуется Idempotency-Key');
    return key.trim();
}
function scheduleWindow(shiftDateValue, startsAtValue, endsAtValue) {
    ...
    if (endsAt <= startsAt || startsAt.toISOString().slice(0, 10) !== shiftDateValue.slice(0, 10) || endsAt.toISOString().slice(0, 10) !== shiftDateValue.slice(0, 10)) {
        throw new errors_1.ValidationError('hr_invalid_schedule_window', 'Смена должна начинаться и заканчиваться в выбранную дату');
    }
...
let HrService = class HrService {
    ...
    async calculatePayroll(tx, period, rawPoint) {
        const PAYROLL_CONFIG = await this.payrollConfig();
        const point = (await (0, store_point_identity_1.resolveActiveStorePoint)(tx, payrollPoint(rawPoint))).inventoryLocation;
        ...
        const [schedules, absences, payments] = await Promise.all([
            ...
            tx.payment.findMany({
                where: (0, seller_revenue_1.sellerRevenueWhere)({ from: start, to: end, point }),
                select: { amount: true, receivedBy: true, shift: { select: { staffId: true } } },
            }),
        ]);
        const candidateIds = new Set(schedules.map((row) => row.staffId));
        payments.forEach((payment) => { const seller = (0, seller_revenue_1.soldBy)(payment); if (seller)
            candidateIds.add(seller); });
        ...
        const lines = staffIds.map((staffId) => {
            ...
            const staffPayments = payments.filter((payment) => (0, seller_revenue_1.soldBy)(payment) === staffId);
    ...
    payrollPreview(period, point) {
        return this.prisma.$transaction((tx) => this.calculatePayroll(tx, period, point));
    }
    async payrollRuns(period, rawPoint) {
        const point = (await (0, store_point_identity_1.resolveActiveStorePoint)(this.prisma, payrollPoint(rawPoint))).inventoryLocation;
    ...
    async postPayroll(period, rawPoint, actor, rawKey) {
        ...
        return this.audit.transaction(async (tx) => {
            ...
            if (replay) {
                if (replay.action !== 'post' || stableJson(replay.request) !== stableJson(request))
                    throw new errors_1.ConflictError('hr_idempotency_mismatch', 'Ключ уже использован для другой операции начисления');
            ...
            if (await tx.hrPayrollRun.findUnique({ where: { period_point: { period, point } } }))
                throw new errors_1.ConflictError('hr_payroll_already_posted', 'Начисление за этот период уже проведено');
            const preview = await this.calculatePayroll(tx, period, point);
            if (!preview.lines.length)
                throw new errors_1.ValidationError('hr_payroll_empty', 'Нет смен или продаж для начисления');
            if (!Number.isSafeInteger(preview.totals.payout) || preview.totals.payout <= 0)
                throw new errors_1.ValidationError('hr_payroll_zero', 'Нельзя провести нулевое начисление');
            const runId = (0, node_crypto_1.randomUUID)();
            const accrual = await (0, accounting_journal_1.postAccountingEntryOnTx)(tx, {
            ...
            return { result: response, events: [
                    { type: event_types_1.EventType.HrPayrollPosted, actor, payload: { runId: run.id, period, point, totalPayout: run.totalPayout, lineCount: run.lines.length, accountingEntryId: accrual.id }, refs: [run.id, ...run.lines.map((line) => line.staffId)] },
                    { type: event_types_1.EventType.AccountingEntryPosted, actor, payload: { accountingEntryId: accrual.id, sourceType: 'hr.payroll.accrual', sourceRef: run.id, amount: run.totalPayout }, refs: [accrual.id, run.id, period] },
                ] };
        });
    }
    async payPayroll(runId, rawExternalRef, actor, rawKey, rawFundingAccountCode) {
        const key = requireKey(rawKey);
        const externalRef = rawExternalRef.trim();
        if (!externalRef)
            throw new errors_1.ValidationError('hr_payroll_external_ref_required', 'Укажите номер платёжного документа');
        const fundingAccountCode = rawFundingAccountCode?.trim() || '1010';
        if (!['1000', '1010', '1020'].includes(fundingAccountCode))
            throw new errors_1.ValidationError('hr_payroll_funding_account_invalid', 'Недопустимый счёт выплаты зарплаты');
        const request = { runId, externalRef, fundingAccountCode };
        return this.audit.transaction(async (tx) => {
            const replay = await tx.hrPayrollCommand.findUnique({ where: { idempotencyKey: key } });
            if (replay) {
                if (replay.action !== 'pay' || stableJson(replay.request) !== stableJson(request))
                    throw new errors_1.ConflictError('hr_idempotency_mismatch', 'Ключ уже использован для другой операции начисления');
                return { result: replay.response, events: [] };
            }
            ...
            if (!current)
                throw new errors_1.ValidationError('hr_payroll_not_found', 'Начисление не найдено');
            if (current.status === 'paid')
                throw new errors_1.ConflictError('hr_payroll_already_paid', 'Начисление уже выплачено');
            if (!current.accrualAccountingEntryId)
                throw new errors_1.ConflictError('hr_payroll_accrual_missing', 'Для payroll отсутствует проводка начисления');
            const paidAt = new Date();
            const payout = await (0, accounting_journal_1.postAccountingEntryOnTx)(tx, {
            ...
            return { result: response, events: [
                    { type: event_types_1.EventType.HrPayrollPaid, actor, payload: { runId, period: paid.period, point: paid.point, totalPayout: paid.totalPayout, externalRef, fundingAccountCode, accountingEntryId: payout.id }, refs: [runId, ...paid.lines.map((line) => line.staffId)] },
                    { type: event_types_1.EventType.AccountingEntryPosted, actor, payload: { accountingEntryId: payout.id, sourceType: 'hr.payroll.payout', sourceRef: runId, amount: paid.totalPayout }, refs: [payout.id, runId, paid.period] },
                ] };
        });
    }
    async week(weekStart, point, staffId) {
        const { start, end } = weekBounds(weekStart);
        const staffWhere = { active: true, ...(staffId ? { id: staffId } : {}) };
        const scheduleWhere = {
    ...
    async createSchedule(dto, actor, rawKey) {
        const key = requireKey(rawKey);
        const point = (await (0, store_point_identity_1.resolveActiveStorePoint)(this.prisma, dto.point)).inventoryLocation;
        ...
        return this.audit.transaction(async (tx) => {
            ...
            if (replay) {
                if (replay.staffId !== dto.staffId || replay.startsAt.getTime() !== startsAt.getTime() || replay.endsAt.getTime() !== endsAt.getTime())
                    throw new errors_1.ConflictError('hr_idempotency_mismatch', 'Ключ уже использован для другой смены');
                return { result: replay, events: [] };
            }
            const staff = await tx.staffUser.findUnique({ where: { id: dto.staffId } });
            if (!staff?.active)
                throw new errors_1.ValidationError('hr_staff_inactive', 'Сотрудник не найден или отключён');
            ...
            if (absence)
                throw new errors_1.ConflictError('hr_approved_absence', 'На эту дату утверждено отсутствие');
            const existing = await tx.hrSchedule.findUnique({ where: { staffId_shiftDate: { staffId: dto.staffId, shiftDate } } });
            if (existing)
                throw new errors_1.ConflictError('hr_schedule_exists', 'На эту дату смена уже назначена');
            ...
            return { result: schedule, events: [{ type: event_types_1.EventType.HrScheduleCreated, actor, payload: { scheduleId: schedule.id, staffId: schedule.staffId, point: schedule.point, startsAt, endsAt }, refs: [schedule.id, schedule.staffId] }] };
        });
    }
    async updateSchedule(id, dto, actor, rawKey) {
        const key = requireKey(rawKey);
        ...
        return this.audit.transaction(async (tx) => {
            ...
            if (replay) {
                if (replay.scheduleId !== id || replay.action !== 'update' || stableJson(replay.request) !== stableJson(request))
                    throw new errors_1.ConflictError('hr_idempotency_mismatch', 'Ключ уже использован для другой команды графика');
                return { result: replay.response, events: [] };
            }
            ...
            if (!schedule)
                throw new errors_1.ValidationError('hr_schedule_not_found', 'Плановая смена не найдена');
            if (schedule.attendance)
                throw new errors_1.ConflictError('hr_schedule_started', 'Начатую смену нельзя менять');
            ...
            return { result: response, events: [{ type: event_types_1.EventType.HrScheduleUpdated, actor, payload: { scheduleId: id, staffId: schedule.staffId, ...request }, refs: [id, schedule.staffId] }] };
        });
    }
    async cancelSchedule(id, reason, actor, rawKey) {
        const key = requireKey(rawKey);
        const request = { reason: reason?.trim() || null };
        return this.audit.transaction(async (tx) => {
            const replay = await tx.hrScheduleCommand.findUnique({ where: { idempotencyKey: key } });
            if (replay) {
                if (replay.scheduleId !== id || replay.action !== 'cancel' || stableJson(replay.request) !== stableJson(request))
                    throw new errors_1.ConflictError('hr_idempotency_mismatch', 'Ключ уже использован для другой команды графика');
                return { result: replay.response, events: [] };
            }
            await tx.$queryRaw `SELECT pg_advisory_xact_lock(hashtext(${'hr-schedule:' + id}))::text AS locked`;
            ...
            if (!schedule)
                throw new errors_1.ValidationError('hr_schedule_not_found', 'Плановая смена не найдена');
            if (schedule.attendance)
                throw new errors_1.ConflictError('hr_schedule_started', 'Начатую смену нельзя отменить');
            if (schedule.cancelledAt)
                throw new errors_1.ConflictError('hr_schedule_cancelled', 'Смена уже отменена');
            ...
            return { result: response, events: [{ type: event_types_1.EventType.HrScheduleCancelled, actor, payload: { scheduleId: id, staffId: schedule.staffId, reason: request.reason }, refs: [id, schedule.staffId] }] };
        });
    }
    async openAttendance(scheduleId, staffId, rawKey) {
        const key = requireKey(rawKey);
        return this.audit.transaction(async (tx) => {
            const replay = await tx.hrAttendance.findUnique({ where: { checkInKey: key } });
            if (replay) {
                if (replay.scheduleId !== scheduleId || replay.staffId !== staffId)
                    throw new errors_1.ConflictError('hr_idempotency_mismatch', 'Ключ уже использован для другой отметки');
                return { result: replay, events: [] };
            }
            ...
            if (schedule.staffId !== staffId)
                throw new errors_1.ForbiddenError('hr_schedule_owner_mismatch', 'Нельзя открыть чужую смену');
            if (schedule.cancelledAt)
                throw new errors_1.ConflictError('hr_schedule_cancelled', 'Отменённую смену нельзя открыть');
            if (schedule.attendance)
                throw new errors_1.ConflictError('hr_attendance_open', 'Смена уже отмечена');
            ...
            if (absence)
                throw new errors_1.ConflictError('hr_approved_absence', 'Нельзя открыть смену во время утверждённого отсутствия');
            ...
            return { result: attendance, events: [{ type: event_types_1.EventType.HrAttendanceOpened, actor: staffId, payload: { attendanceId: attendance.id, scheduleId, point: schedule.point }, refs: [attendance.id, scheduleId, staffId] }] };
        });
    }
    async closeAttendance(scheduleId, staffId, rawKey) {
        const key = requireKey(rawKey);
        return this.audit.transaction(async (tx) => {
            ...
            if (!attendance)
                throw new errors_1.ValidationError('hr_attendance_not_open', 'Сначала отметьте начало смены');
            if (attendance.staffId !== staffId)
                throw new errors_1.ForbiddenError('hr_schedule_owner_mismatch', 'Нельзя закрыть чужую смену');
            if (attendance.checkedOutAt)
                throw new errors_1.ConflictError('hr_attendance_closed', 'Смена уже закрыта');
            const closed = await tx.hrAttendance.update({ where: { id: attendance.id }, data: { checkedOutAt: new Date(), checkOutKey: key } });
            return { result: closed, events: [{ type: event_types_1.EventType.HrAttendanceClosed, actor: staffId, payload: { attendanceId: closed.id, scheduleId }, refs: [closed.id, scheduleId, staffId] }] };
        });
    }
    async requestAbsence(dto, staffId, rawKey) {
        const key = requireKey(rawKey);
        const startsOn = dateOnly(dto.startsOn, 'startsOn');
        const endsOn = dateOnly(dto.endsOn, 'endsOn');
        if (endsOn < startsOn)
            throw new errors_1.ValidationError('hr_invalid_absence_window', 'Дата окончания раньше даты начала');
        return this.audit.transaction(async (tx) => {
            const replay = await tx.hrAbsence.findUnique({ where: { idempotencyKey: key } });
            if (replay) {
                if (replay.staffId !== staffId || replay.startsOn.getTime() !== startsOn.getTime() || replay.endsOn.getTime() !== endsOn.getTime())
                    throw new errors_1.ConflictError('hr_idempotency_mismatch', 'Ключ уже использован для другого отсутствия');
                return { result: replay, events: [] };
            }
            ...
            if (overlap)
                throw new errors_1.ConflictError('hr_absence_overlap', 'На этот период уже есть запрос отсутствия');
            ...
            return { result: absence, events: [{ type: event_types_1.EventType.HrAbsenceRequested, actor: staffId, payload: { absenceId: absence.id, type: absence.type, startsOn, endsOn }, refs: [absence.id, staffId] }] };
        });
    }
    async decideAbsence(id, status, note, actor) {
        if (status !== 'approved' && status !== 'rejected')
            throw new errors_1.ValidationError('hr_invalid_absence_decision', 'Допустимо только одобрить или отклонить');
        return this.audit.transaction(async (tx) => {
            const absence = await tx.hrAbsence.findUnique({ where: { id } });
            if (!absence)
                throw new errors_1.ValidationError('hr_absence_not_found', 'Запрос отсутствия не найден');
            if (absence.status === status)
                return { result: absence, events: [] };
            if (absence.status !== 'requested')
                throw new errors_1.ConflictError('hr_absence_decided', 'Решение уже принято');
            ...
            return { result: updated, events: [{ type: status === 'approved' ? event_types_1.EventType.HrAbsenceApproved : event_types_1.EventType.HrAbsenceRejected, actor, payload: { absenceId: id, staffId: absence.staffId, note: note?.trim() || null }, refs: [id, absence.staffId] }] };
        });
    }
};
