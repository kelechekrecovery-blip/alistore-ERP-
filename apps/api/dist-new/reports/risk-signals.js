...
exports.computeRepeatReturns = computeRepeatReturns;
exports.computeDiscountFrequency = computeDiscountFrequency;
exports.computeWriteOffSpike = computeWriteOffSpike;
exports.computeMarginLeaks = computeMarginLeaks;
exports.buildRiskSignals = buildRiskSignals;
const reports_service_1 = require("./reports.service");
const WEEK_MS = 7 * 24 * 60 * 60 * 1000;
function computeRepeatReturns(orders, threshold = 3) {
...
function computeDiscountFrequency(sales, thresholdPct = 30) {
    const grouped = new Map();
    for (const sale of sales) {
        const current = grouped.get(sale.staffId) ?? { discountedSales: 0, totalSales: 0 };
        current.totalSales += 1;
        if (sale.gross > 0 && sale.total < sale.gross)
            current.discountedSales += 1;
...
function computeWriteOffSpike(movements, now, minimumCurrentQty = 3) {
    let currentQty = 0;
    let previousQty = 0;
    let currentCount = 0;
    for (const movement of movements) {
        const age = now.getTime() - movement.createdAt.getTime();
        if (age < 0 || age >= WEEK_MS * 2)
            continue;
...
function buildRiskSignals(input, now) {
    const signals = [];
    for (const s of input.cashDiscrepancies) {
        const unexplained = !s.closeReason || s.closeReason === shifts_service_1.BLIND_COUNT_REASON;
        signals.push({
    ...
    for (const c of input.codOutstanding) {
        const stale = now.getTime() - c.createdAt.getTime() > reports_service_1.COD_STALE_MS;
        signals.push({
...
//# sourceMappingURL=risk-signals.js.map