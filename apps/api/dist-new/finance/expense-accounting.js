...
exports.expenseAccountingSnapshot = expenseAccountingSnapshot;
const errors_1 = require("../common/errors");
const RATE_SCALE = 1_000_000;
const TAX_SCALE = 10_000;
const DB_INT_MAX = 2_147_483_647;
function expenseAccountingSnapshot(input) {
    if (!Number.isSafeInteger(input.documentAmount) || input.documentAmount <= 0) {
        throw new errors_1.ValidationError('expense_document_amount_invalid', 'Сумма документа должна быть положительным целым числом');
    }
    if (!Number.isSafeInteger(input.exchangeRateMicros) || input.exchangeRateMicros <= 0) {
        throw new errors_1.ValidationError('expense_exchange_rate_invalid', 'Курс должен быть положительным целым числом');
    }
    if (!Number.isSafeInteger(input.taxRateBps) || input.taxRateBps < 0 || input.taxRateBps > TAX_SCALE) {
        throw new errors_1.ValidationError('expense_tax_rate_invalid', 'Ставка налога должна быть от 0 до 10000 базисных пунктов');
    }
    if (input.taxMode === 'none' && input.taxRateBps !== 0) {
        throw new errors_1.ValidationError('expense_tax_mode_mismatch', 'Для режима без налога ставка должна быть равна нулю');
    }
    if (input.taxMode !== 'none' && input.taxRateBps === 0) {
        throw new errors_1.ValidationError('expense_tax_mode_mismatch', 'Для налогового режима требуется положительная ставка');
    }
    ...
    if (taxBaseAmount <= 0 || amount <= 0 || (input.taxMode !== 'none' && taxAmount <= 0)) {
        throw new errors_1.ValidationError('expense_tax_rounding_invalid', 'Сумма слишком мала для выбранной ставки налога');
    }
    if (![taxBaseAmount, taxAmount, amount].every((value) => Number.isSafeInteger(value) && value <= DB_INT_MAX)) {
        throw new errors_1.ValidationError('expense_amount_overflow', 'Сумма расхода превышает допустимый диапазон');
    }
...
//# sourceMappingURL=expense-accounting.js.map