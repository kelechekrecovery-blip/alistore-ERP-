...
const accounting_journal_1 = require("./accounting-journal");
const expense_accounting_1 = require("./expense-accounting");
...
const INCASSATION_DESTINATIONS = {
...
let FinanceService = class FinanceService {
    ...
    list(status) {
        ...
        if (status && !allowed.includes(status)) {
            throw new errors_1.ValidationError('invalid_expense_status', `Неизвестный статус расхода: ${status}`);
        }
    ...
    async accountingPeriodReadiness(rawPeriod) {
    ...
    async createAccountableAdvance(dto, actor) {
        ...
        if (existing)
            return replayAccountableAdvance(existing, { staffId: dto.staffId, amount: dto.amount, purpose, point: dto.point?.trim() || existing.point, dueAt, fundingAccountCode: dto.fundingAccountCode, paymentReference });
        return this.audit.transaction(async (tx) => {
            ...
            if (replay)
                return { result: replayAccountableAdvance(replay, { staffId: dto.staffId, amount: dto.amount, purpose, point: dto.point?.trim() || replay.point, dueAt, fundingAccountCode: dto.fundingAccountCode, paymentReference }), events: [] };
            ...
            if (!staff)
                throw new errors_1.ValidationError('accountable_advance_staff_not_found', 'Сотрудник для аванса не найден');
            if (!staff.active)
                throw new errors_1.ConflictError('accountable_advance_staff_inactive', 'Неактивному сотруднику нельзя выдать аванс');
            const point = dto.point?.trim() || staff.point;
            if (dueAt && dueAt < new Date())
                throw new errors_1.ValidationError('accountable_advance_due_date_invalid', 'Срок отчёта не может быть в прошлом');
            const advanceId = (0, node_crypto_1.randomUUID)();
            ...
            return {
                ...
                events: [
                    { type: event_types_1.EventType.AccountableAdvanceIssued, actor, payload: { advanceId, staffId: dto.staffId, amount: dto.amount, point, accountingEntryId: entry.id }, refs: [advanceId, dto.staffId, entry.id] },
                    { type: event_types_1.EventType.AccountingEntryPosted, actor, payload: { accountingEntryId: entry.id, sourceType: 'finance.accountable_advance.issue', sourceRef: advanceId, amount: dto.amount }, refs: [entry.id, advanceId, dto.staffId] },
                ],
            };
        });
    }
    async settleAccountableAdvance(id, dto, actor) {
        ...
        if (existing)
            return replayAccountableAdvanceSettlement(existing, id, dto.amount, dto.expenseAccountCode, description);
        return this.audit.transaction(async (tx) => {
            ...
            if (replay)
                return { result: replayAccountableAdvanceSettlement(replay, id, dto.amount, dto.expenseAccountCode, description), events: [] };
            ...
            if (!advance)
                throw new errors_1.ValidationError('accountable_advance_not_found', 'Подотчётный аванс не найден');
            if (advance.status === 'settled')
                throw new errors_1.ConflictError('accountable_advance_settled', 'Закрытый подотчётный аванс нельзя дополнить расходом');
            const settlementId = (0, node_crypto_1.randomUUID)();
            const entry = await (0, accounting_journal_1.postAccountingEntryOnTx)(tx, {
            ...
            return {
                ...
                events: [
                    { type: event_types_1.EventType.AccountableAdvanceSettled, actor, payload: { advanceId: id, settlementId, staffId: advance.staffId, amount: dto.amount, balance: accountableAdvanceBalance(updated), accountingEntryId: entry.id }, refs: [id, settlementId, advance.staffId, entry.id] },
                    { type: event_types_1.EventType.AccountingEntryPosted, actor, payload: { accountingEntryId: entry.id, sourceType: 'finance.accountable_advance.settlement', sourceRef: settlementId, amount: dto.amount }, refs: [entry.id, settlementId, id] },
                ],
            };
        });
    }
    async returnAccountableAdvance(id, dto, actor) {
        return this.closeAccountableAdvance(id, dto, actor, 'return');
    }
    async reimburseAccountableAdvance(id, dto, actor) {
        return this.closeAccountableAdvance(id, dto, actor, 'reimburse');
    }
    async closeAccountableAdvance(id, dto, actor, kind) {
        ...
        if (existing)
            return replayAccountableAdvanceClose(existing, id, dto, kind);
        return this.audit.transaction(async (tx) => {
            await tx.$executeRaw `SELECT pg_advisory_xact_lock(hashtext(${`accountable-advance:${id}`}))`;
            ...
            if (kind === 'return' && balance <= 0)
                throw new errors_1.ConflictError('accountable_advance_no_return_due', 'Нет остатка аванса для возврата');
            if (kind === 'reimburse' && balance >= 0)
                throw new errors_1.ConflictError('accountable_advance_no_reimbursement_due', 'Нет перерасхода для компенсации');
            const due = kind === 'return' ? balance : -balance;
            if (dto.amount > due)
                throw new errors_1.ConflictError('accountable_advance_close_exceeds_balance', `Сумма превышает доступный остаток: ${due}`);
            ...
            const eventType = kind === 'return' ? event_types_1.EventType.AccountableAdvanceReturned : event_types_1.EventType.AccountableAdvanceReimbursed;
            return {
                ...
                events: [
                    ...
                    { type: event_types_1.EventType.AccountingEntryPosted, actor, payload: { accountingEntryId: entry.id, sourceType, sourceRef: actionId, amount: dto.amount }, refs: [entry.id, actionId, id] },
                ],
            };
        });
    }
    async createFixedAsset(dto, actor) {
        ...
        if (inServiceAt < acquiredAt)
            throw new errors_1.ValidationError('fixed_asset_service_before_acquisition', 'Дата ввода в эксплуатацию не может быть раньше покупки');
        if (dto.usefulLifeMonths > dto.acquisitionCost)
            throw new errors_1.ValidationError('fixed_asset_useful_life_too_long', 'Срок амортизации в месяцах не может превышать стоимость в сомах');
        const input = {
        ...
        if (existing)
            return replayFixedAsset(existing, input);
        return this.audit.transaction(async (tx) => {
            ...
            if (replay)
                return { result: replayFixedAsset(replay, input), events: [] };
            const byNumber = await tx.fixedAsset.findUnique({ where: { assetNumber: input.assetNumber }, select: { id: true } });
            if (byNumber)
                throw new errors_1.ConflictError('fixed_asset_number_exists', 'Инвентарный номер основного средства уже используется');
            ...
            return {
                ...
                events: [
                    { type: event_types_1.EventType.FixedAssetAcquired, actor, payload: { fixedAssetId: asset.id, assetNumber: asset.assetNumber, acquisitionCost: asset.acquisitionCost, accountingEntryId: accountingEntry.id }, refs: [asset.id, asset.assetNumber, accountingEntry.id] },
                    { type: event_types_1.EventType.AccountingEntryPosted, actor, payload: { accountingEntryId: accountingEntry.id, sourceType: 'finance.fixed_asset.acquisition', sourceRef: asset.id, amount: asset.acquisitionCost }, refs: [accountingEntry.id, asset.id, asset.assetNumber] },
                ],
            };
        });
    }
    async depreciateFixedAsset(id, dto, actor) {
        ...
        if (existing)
            return replayFixedAssetDepreciation(existing, id, period);
        return this.audit.transaction(async (tx) => {
            ...
            if (replay)
                return { result: replayFixedAssetDepreciation(replay, id, period), events: [] };
            const asset = await tx.fixedAsset.findUnique({ where: { id }, include: { depreciationEntries: { orderBy: { period: 'asc' } } } });
            if (!asset)
                throw new errors_1.ValidationError('fixed_asset_not_found', 'Основное средство не найдено');
            if (asset.status === 'disposed')
                throw new errors_1.ConflictError('fixed_asset_disposed', 'Для списанного основного средства амортизация запрещена');
            const index = asset.depreciationEntries.length;
            if (index >= asset.usefulLifeMonths || asset.accumulatedDepreciation >= asset.acquisitionCost)
                throw new errors_1.ConflictError('fixed_asset_fully_depreciated', 'Основное средство уже полностью самортизировано');
            const expectedPeriod = addMonthsToPeriod(monthKey(asset.inServiceAt), index);
            if (period !== expectedPeriod)
                throw new errors_1.ConflictError('fixed_asset_depreciation_sequence', `Следующий период амортизации: ${expectedPeriod}`);
            ...
            const depreciationId = (0, node_crypto_1.randomUUID)();
            const accountingEntry = await (0, accounting_journal_1.postAccountingEntryOnTx)(tx, {
            ...
            return {
                ...
                events: [
                    { type: event_types_1.EventType.FixedAssetDepreciated, actor, payload: { fixedAssetId: asset.id, depreciationId, assetNumber: asset.assetNumber, period, amount, accountingEntryId: accountingEntry.id }, refs: [asset.id, depreciationId, accountingEntry.id, period] },
                    { type: event_types_1.EventType.AccountingEntryPosted, actor, payload: { accountingEntryId: accountingEntry.id, sourceType: 'finance.fixed_asset.depreciation', sourceRef: depreciationId, amount }, refs: [accountingEntry.id, depreciationId, asset.id, period] },
                ],
            };
        });
    }
    async createOpeningBalance(dto, actor) {
        const period = validateMonth(dto.period);
        ...
        if (new Set(lines.map((line) => line.accountCode)).size !== lines.length) {
            throw new errors_1.ValidationError('opening_balance_duplicate_account', 'Счёт может присутствовать в opening balance только один раз');
        }
        if (lines.some((line) => (line.debit > 0 && line.credit > 0) || (line.debit === 0 && line.credit === 0))) {
            throw new errors_1.ValidationError('opening_balance_line_side_invalid', 'Каждая строка opening balance должна иметь только дебет или кредит');
        }
        const debit = lines.reduce((sum, line) => sum + line.debit, 0);
        const credit = lines.reduce((sum, line) => sum + line.credit, 0);
        if (!Number.isSafeInteger(debit) || !Number.isSafeInteger(credit) || debit <= 0 || debit !== credit) {
            throw new errors_1.ValidationError('opening_balance_unbalanced', 'Opening balance должен быть сбалансирован и больше нуля');
        }
        ...
        if (existing)
            return replayOpeningBalance(existing, fingerprint);
        try {
            return await this.audit.transaction(async (tx) => {
                ...
                if (replay)
                    return { result: replayOpeningBalance(replay, fingerprint), events: [] };
                const periodExisting = await tx.accountingOpeningBalance.findUnique({ where: { period }, select: { id: true, documentNumber: true } });
                if (periodExisting)
                    throw new errors_1.ConflictError('opening_balance_period_exists', `Opening balance за период ${period} уже создан`);
                const openingBalanceId = (0, node_crypto_1.randomUUID)();
                ...
                return {
                    ...
                    events: [
                        ...
                        { type: event_types_1.EventType.AccountingEntryPosted, actor, payload: { accountingEntryId: entry.id, sourceType: 'finance.opening-balance', sourceRef: openingBalanceId, amount: debit }, refs: [entry.id, openingBalanceId, period] },
                    ],
                };
            });
        }
            ...
            if ((0, prisma_errors_1.isUniqueConstraintViolation)(error)) {
                ...
                if (replay)
                    return replayOpeningBalance(replay, fingerprint);
            }
            throw error;
        }
    }
    listCurrencyRates(query) {
    ...
    async createCurrencyRate(dto, actor) {
        const input = {
        ...
        if (input.currency === input.baseCurrency) {
            throw new errors_1.ValidationError('currency_rate_base_pair_invalid', 'Для KGS используется фиксированный курс 1.0 без отдельной записи');
        }
        const existing = await this.prisma.accountingCurrencyRate.findUnique({ where: { idempotencyKey: dto.idempotencyKey } });
        if (existing)
            return replayCurrencyRate(existing, input);
        try {
            return await this.audit.transaction(async (tx) => {
                ...
                return {
                    ...
                    events: [{
                            type: event_types_1.EventType.FinanceCurrencyRateRecorded,
                            actor,
            ...
            if ((0, prisma_errors_1.isUniqueConstraintViolation)(error)) {
                ...
                if (replay)
                    return replayCurrencyRate(replay, input);
                throw new errors_1.ConflictError('currency_rate_effective_at_conflict', 'Курс этой валюты на указанную дату уже зарегистрирован');
            }
            throw error;
        }
    }
    async fxExposure(query) {
        const asOf = query.asOf ? new Date(query.asOf) : new Date();
        if (Number.isNaN(asOf.getTime()))
            throw new errors_1.ValidationError('invalid_fx_exposure_as_of', 'Дата FX-отчёта некорректна');
        ...
        const expenses = await this.prisma.expense.findMany({
            where: {
                ...
                status: { in: [client_1.ExpenseStatus.submitted, client_1.ExpenseStatus.approved] },
                incurredAt: { lte: asOf },
                ...(point ? { point } : {}),
            },
            include: {
        ...
        for (const rate of rates)
            if (!currentRates.has(rate.currency))
                currentRates.set(rate.currency, rate);
        const rows = expenses.map((expense) => {
            const currentRate = currentRates.get(expense.currency) ?? null;
            let currentBaseAmount = null;
            let valuationStatus = currentRate ? 'ready' : 'missing_rate';
            if (currentRate) {
                try {
                    currentBaseAmount = (0, expense_accounting_1.expenseAccountingSnapshot)({
                    ...
                    if (error instanceof errors_1.ValidationError && error.code === 'expense_amount_overflow') {
        ...
        for (const row of rows) {
            ...
            if (row.valuationStatus === 'missing_rate')
                total.missingRateDocuments += 1;
            if (row.valuationStatus === 'overflow')
                total.overflowDocuments += 1;
        ...
        return {
            ...
            coverage: {
                complete: true,
                statuses: [client_1.ExpenseStatus.submitted, client_1.ExpenseStatus.approved],
                limit: 500,
    ...
    listBankStatements(accountCode) {
    ...
    async createCashIncassation(shiftId, dto, actor, idempotencyKey) {
        return this.audit.transaction(async (tx) => {
            ...
            if (!destinationSpec)
                throw new errors_1.ValidationError('cash_incassation_destination_invalid', `Счет ${destinationCode} не разрешен для инкассации`);
            const existing = await tx.cashIncassation.findUnique({ where: { idempotencyKey }, include: { accountingEntry: true } });
            if (existing) {
                if (existing.shiftId !== shiftId || existing.amount !== dto.amount || existing.destinationCode !== destinationCode)
                    throw new errors_1.ConflictError('cash_incassation_idempotency_mismatch', 'Ключ инкассации уже использован для другой суммы, смены или назначения');
            ...
            if (!shift)
                throw new errors_1.ValidationError('shift_not_found', `Смена ${shiftId} не найдена`);
            if (!shift.closedAt || shift.closeCash === null)
                throw new errors_1.ConflictError('shift_must_be_closed', 'Инкассация доступна только после закрытия и пересчета смены');
            ...
            if (dto.amount > available)
                throw new errors_1.ConflictError('cash_incassation_exceeds_drawer', `Инкассация ${dto.amount} сом превышает доступный остаток кассы ${available} сом`);
            ...
            if (!destination?.active || destination.type !== destinationSpec.type)
                throw new errors_1.ValidationError('cash_incassation_destination_invalid', `Счет назначения ${destinationCode} не настроен`);
    ...
    async importBankStatement(dto, actor) {
        const periodStart = new Date(dto.periodStart);
        const periodEnd = new Date(dto.periodEnd);
        if (Number.isNaN(periodStart.getTime()) || Number.isNaN(periodEnd.getTime()) || periodStart >= periodEnd) {
            throw new errors_1.ValidationError('invalid_bank_statement_period', 'Период банковской выписки некорректен');
        }
        if (dto.lines.some((line) => line.amount === 0) || new Set(dto.lines.map((line) => line.externalId.trim())).size !== dto.lines.length) {
            throw new errors_1.ValidationError('invalid_bank_statement_lines', 'Строки выписки должны иметь уникальный внешний ID и ненулевую сумму');
        }
        const movement = dto.lines.reduce((sum, line) => sum + line.amount, 0);
        if (dto.openingBalance + movement !== dto.closingBalance) {
            throw new errors_1.ConflictError('bank_statement_balance_mismatch', 'Начальный баланс плюс движения не равен конечному балансу');
        }
        const existing = await this.prisma.bankStatement.findUnique({ where: { idempotencyKey: dto.idempotencyKey }, include: { lines: true } });
        if (existing) {
            if (existing.statementNumber !== dto.statementNumber.trim() || existing.closingBalance !== dto.closingBalance)
                throw new errors_1.ConflictError('bank_statement_idempotency_conflict', 'Ключ выписки уже использован для других данных');
            return { ...existing, idempotent: true };
        }
        return this.audit.transaction(async (tx) => {
            const account = await tx.accountingAccount.findUnique({ where: { code: dto.accountCode.trim() } });
            if (!account || account.type !== 'asset' || !account.active)
                throw new errors_1.ValidationError('bank_statement_account_invalid', 'Счёт выписки должен быть активным счётом актива');
    ...
    async reconcileBankStatementLine(id, dto, actor) {
        return this.audit.transaction(async (tx) => {
            ...
            if (!line)
                throw new errors_1.ValidationError('bank_statement_line_not_found', `Строка выписки ${id} не найдена`);
            if (line.reconciliationKey === dto.idempotencyKey)
                return { result: { ...line, idempotent: true }, events: [] };
            if (line.status === 'matched')
                throw new errors_1.ConflictError('bank_statement_line_matched', 'Строка уже сверена другим ключом');
            ...
            if (entry.occurredAt < line.statement.periodStart || entry.occurredAt >= line.statement.periodEnd)
                throw new errors_1.ConflictError('bank_statement_entry_outside_period', 'Проводка находится вне периода выписки');
            ...
            if (movement !== line.amount)
                throw new errors_1.ConflictError('bank_statement_amount_mismatch', 'Сумма строки не совпадает с движением по банковскому счёту в проводке');
            const duplicate = await tx.bankStatementLine.findUnique({ where: { matchedEntryId: entry.id }, select: { id: true } });
            if (duplicate && duplicate.id !== id)
                throw new errors_1.ConflictError('accounting_entry_already_reconciled', 'Проводка уже связана с другой строкой выписки');
    ...
    async closeAccountingPeriod(rawPeriod, dto, actor) {
        const period = validateMonth(rawPeriod);
        return this.audit.transaction(async (tx) => {
            ...
            if ((!existing || existing.status === 'open') && dto.status !== 'soft_closed') {
                throw new errors_1.ConflictError('accounting_period_soft_close_required', 'Сначала выполните мягкое закрытие периода');
            }
            ...
            if (dto.status === 'hard_closed') {
                ...
                if (!readiness.ready) {
                    ...
                    throw new errors_1.ConflictError(first.code, first.message);
                }
                const taxSettlement = await tx.accountingTaxSettlement.findUnique({
                    where: { period_point: { period, point: '' } },
                });
                if (!taxSettlement) {
                    throw new errors_1.ConflictError('tax_period_settlement_required', 'Перед закрытием периода зафиксируйте итоговую налоговую сверку по всем точкам');
                }
                ...
                if (taxSettlement.outputTax !== currentTax.outputTax
                    ...
                    throw new errors_1.ConflictError('tax_period_settlement_stale', 'Налоговые движения изменились после сверки; период нельзя закрыть');
                }
            }
    ...
    async taxPeriod(rawPeriod, rawPoint) {
        const period = validateMonth(rawPeriod);
        const point = normalizePoint(rawPoint);
        return this.prisma.$transaction((tx) => taxPeriodOnTx(tx, period, point));
    }
    async settleTaxPeriod(rawPeriod, dto, actor) {
        const period = validateMonth(rawPeriod);
        if (normalizePoint(dto.point)) {
            throw new errors_1.ValidationError('tax_settlement_global_only', 'Фиксация НДС выполняется по всем точкам; точка доступна только как аналитический фильтр');
        }
        const point = '';
        return this.audit.transaction(async (tx) => {
            ...
            if (replay) {
                if (replay.period !== period || replay.point !== point) {
                    throw new errors_1.ConflictError('idempotency_key_reused', 'Ключ налоговой сверки уже использован для другого периода или точки');
                }
            ...
            if (duplicate) {
                throw new errors_1.ConflictError('tax_period_already_settled', 'Налоговая сверка этого периода и точки уже зафиксирована');
            }
            const accountingPeriod = await tx.accountingPeriod.findUnique({ where: { period } });
            if (accountingPeriod?.status !== 'soft_closed') {
                throw new errors_1.ConflictError('tax_period_soft_close_required', 'Налоговая сверка доступна только после мягкого закрытия периода');
            }
            const report = await taxPeriodOnTx(tx, period, point);
            ...
            if (report.offsetAmount > 0) {
                ...
                accountingEntry = await (0, accounting_journal_1.postAccountingEntryOnTx)(tx, {
    ...
    async supplierAging(query) {
        const reportLimit = 500;
        const asOf = query.asOf ? new Date(query.asOf) : new Date();
        if (Number.isNaN(asOf.getTime()))
            throw new errors_1.ValidationError('invalid_ap_as_of', 'Дата AP aging некорректна');
        ...
        const buckets = ['current', '1_30', '31_60', '61_90', '90_plus', 'paid'];
    ...
    async customerAging(query) {
        const asOf = query.asOf ? new Date(query.asOf) : new Date();
        if (Number.isNaN(asOf.getTime()))
            throw new errors_1.ValidationError('invalid_ar_as_of', 'Дата AR aging некорректна');
        ...
        const rows = debts.map((debt) => {
            ...
            const historicalBalance = Math.min(debt.principal, Math.max(0, debt.balance + paymentsAfterAsOf.reduce((sum, payment) => sum + payment.amount, 0)));
        ...
        const buckets = ['current', '1_30', '31_60', '61_90', '90_plus', 'paid'];
        const totals = Object.fromEntries(buckets.map((bucket) => [bucket, rows.filter((row) => row.bucket === bucket).reduce((sum, row) => sum + row.outstanding, 0)]));
        return {
            asOf,
            rows,
            totals,
    ...
    async customerDebtDrilldown(id, query) {
        const report = await this.customerAging({ ...query, status: undefined });
        const row = report.rows.find((candidate) => candidate.id === id);
        if (!row)
            throw new errors_1.ValidationError('ar_document_not_found', `Долг ${id} не найден на выбранную дату`);
        return { asOf: report.asOf, ...row };
    }
    async accountingJournalExport(query) {
    ...
    async listManualAdjustments(status) {
        const allowed = ['requested', 'approved', 'rejected'];
        if (status && !allowed.includes(status)) {
            throw new errors_1.ValidationError('invalid_manual_adjustment_status', `Неизвестный статус ручной корректировки: ${status}`);
        }
        const approvals = await this.prisma.approval.findMany({
            where: { action: 'manual_adjustment', ...(status ? { status: status } : {}) },
            orderBy: { createdAt: 'desc' },
            take: 200,
        });
        const sourceRefs = approvals.map((approval) => approval.sourceRef).filter((value) => Boolean(value));
        ...
        return approvals.map((approval) => ({
            ...
            snapshot: approval.evidence?.payload ?? null,
            accountingEntry: approval.sourceRef ? bySourceRef.get(approval.sourceRef) ?? null : null,
        }));
    }
    async createManualAdjustment(dto, actor) {
        const description = dto.description.trim();
        const documentNumber = dto.documentNumber.trim();
        const point = dto.point?.trim() || null;
        const occurredAt = new Date(dto.occurredAt);
        if (Number.isNaN(occurredAt.getTime()))
            throw new errors_1.ValidationError('manual_adjustment_date_invalid', 'Дата корректировки некорректна');
        const lines = dto.lines.map((line) => ({
            accountCode: line.accountCode.trim(),
            debit: line.debit ?? 0,
        ...
        if (lines.length < 2 || debit <= 0 || debit !== credit || lines.some((line) => (line.debit > 0) === (line.credit > 0))) {
            throw new errors_1.ValidationError('manual_adjustment_unbalanced', 'Ручная корректировка должна содержать сбалансированные строки');
        }
        ...
        return this.audit.transaction(async (tx) => {
            ...
            if (existing) {
                const existingPayload = existing.evidence?.payload;
                if (existing.action !== 'manual_adjustment' || stableJson(existingPayload) !== stableJson(payload)) {
                    throw new errors_1.ConflictError('manual_adjustment_idempotency_conflict', 'Idempotency key уже использован для другой корректировки');
                }
            ...
            if (sameDocument)
                throw new errors_1.ConflictError('manual_adjustment_document_exists', 'Первичный документ корректировки уже зарегистрирован');
            const approval = await tx.approval.create({
                data: {
                    ...
                    status: 'requested',
                    evidence: { payload, evidence: { documentNumber, amount: debit } },
                },
            });
            return {
                result: { approvalId: approval.id, status: approval.status, idempotent: false, snapshot: payload },
                events: [{ type: event_types_1.EventType.FinanceManualAdjustmentRequested, actor, payload: { approvalId: approval.id, documentNumber, amount: debit }, refs: [approval.id, documentNumber] }],
            };
        });
    }
    async reverseAccountingEntry(id, dto, actor) {
        return this.audit.transaction(async (tx) => {
            ...
            if (!original)
                throw new errors_1.ValidationError('accounting_entry_not_found', `Проводка ${id} не найдена`);
            if (original.reversalOfId)
                throw new errors_1.ConflictError('accounting_reversal_of_reversal', 'Нельзя сторнировать уже обратную проводку');
            const existingReversal = await tx.accountingJournalEntry.findUnique({ where: { reversalOfId: id }, include: { lines: true } });
            if (existingReversal)
                return { result: { ...existingReversal, idempotent: true }, events: [] };
            const reversal = await (0, accounting_journal_1.postAccountingEntryOnTx)(tx, {
    ...
    async trialBalance(query) {
        ...
        const rows = accounts.map((account) => {
            ...
            return { code: account.code, name: account.name, type: account.type, debit, credit, balance: (0, accounting_journal_1.normalBalance)(account.type, debit, credit) };
        });
    ...
    async financialStatements(query) {
        const period = accountingPeriod(query);
        const point = normalizePoint(query.point);
        const where = {
            occurredAt: { gte: period.from, lt: period.to },
        ...
        const rows = [...totals.entries()].map(([code, value]) => ({ code, ...value, balance: (0, accounting_journal_1.normalBalance)(value.type, value.debit, value.credit) }));
    ...
    async setBudget(dto, actor) {
        ...
        try {
            return await this.audit.transaction(async (tx) => {
                ...
                if (command)
                    return { result: replayBudgetCommand(command, input), events: [] };
                ...
                return {
                    result: response,
                    events: [{
                            type: event_types_1.EventType.FinanceBudgetSet,
                            actor,
                            payload: {
                                budgetId: budget.id,
                                period: input.period,
            ...
            if ((0, prisma_errors_1.isUniqueConstraintViolation)(error)) {
                ...
                if (command)
                    return replayBudgetCommand(command, input);
            }
            throw error;
        }
    }
    async planFact(period, point) {
        ...
        const rows = finance_dto_1.EXPENSE_CATEGORIES.map((category) => {
    ...
    async createSettlement(dto, actor) {
        ...
        if (new Set(entries.map((entry) => `${entry.sourceType}:${entry.sourceRef}`)).size !== entries.length) {
            throw new errors_1.ValidationError('duplicate_settlement_source', 'Один источник нельзя добавить в сверку дважды');
        }
        return this.audit.transaction(async (tx) => {
            ...
            if (replay)
                return { result: replaySettlementCommand(replay, 'create', fingerprint), events: [] };
            const sources = [];
            ...
            for (const entry of entries) {
                ...
                if (existing)
                    throw new errors_1.ConflictError('settlement_source_already_used', `Источник ${entry.sourceRef} уже сверен`);
                if (entry.actualAmount !== source.expectedAmount && !entry.reason) {
                    throw new errors_1.ValidationError('settlement_reason_required', `Расхождение по источнику ${entry.sourceRef} требует причину`);
                }
                sources.push(source);
            }
            ...
            await tx.financeSettlementCommand.create({
                data: { idempotencyKey: dto.idempotencyKey, action: 'create', runId: run.id, input: fingerprint, response: jsonValue(response), createdBy: actor },
            });
            return { result: response, events: [{
                        type: event_types_1.EventType.FinanceSettlementCreated, actor,
    ...
    async resolveSettlement(runId, dto, actor) {
        ...
        return this.audit.transaction(async (tx) => {
            ...
            const replay = await tx.financeSettlementCommand.findUnique({ where: { idempotencyKey: dto.idempotencyKey } });
            if (replay)
                return { result: replaySettlementCommand(replay, 'resolve', input), events: [] };
            const run = await tx.financeSettlementRun.findUnique({ where: { id: runId }, include: { lines: true } });
            if (!run)
                throw new errors_1.ValidationError('settlement_not_found', `Сверка ${runId} не найдена`);
            if (run.status === 'closed')
                throw new errors_1.ConflictError('settlement_closed', 'Закрытую сверку нельзя изменить');
            const line = run.lines.find((candidate) => candidate.id === dto.lineId);
            if (!line)
                throw new errors_1.ValidationError('settlement_line_not_found', `Строка ${dto.lineId} не найдена в сверке`);
            ...
            await tx.financeSettlementCommand.create({ data: { idempotencyKey: dto.idempotencyKey, action: 'resolve', runId, input: input, response: jsonValue(response), createdBy: actor } });
            const variance = line.actualAmount + dto.adjustmentAmount - line.expectedAmount;
            return { result: response, events: [
                    { type: event_types_1.EventType.FinanceSettlementAdjustmentCreated, actor, payload: { runId, lineId: line.id, sourceType: line.sourceType, sourceRef: line.sourceRef, observedActual: line.actualAmount, adjustmentAmount: dto.adjustmentAmount, reason: dto.reason.trim() }, refs: [runId, line.sourceRef] },
                    { type: event_types_1.EventType.FinanceSettlementDisputeResolved, actor, payload: { runId, lineId: line.id, previousVariance: line.variance, variance, reason: dto.reason.trim() }, refs: [runId, line.sourceRef] },
                ] };
        });
    }
    async closeSettlement(runId, idempotencyKey, actor) {
        const input = { runId };
        return this.audit.transaction(async (tx) => {
            ...
            if (replay)
                return { result: replaySettlementCommand(replay, 'close', input), events: [] };
            const run = await tx.financeSettlementRun.findUnique({ where: { id: runId }, include: { lines: { orderBy: { createdAt: 'asc' } } } });
            if (!run)
                throw new errors_1.ValidationError('settlement_not_found', `Сверка ${runId} не найдена`);
            if (run.status === 'closed')
                throw new errors_1.ConflictError('settlement_closed', 'Сверка уже закрыта другим ключом');
            if (run.variance !== 0 || run.lines.some((line) => line.variance !== 0)) {
                throw new errors_1.ConflictError('settlement_has_disputes', 'Сначала устраните все расхождения');
            }
            const now = new Date();
            ...
            for (const line of paymentLines) {
                ...
                if (changed.count !== 1)
                    throw new errors_1.ConflictError('settlement_source_changed', `Платёж ${line.sourceRef} изменился во время сверки`);
            }
            ...
            await tx.financeSettlementCommand.create({ data: { idempotencyKey, action: 'close', runId, input: input, response: jsonValue(response), createdBy: actor } });
            return { result: response, events: [
                    ...paymentLines.map((line) => ({ type: event_types_1.EventType.PaymentReconciled, actor, payload: { runId, paymentId: line.sourceRef, amount: line.expectedAmount }, refs: [runId, line.sourceRef] })),
                    { type: event_types_1.EventType.FinanceSettlementClosed, actor, payload: { runId, expectedTotal: run.expectedTotal, actualTotal: run.actualTotal, sources: run.lines.length }, refs: [runId, ...run.lines.map((line) => line.sourceRef)] },
                ] };
        });
    }
    async create(dto, actor) {
        ...
        if (existing)
            return replayExpense(existing, input);
        if (input.supplierId) {
        ...
        try {
            return await this.audit.transaction(async (tx) => {
                ...
                return {
                    result: expense,
                    events: [{
                            type: event_types_1.EventType.ExpenseSubmitted,
                            actor,
            ...
            if ((0, prisma_errors_1.isUniqueConstraintViolation)(error)) {
                ...
                if (replay)
                    return replayExpense(replay, input);
            }
            throw error;
        }
    }
    ...
    async pay(id, dto, actor) {
        try {
            return await this.audit.transaction(async (tx) => {
                ...
                if (!expense)
                    throw new errors_1.ValidationError('expense_not_found', `Расход ${id} не найден`);
                if (expense.status === 'paid') {
                    if (expense.paymentKey !== dto.idempotencyKey || expense.paymentAccountCode !== dto.fundingAccountCode || expense.paymentReference !== (dto.paymentReference?.trim() || null)) {
                        throw new errors_1.ConflictError('expense_already_paid', 'Расход уже выплачен с другим idempotency key');
                    }
                ...
                if (expense.status !== 'approved') {
                    throw new errors_1.ConflictError('expense_not_approved', 'Выплатить можно только согласованный расход');
                }
                ...
                const entry = await (0, accounting_journal_1.postAccountingEntryOnTx)(tx, {
                    ...
                    createdBy: actor,
                    lines: [
                        { accountCode: (0, accounting_journal_1.expenseAccountCode)(expense.category), debit: expense.taxBaseAmount, memo: expense.description },
                ...
                return {
                    ...
                    events: [
                        {
                            type: event_types_1.EventType.ExpensePaid,
                            actor,
            ...
            if ((0, prisma_errors_1.isUniqueConstraintViolation)(error)) {
                throw new errors_1.ConflictError('expense_payment_idempotency_conflict', 'Ключ выплаты уже использован для другой операции');
            }
            throw error;
        }
    }
    transitionSubmitted(id, actor, to, note) {
        return this.audit.transaction(async (tx) => {
            await tx.$queryRaw `SELECT id FROM "Expense" WHERE id = ${id} FOR UPDATE`;
            ...
            if (expense.status !== 'submitted') {
                throw new errors_1.ConflictError('expense_already_reviewed', 'Расход уже рассмотрен');
            }
            ...
            return {
                result: updated,
                events: [{
                        type: to === 'approved' ? event_types_1.EventType.ExpenseApproved : event_types_1.EventType.ExpenseRejected,
                        actor,
...
async function taxPeriodOnTx(tx, period, point) {
...
function settlementPeriod(query) {
    const from = new Date(query.from);
    const to = new Date(query.to);
    if (!(from < to))
        throw new errors_1.ValidationError('invalid_settlement_period', 'Конец периода должен быть позже начала');
    if (to.getTime() - from.getTime() > 366 * 24 * 60 * 60 * 1000)
        throw new errors_1.ValidationError('settlement_period_too_long', 'Период сверки не может превышать 366 дней');
    return { from, to };
}
function accountingPeriod(query) {
    ...
    const to = new Date(query.to);
    if (!(from < to))
        throw new errors_1.ValidationError('invalid_accounting_period', 'Конец периода должен быть позже начала');
    if (to.getTime() - from.getTime() > 366 * 24 * 60 * 60 * 1000)
        throw new errors_1.ValidationError('accounting_period_too_long', 'Период отчёта не может превышать 366 дней');
    return { from, to };
}
async function settlementSourceOnTx(tx, sourceType, sourceRef, period, point) {
    if (sourceType === 'provider_payment' || sourceType === 'refund') {
        ...
        if (!payment || !expectedSign || !expectedStatus || ['cash', 'gift_card'].includes(payment.method) || payment.createdAt < period.from || payment.createdAt >= period.to || (point && paymentPoint !== point)) {
            throw new errors_1.ValidationError('settlement_source_invalid', `Платёж ${sourceRef} недоступен для этой сверки`);
        }
    ...
    if (sourceType === 'pos_shift') {
        ...
        if (!shift?.closedAt || shift.closedAt < period.from || shift.closedAt >= period.to || (point && shift.point !== point))
            throw new errors_1.ValidationError('settlement_source_invalid', `POS смена ${sourceRef} недоступна для этой сверки`);
        const expected = shift.payments.reduce((sum, payment) => sum + payment.amount, 0);
    ...
    const runPoint = courierRunPoint(run?.orders ?? []) ?? '';
    if (!run?.handedOver || !run.handedOverAt || run.handedOverAt < period.from || run.handedOverAt >= period.to || (point && runPoint !== point))
        throw new errors_1.ValidationError('settlement_source_invalid', `COD рейс ${sourceRef} недоступен для этой сверки`);
...
function courierRunPoint(orders) {
    const points = new Set(orders.map((order) => order.storePointCode || order.storePointName).filter((value) => Boolean(value)));
    return points.size === 1 ? [...points][0] : null;
}
function replaySettlementCommand(command, action, input) {
    if (command.action !== action || stableJson(command.input) !== stableJson(input))
        throw new errors_1.ConflictError('idempotency_key_reused', 'Idempotency key уже использован другой командой сверки');
    return { ...command.response, idempotent: true };
}
...
async function normalizedExpense(prisma, dto) {
    ...
    if (currency === 'KGS') {
        if (dto.exchangeRateId)
            throw new errors_1.ValidationError('expense_exchange_rate_not_allowed', 'Для расхода в KGS отдельный курс не требуется');
    }
    else {
        if (!dto.exchangeRateId)
            throw new errors_1.ValidationError('expense_exchange_rate_required', `Для валюты ${currency} требуется зарегистрированный курс`);
        ...
        if (!rate || rate.currency !== currency || rate.baseCurrency !== 'KGS') {
            throw new errors_1.ValidationError('expense_exchange_rate_mismatch', 'Курс не соответствует валюте документа и базовой валюте KGS');
        }
        if (rate.effectiveAt.getTime() > rateDate.getTime()) {
            throw new errors_1.ValidationError('expense_exchange_rate_future', 'Нельзя использовать курс, который вступает в силу после даты расхода');
        }
    ...
    const snapshot = (0, expense_accounting_1.expenseAccountingSnapshot)({
        documentAmount: dto.amount,
        exchangeRateMicros,
        taxMode,
        taxRateBps,
...
function replayExpense(expense, input) {
    ...
    if (!same)
        throw new errors_1.ConflictError('idempotency_key_reused', 'Idempotency key уже использован с другим расходом');
    return { ...expense, idempotent: true };
}
function replayOpeningBalance(document, input) {
    const actual = {
    ...
    if (stableJson(actual) !== stableJson(input)) {
        throw new errors_1.ConflictError('opening_balance_idempotency_conflict', 'Ключ opening balance уже использован для других данных');
    }
    return { ...document, idempotent: true };
}
function fixedAssetDate(value, field) {
    const date = new Date(value);
    if (Number.isNaN(date.getTime())) {
        throw new errors_1.ValidationError('fixed_asset_invalid_date', `${field}: неверная дата`);
    }
    return date;
}
function optionalFinanceDate(value, field) {
    if (!value)
        return null;
...
function replayFixedAsset(asset, input) {
    ...
    if (!same)
        throw new errors_1.ConflictError('fixed_asset_idempotency_conflict', 'Ключ основного средства уже использован для других данных');
    return { ...asset, idempotent: true };
}
function replayFixedAssetDepreciation(depreciation, fixedAssetId, period) {
    if (depreciation.fixedAssetId !== fixedAssetId || depreciation.period !== period) {
        throw new errors_1.ConflictError('fixed_asset_depreciation_idempotency_conflict', 'Ключ амортизации уже использован для другого актива или периода');
    }
    return { ...depreciation, idempotent: true };
}
function accountableAdvanceBalance(advance) {
    return advance.amount - advance.settledAmount - advance.returnedAmount + advance.reimbursedAmount;
}
function accountableAdvanceStatus(amount, settledAmount, returnedAmount, reimbursedAmount) {
    const balance = amount - settledAmount - returnedAmount + reimbursedAmount;
    if (balance === 0)
        return 'settled';
    if (settledAmount === 0 && returnedAmount === 0 && reimbursedAmount === 0)
        return 'open';
    return 'partially_settled';
}
function accountableAdvanceResponse(advance) {
    return { ...advance, balance: accountableAdvanceBalance(advance) };
}
function replayAccountableAdvance(advance, input) {
    ...
    if (!same)
        throw new errors_1.ConflictError('accountable_advance_idempotency_conflict', 'Ключ подотчётного аванса уже использован для других данных');
    return { ...accountableAdvanceResponse(advance), idempotent: true };
}
function replayAccountableAdvanceSettlement(settlement, advanceId, amount, expenseAccountCode, description) {
    if (settlement.advanceId !== advanceId || settlement.amount !== amount || settlement.expenseAccountCode !== expenseAccountCode || settlement.description !== description) {
        throw new errors_1.ConflictError('accountable_advance_settlement_idempotency_conflict', 'Ключ закрытия расхода уже использован для других данных');
    }
...
function accountableAdvanceActionResponse(action) {
    return {
        id: action.id,
...
function replayAccountableAdvanceClose(action, advanceId, dto, kind) {
    if (action.advanceId !== advanceId || action.amount !== dto.amount || action.fundingAccountCode !== dto.fundingAccountCode || action.paymentReference !== dto.paymentReference.trim()) {
        throw new errors_1.ConflictError('accountable_advance_close_idempotency_conflict', 'Ключ закрытия подотчёта уже использован для других данных');
    }
    return { advance: accountableAdvanceResponse(action.advance), [kind]: action, idempotent: true };
}
function replayCurrencyRate(rate, input) {
    ...
    if (!same)
        throw new errors_1.ConflictError('idempotency_key_reused', 'Idempotency key уже использован с другим курсом');
...
function replayBudgetCommand(command, input) {
    ...
    if (!same)
        throw new errors_1.ConflictError('idempotency_key_reused', 'Idempotency key уже использован для другого бюджета');
    return { ...command.response, idempotent: true };
}
...
async function accountingPeriodReadinessOnTx(tx, period) {
    const [from, to] = periodBounds(period);
...
function validateMonth(period) {
    if (!/^\d{4}-(0[1-9]|1[0-2])$/.test(period)) {
        throw new errors_1.ValidationError('invalid_accounting_period', 'Период должен быть в формате YYYY-MM');
    }
    return period;
}
