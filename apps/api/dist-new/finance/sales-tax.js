...
exports.salesTaxSnapshot = salesTaxSnapshot;
exports.includedTax = includedTax;
exports.cumulativeTaxDelta = cumulativeTaxDelta;
exports.outputTaxMetadata = outputTaxMetadata;
exports.assertTaxClassification = assertTaxClassification;
const errors_1 = require("../common/errors");
const TAX_SCALE = 10_000;
const DB_INT_MAX = 2_147_483_647;
function salesTaxSnapshot(lines, finalMerchandiseAmount) {
    if (lines.length === 0)
        throw new errors_1.ValidationError('sales_tax_lines_required', 'Для расчёта налога нужны позиции продажи');
    ...
    if (!Number.isSafeInteger(finalMerchandiseAmount) || finalMerchandiseAmount < 0 || finalMerchandiseAmount > grossAmount) {
        throw new errors_1.ValidationError('sales_tax_consideration_invalid', 'Итоговая стоимость товаров не совпадает с первичными позициями');
    }
    ...
    const snapshots = ordered.map((line, index) => {
        ...
        if (!Number.isSafeInteger(line.grossAmount) || line.grossAmount < 0) {
            throw new errors_1.ValidationError('sales_tax_line_amount_invalid', 'Сумма позиции должна быть целым неотрицательным числом');
        }
...
function includedTax(grossAmount, taxRateBps) {
    if (!Number.isSafeInteger(grossAmount) || grossAmount < 0) {
        throw new errors_1.ValidationError('sales_tax_amount_invalid', 'Сумма для расчёта налога некорректна');
    }
    assertTaxClassification({ taxCode: taxRateBps === 0 ? 'none' : 'taxable', taxRateBps });
    if (grossAmount === 0 || taxRateBps === 0)
        return 0;
    return roundedRatio(grossAmount, taxRateBps, TAX_SCALE + taxRateBps);
}
function cumulativeTaxDelta(totalTax, documentTotal, processedBefore, movementAmount) {
    for (const value of [totalTax, documentTotal, processedBefore, movementAmount]) {
        if (!Number.isSafeInteger(value) || value < 0) {
            throw new errors_1.ValidationError('sales_tax_allocation_invalid', 'Параметры распределения налога некорректны');
        }
    }
    if (documentTotal === 0)
        return 0;
    if (totalTax > documentTotal || processedBefore + movementAmount > documentTotal) {
        throw new errors_1.ValidationError('sales_tax_allocation_exceeded', 'Распределение налога превышает сумму первичного документа');
    }
...
function outputTaxMetadata(lines) {
    const taxable = lines.filter((line) => line.taxAmount > 0);
    if (taxable.length === 0)
        return { taxCode: 'none', taxRateBps: 0 };
...
function assertTaxClassification(input) {
    if (!input.taxCode.trim() || input.taxCode.length > 64) {
        throw new errors_1.ValidationError('sales_tax_code_invalid', 'Налоговый код должен содержать от 1 до 64 символов');
    }
    if (!Number.isSafeInteger(input.taxRateBps) || input.taxRateBps < 0 || input.taxRateBps > TAX_SCALE) {
        throw new errors_1.ValidationError('sales_tax_rate_invalid', 'Ставка налога должна быть от 0 до 10000 базисных пунктов');
    }
}
function floorRatio(value, numerator, denominator) {
    ...
    if (!Number.isSafeInteger(result) || result > DB_INT_MAX) {
        throw new errors_1.ValidationError('sales_tax_amount_overflow', 'Сумма налога превышает допустимый диапазон');
    }
    return result;
}
