...
exports.DEFAULT_EARN_RATE_BPS = void 0;
exports.loyaltyBalanceOnTx = loyaltyBalanceOnTx;
exports.redeemLoyaltyOnTx = redeemLoyaltyOnTx;
exports.loyaltyEarnAmount = loyaltyEarnAmount;
exports.earnLoyaltyOnTx = earnLoyaltyOnTx;
exports.reconcileRefundLoyaltyOnTx = reconcileRefundLoyaltyOnTx;
const event_types_1 = require("../audit/event-types");
const errors_1 = require("../common/errors");
exports.DEFAULT_EARN_RATE_BPS = 100;
async function loyaltyBalanceOnTx(tx, customerId, now = new Date()) {
...
async function redeemLoyaltyOnTx(tx, input, events) {
    if (!Number.isInteger(input.requested) || input.requested < 0) {
        throw new errors_1.ValidationError('invalid_loyalty_amount', 'Количество бонусов должно быть целым неотрицательным числом');
    }
    if (input.requested === 0)
        return 0;
    if (input.requested > input.maximum) {
        throw new errors_1.ValidationError('loyalty_exceeds_order', `Для этого заказа можно списать не более ${input.maximum} бонусов`);
    }
    ...
    if (input.requested > available) {
        throw new errors_1.ConflictError('insufficient_loyalty_balance', `Доступно только ${available} бонусов`);
    }
    ...
    events.push({
        type: event_types_1.EventType.LoyaltyRedeemed,
        actor: input.actor,
...
function loyaltyEarnAmount(paidTotal, earnRateBps) {
    return Math.floor((Math.max(0, paidTotal) * (earnRateBps ?? exports.DEFAULT_EARN_RATE_BPS)) / 10_000);
}
async function earnLoyaltyOnTx(tx, input, events) {
    ...
    events.push({
        type: event_types_1.EventType.LoyaltyEarned,
        actor: input.actor,
        payload: { customerId: input.customerId, orderId: input.orderId, paymentId: input.paymentId ?? null, entryId: entry.id, amount },
        refs: [input.customerId, input.orderId, input.paymentId, entry.id].filter((value) => Boolean(value)),
    });
    return amount;
}
async function reconcileRefundLoyaltyOnTx(tx, input, events) {
    const { order } = input;
    if (order.total <= 0 || (order.loyaltyRedeemed <= 0 && order.loyaltyEarned <= 0))
        return;
    ...
    if (clawbackDelta > 0) {
        ...
        events.push({
            type: event_types_1.EventType.LoyaltyRefundClawedBack,
            actor: input.actor,
...
async function lockCustomerLoyalty(tx, customerId) {
