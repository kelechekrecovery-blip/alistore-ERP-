...
exports.normalizeSegmentRules = normalizeSegmentRules;
exports.segmentLabel = segmentLabel;
exports.parseSegmentLabel = parseSegmentLabel;
exports.describeSegment = describeSegment;
exports.buildSegmentAudience = buildSegmentAudience;
const DEFAULT_LIMIT = 100;
const MAX_LIMIT = 500;
function normalizeSegmentRules(input) {
...
function parseSegmentLabel(label) {
    try {
...
function describeSegment(rules) {
    const normalized = normalizeSegmentRules(rules);
    const parts = [
...
function buildSegmentAudience(customers, input) {
...
function matchesRules(customer, rules) {
    if (rules.level && !hasSegment(customer, rules.level))
        return false;
    if (rules.city && !hasCity(customer, rules.city))
        return false;
    for (const tag of rules.tags ?? []) {
        if (!hasSegment(customer, tag))
            return false;
    }
    if (rules.minSpent !== undefined && customer.spent < rules.minSpent)
        return false;
    if (rules.maxSpent !== undefined && customer.spent > rules.maxSpent)
        return false;
    if (rules.minLtv !== undefined && customer.ltv < rules.minLtv)
        return false;
    if (rules.maxLtv !== undefined && customer.ltv > rules.maxLtv)
        return false;
    return true;
}
function hasSegment(customer, value) {
    const wanted = normalize(value);
    return customer.segments.some((segment) => normalize(segment) === wanted);
}
function hasCity(customer, city) {
