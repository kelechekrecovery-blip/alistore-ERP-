...
let ReturnsService = class ReturnsService {
    ...
    async request(orderId, reason, requester, expectedCustomerId, idempotencyKey, selections) {
        ...
        if (key) {
            ...
            if (existing)
                return replayReturn(existing, orderId, reason, selections);
        }
        return this.audit.transaction(async (tx) => {
            if (key) {
                ...
                if (replay)
                    return { result: replayReturn(replay, orderId, reason, selections), events: [] };
            }
            ...
            if (expectedCustomerId && order.customerId !== expectedCustomerId) {
                throw new errors_1.ForbiddenError('return_order_forbidden', 'Нельзя оформить возврат по чужому заказу');
            }
            ...
            return {
                ...
                events: [
                    {
                        type: event_types_1.EventType.ReturnRequested,
                        actor,
    ...
    async transition(id, status, actor, location) {
        return this.audit.transaction(async (tx) => {
            await tx.$queryRaw `SELECT id FROM "Return" WHERE id = ${id} FOR UPDATE`;
            const ret = await tx.return.findUnique({ where: { id } });
            if (!ret) {
                throw new errors_1.ValidationError('return_not_found', `Возврат ${id} не найден`);
            }
            if (ret.status === status) {
                return { result: ret, events: [] };
            }
            if (status === 'paid') {
                throw new errors_1.ConflictError('return_paid_by_refund_only', 'Статус paid устанавливает только выполненный refund');
            }
            if (!RETURN_TRANSITIONS[ret.status].has(status)) {
                throw new errors_1.ConflictError('invalid_return_transition', `Переход возврата ${ret.status} → ${status} запрещён`);
            }
    ...
    async reconcileOnTx(tx, ret, actor, requestedLocation) {
        const location = requestedLocation?.trim();
        if (!location) {
            throw new errors_1.ValidationError('return_location_required', 'Укажите склад возврата');
        }
        ...
        if (!order)
            throw new errors_1.ValidationError('order_not_found', `Заказ ${ret.orderId} не найден`);
        ...
        if (storedReturnItems.length === 0 && ret.isFullOrder) {
            for (const item of order.items)
                selectedQty.set(item.id, item.qty);
        }
        else {
            for (const item of storedReturnItems)
                selectedQty.set(item.orderItemId, item.qty);
        }
        const events = [];
        const quantityByProduct = new Map();
        const quantityMovementByProduct = new Map();
        const returnedQuantityAllocations = [];
        for (const item of order.items) {
            ...
            for (const allocation of allocations) {
                ...
                if (changed.count !== 1)
                    throw new errors_1.ConflictError('return_quantity_conflict', 'Остаток возврата изменён параллельно');
            ...
            const supplyAllocations = (item.orderLineSupply?.quantityAllocations ?? [])
                .sort((a, b) => a.createdAt.getTime() - b.createdAt.getTime());
            if (allocations.length === 0 && supplyAllocations.length === 0)
                continue;
            for (const allocation of supplyAllocations) {
                ...
                if (changed.count !== 1) {
                    throw new errors_1.ConflictError('return_quantity_conflict', 'Остаток поставочного возврата изменён параллельно');
                }
                quantityByProduct.set(allocation.productId, (quantityByProduct.get(allocation.productId) ?? 0) + qty);
                returnedQuantityAllocations.push({ id: allocation.id, productId: allocation.productId, qty });
                remaining -= qty;
            ...
            if (remaining > 0) {
                throw new errors_1.ConflictError('return_quantity_state_mismatch', `Недостаточно списанного количества для позиции ${item.sku}`);
            }
        }
        ...
        for (const returned of returnedQuantityAllocations) {
            ...
            const valuation = await (0, inventory_valuation_1.reverseQuantityCostOnTx)(tx, {
                orderId: order.id,
        ...
        for (const returned of returnedQuantityAllocations) {
            ...
            for (const allocation of quantityConsignments) {
                ...
                const saleAmount = proportionalDelta(allocation.salePrice ?? 0, allocation.qty, allocation.returnedQty, nextReturnedQty);
                const commissionAmount = proportionalDelta(allocation.commissionAmount ?? 0, allocation.qty, allocation.returnedQty, nextReturnedQty);
                const ownerAmount = proportionalDelta(allocation.ownerAmount ?? 0, allocation.qty, allocation.returnedQty, nextReturnedQty);
                ...
                if (allocation.payout?.status === 'paid') {
                    ...
                    const accountingEntry = await (0, consignment_accounting_1.postConsignmentReturnAccountingOnTx)(tx, {
                        returnId: ret.id,
                    ...
                    events.push({
                        type: event_types_1.EventType.ConsignmentAdjustmentCreated,
                        actor,
                        payload: {
                            returnId: ret.id,
                ...
                events.push({
                    type: event_types_1.EventType.ConsignmentReturned,
                    actor,
                    payload: {
                        returnId: ret.id,
                        quantityConsignmentAllocationId: allocation.id,
        ...
        for (const item of order.items) {
            ...
            for (const list of perComponent.values()) {
                ...
                if (!Number.isInteger(perBundle) || selected.length !== qty * perBundle) {
                    throw new errors_1.ConflictError('return_bundle_state_mismatch', `Недостаточно проданных компонентов набора ${item.sku}`);
                }
        ...
        if (regularImeis.length > 0) {
            ...
            if (restored.count !== regularImeis.length) {
                throw new errors_1.ConflictError('return_unit_state_mismatch', 'Не все серийные товары заказа находятся в статусе sold');
            }
            for (const unit of regularUnits) {
                ...
                if (issue) {
                    const reversed = await (0, inventory_valuation_1.reverseInventoryCostOnTx)(tx, {
                        issueId: issue.id,
                        quantity: 1,
                        returnId: ret.id,
                        location,
        ...
        for (const item of consignments) {
            if (item.status !== 'sold' && item.status !== 'settled') {
                throw new errors_1.ConflictError('return_consignment_state_mismatch', `Комиссионная позиция ${item.id} уже ${item.status}`);
            }
...
function proportionalDelta(total, qty, beforeQty, afterQty) {
    if (qty <= 0)
        return 0;
    return Math.floor((total * afterQty) / qty) - Math.floor((total * beforeQty) / qty);
}
function buildReturnQuote(order, previous, selections) {
    if (order.items.length === 0) {
        throw new errors_1.ValidationError('return_order_empty', 'В заказе нет позиций для возврата');
    }
    ...
    for (const ret of previous) {
        if (ret.items.length === 0 && ret.isFullOrder) {
            for (const item of orderedItems)
                returnedByItem.set(item.id, item.qty);
            continue;
        }
    ...
    if (requested.length === 0) {
        throw new errors_1.ConflictError('return_nothing_remaining', 'Все позиции заказа уже оформлены на возврат');
    }
    const requestedByItem = new Map();
    for (const selection of requested) {
        const item = byId.get(selection.orderItemId);
        if (!item) {
            throw new errors_1.ValidationError('return_item_not_found', `Позиция ${selection.orderItemId} не относится к заказу`);
        }
        const qty = Number(selection.qty);
        if (!Number.isInteger(qty) || qty <= 0) {
            throw new errors_1.ValidationError('return_item_qty_invalid', 'Количество возврата должно быть положительным целым числом');
        }
    ...
    for (const [itemId, qty] of requestedByItem) {
        ...
        if (qty > remaining) {
            throw new errors_1.ConflictError('return_item_qty_exceeded', `Для позиции ${item.sku} доступно к возврату: ${remaining}`);
        }
    }
    ...
    const cumulativeComplete = orderedItems.every((item) => (returnedByItem.get(item.id) ?? 0) + (requestedByItem.get(item.id) ?? 0) === item.qty);
...
function replayReturn(ret, orderId, reason, selections) {
    if (ret.orderId !== orderId || ret.reason !== reason) {
        throw new errors_1.ConflictError('idempotency_key_reused', 'Idempotency key уже использован с другим возвратом');
    }
    if (selections?.length) {
        ...
        if (existing !== requested) {
            throw new errors_1.ConflictError('idempotency_key_reused', 'Idempotency key уже использован с другим составом возврата');
        }
    }
    return ret;
}
