...
export function ConsignmentOps({ accessToken, role }: { accessToken: string; role: string }) {
  ...
  async function submitReceive() {
    if (!form.productId || !form.imei.trim() || !form.ownerName.trim()) return;
    setBusy('receive');
    try {
      await receiveConsignment({
        ...
        commissionBps: Math.round(Number(form.commissionPct) * 100),
        grade: form.grade,
      }, accessToken);
      setForm((current) => ({ ...current, imei: '', ownerName: '', ownerContact: '' }));
      setMessage('Комиссионный товар принят');
      await load();
    } catch (error) {
    ...
    <section className="mb-4 border-y border-[#2E2822] bg-[#16130F] py-5" aria-label="Комиссионный склад">
      ...
      <div className="grid gap-4 px-4 xl:grid-cols-[340px_1fr]">
        <div className="space-y-2 border-r border-[#2E2822] pr-4 max-xl:border-r-0 max-xl:pr-0">
          ...
          <div className="grid grid-cols-2 gap-2"><input aria-label="IMEI комиссионного товара" value={form.imei} onChange={(event) => setForm({ ...form, imei: event.target.value })} placeholder="IMEI / SN" className={inputClass} /><input aria-label="Склад комиссионного товара" value={form.location} onChange={(event) => setForm({ ...form, location: event.target.value })} placeholder="Склад" className={inputClass} /></div>
