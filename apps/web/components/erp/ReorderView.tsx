            ...
            <div key={r.sku} className="border-b border-surface-2 px-5 py-3 last:border-0">
              <div className="grid grid-cols-[1fr_auto_auto_auto] items-center gap-4">
                ...
                <div className="text-right font-mono text-[12px] text-muted">
                  <span className={r.inStock === 0 ? 'text-danger-soft' : 'text-white'}>{r.inStock}</span> · {r.reserved} ·{' '}
                ...
                <div className="flex items-center justify-end gap-1.5">
                  {r.needsReorder ? (
                    ...
                    <span className="text-[11px] text-faint">достаточно</span>
                  )}
                </div>
              </div>
              {r.needsReorder && <div className="mt-1.5 text-[12px] text-subtle">{r.reason}</div>}
            </div>
          );
        })}
      </div>
