...
import { CheckCircle2, Clock3, RefreshCw, Stethoscope, UserRound, Wrench } from 'lucide-react';
...
export function ServiceCenterView({ accessToken, staffId }: { accessToken: string; staffId: string }) {
  ...
  return (
    <section data-testid="service-center-view" className="space-y-4">
              ...
              <tbody className="divide-y divide-[#2E2822] bg-[#16130F]">
                ...
                {items?.map((item) => {
                  ...
                  return <tr key={item.id} data-testid={`service-case-${item.id}`}>
                    <td className="px-4 py-3"><div className="font-semibold text-white">{item.productName}</div><div className="mt-1 font-mono text-[10px] text-[#8A7F76]">#{item.id.slice(-7)} · {item.imei}</div><div className="mt-1 max-w-[280px] text-[#A79C92]">{item.problem}</div></td>
      ...
      {tab === 'paid' && <EmptyModule title="Платный ремонт" body="Нет активных ремонтов внешних устройств." />}
