...
/*
 * The fixture block that used to live here — invented zones, couriers «Данияр»
 * and «Азамат», pending orders with real-looking phone numbers, run R-08 with
 * 127 500 сом of COD and four pickup points — was substituted on ANY request
 * failure, silently. A dispatcher could be looking at fiction and dispatching
 * against it. Failures now surface as an error with a retry.
 */
export function LogisticsView({ accessToken }: { accessToken: string }) {
  ...
  const reload = useCallback(async () => {
    ...
    try {
      ...
      if (sequence !== reloadSequence.current) return;
      setData(null);
      setError(cause instanceof Error ? cause.message : 'Не удалось загрузить логистику');
    }
  }, [accessToken, date]);
  ...
  return <div data-testid="logistics-view" className="space-y-4">
    ...
    {/* Load failure is now visible and recoverable instead of being papered over with fixtures. */}
    {error && <div role="alert" className="flex flex-wrap items-center gap-3 rounded-[6px] border border-danger-soft/40 bg-danger-soft/10 px-3 py-2 text-xs text-danger-soft">
      <span>Данные логистики не загрузились: {error}</span>
      <button type="button" onClick={() => void reload()} className="rounded-[6px] border border-danger-soft/50 px-2.5 py-1 font-semibold">Повторить</button>
    </div>}
    {!error && data === null && <div className="rounded-[6px] border border-surface-3 bg-surface-2 px-3 py-2 text-xs text-muted">Загрузка…</div>}
