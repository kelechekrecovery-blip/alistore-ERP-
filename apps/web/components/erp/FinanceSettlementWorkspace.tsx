    ...
    <section aria-labelledby="settlements-title" className="border-b border-surface-3 pb-5">
      <div className="mb-4 flex flex-wrap items-end justify-between gap-3">
        ...
        <div className="flex flex-wrap items-end gap-2">
          ...
          <button type="button" onClick={loadSources} disabled={busy === 'sources'} className="h-9 rounded-[6px] bg-surface-3 px-4 text-xs font-semibold text-lime disabled:opacity-50">{busy === 'sources' ? 'Ищем...' : 'Найти источники'}</button>
        </div>
      </div>
      ...
      {sources.length > 0 && <div className="overflow-hidden rounded-[7px] border border-surface-3 bg-ink-dark">
        ...
        <div className="flex flex-wrap items-center justify-between gap-3 bg-surface px-4 py-3 text-xs">
          ...
          <button type="button" onClick={createRun} disabled={busy === 'create'} className="h-9 rounded-[6px] bg-lime px-4 font-bold text-coal disabled:opacity-50">{busy === 'create' ? 'Создаём...' : 'Создать сверку'}</button>
        </div>
      </div>}
      <div className="mt-4 grid gap-2">
        {runs.slice(0, 12).map((run) => <article key={run.id} data-testid="finance-settlement-run" className="rounded-[7px] border border-surface-3 bg-ink-dark p-4">
          <div className="flex flex-wrap items-start justify-between gap-3">
            <div><div className="flex items-center gap-2"><strong className="text-sm">Сверка {run.id.slice(-7)}</strong><span className={`rounded px-2 py-0.5 text-[10px] ${run.status === 'disputed' ? 'bg-[#472521] text-coral-tint' : run.status === 'closed' ? 'bg-[#233426] text-success-soft' : 'bg-surface-3 text-[#E6E183]'}`}>{STATUS_LABEL[run.status]}</span></div><div className="mt-1 text-[11px] text-subtle">{new Date(run.periodStart).toLocaleDateString('ru-RU')} – {new Date(run.periodEnd).toLocaleDateString('ru-RU')}{run.point ? ` · ${run.point}` : ''}</div></div>
          ...
          <div className="mt-3 grid gap-1">
            {run.lines.map((line) => <div key={line.id} className="grid gap-2 border-t border-surface-2 py-2 text-xs md:grid-cols-[minmax(180px,1fr)_110px_110px_minmax(180px,0.8fr)] md:items-center">
              ...
              {line.status === 'disputed' ? <div className="flex gap-2"><input aria-label={`Основание компенсации ${line.label}`} value={reasons[line.id] ?? ''} onChange={(event) => setReasons((current) => ({ ...current, [line.id]: event.target.value }))} placeholder={`Компенсация ${som(line.expectedAmount - line.actualAmount)}`} className="h-8 min-w-0 flex-1 rounded-[5px] border border-line bg-surface px-2 text-xs" /><button type="button" onClick={() => resolveLine(run.id, line.id, line.expectedAmount, line.actualAmount)} disabled={busy === line.id} className="rounded-[5px] bg-surface-3 px-3 text-lime disabled:opacity-50">Создать корректировку</button></div> : <span className="text-subtle">{line.adjustmentAmount ? `Корр. ${som(line.adjustmentAmount)}` : line.status === 'reconciled' ? 'Проведено' : 'Готово'}</span>}
            </div>)}
          </div>
          {run.status === 'balanced' && <div className="mt-3 flex justify-end"><button type="button" onClick={() => closeRun(run.id)} disabled={busy === run.id} className="h-9 rounded-[6px] bg-lime px-4 text-xs font-bold text-coal disabled:opacity-50">Провести и закрыть</button></div>}
        </article>)}
