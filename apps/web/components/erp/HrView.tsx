    ...
    <div data-testid="hr-view" className="space-y-4">
      <div className="flex flex-wrap items-end justify-between gap-3 border-b border-surface-3 pb-4">
        <div className="flex min-w-0 gap-1 overflow-x-auto" role="tablist">
          {TABS.map((item) => <button key={item.id} type="button" role="tab" aria-selected={tab === item.id} onClick={() => setTab(item.id)} className={`h-9 whitespace-nowrap border-b-2 px-3 text-xs font-semibold ${tab === item.id ? 'border-[#FF6B55] text-white' : 'border-transparent text-subtle'}`}>{item.label}</button>)}
        </div>
      ...
      {tab === 'schedule' && <>
        <div className="overflow-x-auto rounded-[7px] border border-surface-3">
          ...
          {(data?.staff ?? []).map((person) => <div key={person.id} data-testid={`hr-row-${person.id}`} className="grid min-w-[780px] grid-cols-[150px_repeat(7,minmax(86px,1fr))] border-t border-surface-3 bg-ink-dark text-xs">
            ...
            {DAYS.map((_, index) => { const date = dayIso(weekStart, index); const row = data?.schedules.find((item) => item.staffId === person.id && item.shiftDate.slice(0, 10) === date); const absence = data?.absences.find((item) => item.staffId === person.id && item.status === 'approved' && item.startsOn.slice(0, 10) <= date && item.endsOn.slice(0, 10) >= date); return <div key={date} className="grid min-h-16 place-items-center border-l border-surface-3 p-1.5">{absence ? <span className="rounded-[5px] bg-[#593127] px-2 py-1 text-center text-[10px] text-[#FFB5AA]">{ABSENCE_LABEL[absence.type]}</span> : row ? <div className={`w-full rounded-[5px] px-1.5 py-1 text-center font-mono text-[10px] ${row.cancelledAt ? 'bg-[#302A25] text-subtle' : 'bg-[#26351B] text-lime'}`}><span className="block">{row.cancelledAt ? 'отменена' : `${time(row.startsAt)}–${time(row.endsAt)}`}</span>{row.attendance ? <small className="mt-0.5 block text-success-soft">{row.attendance.checkedOutAt ? 'закрыта' : 'на смене'}</small> : <span className="mt-1 flex justify-center gap-2 font-sans"><button type="button" aria-label={`Изменить смену ${person.username} ${date}`} onClick={() => editSchedule(row)} className="text-bright">изменить</button>{!row.cancelledAt && <button type="button" aria-label={`Отменить смену ${person.username} ${date}`} disabled={busy === row.id} onClick={() => cancelSchedule(row)} className="text-danger-soft">отменить</button>}</span>}</div> : <span className="text-[#5F5750]">выходной</span>}</div>; })}
          </div>)}
      ...
      {tab === 'profile' && <div className="grid gap-4 lg:grid-cols-[280px_1fr]">
        <section className="rounded-[7px] border border-surface-3 bg-surface p-5"><div className="mb-4 grid h-14 w-14 place-items-center rounded-full bg-[#2A241F] text-xl font-bold">{chosen?.username.slice(0, 1).toUpperCase() || '—'}</div><h3 className="font-display text-lg font-bold">{chosen?.username || 'Сотрудник'}</h3><p className="mt-1 text-xs text-subtle">{chosen?.role || '—'} · {chosen?.active ? 'активен' : 'отключён'}</p><select aria-label="Профиль сотрудника" value={selectedStaff} onChange={(event) => setSelectedStaff(event.target.value)} className="mt-5 h-10 w-full rounded-[6px] border border-line bg-ink-dark px-3 text-sm">{data?.staff.map((person) => <option key={person.id} value={person.id}>{person.username}</option>)}</select></section>
