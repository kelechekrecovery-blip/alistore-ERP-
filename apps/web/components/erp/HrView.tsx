...
/*
 * The fixture block that used to live here — HR_STAFF (Азизбек, Сайкал, Сыдык,
 * Риезидин), a week of invented shifts, an open drawer holding 218 000 сом and
 * DEFAULT_PAYROLL with a 273 200 сом payout — was substituted whenever a request
 * failed. Two of those catches showed a banner while still rendering the numbers
 * underneath it, which is worse than a silent failure: the banner reads as a
 * warning about something else while an owner reads a payroll run for people who
 * may not exist. Failures now render as errors with a retry, and nothing else.
 */
function mondayIso() {
  const now = new Date();
...
export function HrView({ accessToken }: { accessToken: string }) {
  ...
  // Three independent loads (week / handover / payroll) need three independent
  // error states: one shared banner made a single failure look like a whole-page
  // outage while the other two panels quietly kept rendering.
  ...
  const reload = useCallback(() => {
    ...
    }).catch((error) => { setData(null); setWeekError(error instanceof Error ? error.message : 'Не удалось загрузить график и табель'); });
  ...
  const reloadHandovers = useCallback(() => {
    ...
    }).catch((error) => { setHandovers(null); setHandoverError(error instanceof Error ? error.message : 'Не удалось загрузить передачу кассы'); });
  ...
  const reloadPayroll = useCallback(() => {
    ...
    Promise.all([fetchHrPayrollPreview(payrollPeriod, point, accessToken), fetchHrPayrollRuns(payrollPeriod, point, accessToken)])
      ...
      .catch((error) => { setPayroll(null); setPayrollRuns([]); setPayrollError(error instanceof Error ? error.message : 'Не удалось загрузить начисления'); });
  ...
  return (
    <div data-testid="hr-view" className="space-y-4">
      <header className="flex flex-col gap-3 border-b border-surface-3 pb-4 lg:flex-row lg:items-end lg:justify-between">
        ...
        <div className="text-[11px] text-subtle"><span className="mr-1 inline-block h-2 w-2 rounded-full bg-lime" /> {data ? `${data.staff.length} сотрудников в контуре` : '— сотрудников в контуре'}</div>
      </header>
      ...
      {([['график и табель', weekError, () => reload()], ['передачу кассы', handoverError, () => reloadHandovers()], ['начисления', payrollError, () => reloadPayroll()]] as const)
        .filter(([, text]) => text)
        .map(([what, text, retry]) => (
          <div key={what} role="alert" className="flex flex-wrap items-center gap-3 rounded-[6px] border border-danger-soft/40 bg-danger-soft/10 px-3 py-2 text-xs text-danger-soft">
            <span>Не удалось загрузить {what}: {text}</span>
            <button type="button" onClick={() => void retry()} className="rounded-[6px] border border-danger-soft/50 px-2.5 py-1 font-semibold">Повторить</button>
          </div>
        ))}
      ...
      {tab === 'payroll' && <div className="space-y-4" data-testid="hr-payroll">
        <div className="flex flex-wrap items-end justify-between gap-3 border-b border-surface-3 pb-4">
          ...
          <div className="text-right"><div className="text-[10px] uppercase text-subtle">К выплате</div><strong className="font-mono text-xl text-lime">{payroll ? som(payroll.totals.payout) : '—'}</strong></div>
        </div>
