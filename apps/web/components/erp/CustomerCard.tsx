    ...
    <div className="rounded-[16px] border border-surface-3 bg-surface p-5">
      <div className="flex items-start gap-3">
        ...
        <div className="min-w-0">
          ...
          <div className="font-mono text-xs text-subtle">{c.phone}</div>
        </div>
        <button
          type="button"
          ...
          className={`ml-auto rounded-chip px-3 py-1.5 text-[11px] font-bold transition ${
            c.consent ? 'bg-[#1E3A1E] text-lime' : 'bg-surface-2 text-subtle hover:text-white'
          }`}
...
function Stat({ label, value, color = '#fff' }: { label: string; value: string; color?: string }) {
  return (
    <div className="rounded-[12px] border border-surface-3 bg-surface-2 p-3">
...
function Section({ title, count, children }: { title: string; count: number; children: React.ReactNode }) {
  return (
    <div className="mt-4">
      <div className="mb-1.5 flex items-center gap-2 text-[11px] font-semibold uppercase tracking-wide text-subtle">
        {title}
        {count > 0 && <span className="rounded-chip bg-surface-2 px-1.5 text-[10px] text-bright">{count}</span>}
      </div>
...
function Row({ left, right, dot }: { left: React.ReactNode; right: string; dot?: string }) {
  return (
    <div className="flex items-center gap-2 border-b border-surface-2 py-1.5 text-[13px] text-bright">
      ...
      <span className="font-mono text-xs text-subtle">{right}</span>
    </div>
  );
}

function Empty() {
  return <div className="py-1.5 text-xs text-faint">—</div>;
}
