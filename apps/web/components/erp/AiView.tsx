...
export function AiView({ insights }: { insights: Insight[] | null }) {
  if (insights === null) return <p className="font-mono text-sm text-[#6E645C]">Ассистент думает…</p>;
  return (
    <div className="max-w-3xl">
      <div className="mb-4 flex items-center gap-2.5 rounded-[14px] border border-[#2E2822] bg-[#1A1611] px-4 py-3">
        ...
        <div>
          ...
          <div className="text-[11px] text-[#8A7F76]">Инсайты считаются из Event Ledger · подключите ключ LLM для развёрнутого разбора</div>
        </div>
      </div>
      {insights.length === 0 && <p className="text-sm text-[#8A7F76]">Пока недостаточно данных для инсайтов.</p>}
      <div className="flex flex-col gap-2.5">
        {insights.map((i, idx) => (
          <div key={idx} className="flex gap-3 rounded-[14px] border border-[#2E2822] bg-[#1A1611] p-4">
            ...
            <div>
              ...
              <div className="mt-0.5 text-[13px] leading-snug text-[#A79C92]">{i.detail}</div>
            </div>
          </div>
        ))}
      </div>
