        ...
        <div className="mt-3.5">
          <Card>
            <div className="mb-3.5 flex items-center">
              ...
              <span className="ml-auto text-xs text-[#8A7F76]">
                база {som(payroll.base)} + {payroll.commissionPct}% с оборота · фонд {som(payroll.totalPayout)}
              </span>
            </div>
            <div className="grid grid-cols-[1fr_auto_auto_auto] gap-3 border-b border-[#2E2822] pb-2 text-[11px] uppercase tracking-wide text-[#8A7F76]">
              <span>Продавец</span>
            ...
            {payroll.rows.map((r) => (
              <div key={r.staffId} className="grid grid-cols-[1fr_auto_auto_auto] items-center gap-3 border-b border-[#221E19] py-2.5 text-[13px] last:border-0">
                <span className="min-w-0 truncate text-[#D8CFC6]">{r.staffId}</span>
                <span className="text-right font-mono tabular text-[#8A7F76]">{som(r.base)}</span>
