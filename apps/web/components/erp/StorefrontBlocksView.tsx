...
import { ArrowDown, ArrowUp, Archive, CalendarClock, Image as ImageIcon, Pencil, Plus, Send, X } from 'lucide-react';
...
const EMPTY = {
  ...
  title: '', eyebrow: '', body: '', ctaLabel: '', ctaHref: '/catalog', imageUrl: '', tone: 'dark' as StorefrontBlock['tone'],
};
...
export function StorefrontBlocksView({ accessToken }: { accessToken: string }) {
  ...
  function fields() {
    return {
      type: form.type, device: form.device, title: form.title, tone: form.tone,
  ...
  function reset() { setForm(EMPTY); setSelected([]); setStartsAt(''); setEndsAt(''); setEditingId(null); }

  function startEdit(target: StorefrontBlock) {
    setEditingId(target.id);
    setForm({
      type: target.type, device: target.device, title: target.title,
      eyebrow: target.eyebrow ?? '', body: target.body ?? '', ctaLabel: target.ctaLabel ?? '',
      ctaHref: target.ctaHref ?? '', imageUrl: target.imageUrl ?? '', tone: target.tone,
    });
    setSelected(target.type === 'collection' ? target.productIds : []);
    setStartsAt(''); setEndsAt(''); setNotice('');
  }

  async function submit() {
    if (editingId) {
      await run(async () => { await updateStorefrontBlock(editingId, fields(), accessToken); reset(); }, 'Черновик обновлён');
      return;
    }
    await run(async () => {
      const block = await createStorefrontBlock(fields(), accessToken);
  ...
  return <div className="grid gap-4 xl:grid-cols-[.85fr_1.15fr]">
    <section className="rounded-[8px] border border-surface-3 bg-surface p-5">
      ...
      {form.type === 'collection' && <div className="mt-4"><div className="text-xs text-muted">Товары подборки · {selected.length}/12</div><div className="mt-2 max-h-56 overflow-y-auto rounded-[8px] border border-surface-3">{products.map((product) => <button key={product.id} type="button" onClick={() => setSelected((ids) => ids.includes(product.id) ? ids.filter((id) => id !== product.id) : ids.length < 12 ? [...ids, product.id] : ids)} className="flex w-full items-center gap-2 border-b border-surface-3 px-3 py-2 text-left text-xs last:border-0"><span className={selected.includes(product.id) ? 'text-lime' : 'text-faint'}>{selected.includes(product.id) ? '✓' : '+'}</span><span className="truncate">{product.name}</span><span className="ml-auto text-subtle">{product.availableUnits} шт.</span></button>)}</div></div>}
      ...
      <div className="mt-4 flex items-center gap-2">
        <button type="button" disabled={busy || !form.title.trim() || (form.type === 'collection' && selected.length === 0)} onClick={submit} className="inline-flex items-center gap-2 rounded-[8px] bg-lime px-4 py-2.5 text-sm font-bold text-lime-ink disabled:opacity-40"><Send size={15} />{editingId ? 'Сохранить черновик' : startsAt ? 'Создать и запланировать' : 'Создать черновик'}</button>
        {editingId && <button type="button" disabled={busy} onClick={reset} className="inline-flex items-center gap-2 rounded-[8px] border border-line px-3 py-2.5 text-sm text-muted disabled:opacity-40"><X size={15} />Отмена</button>}
      </div>
    ...
    <section className="rounded-[8px] border border-surface-3 bg-surface p-5">
      ...
      <p className="mt-1 text-xs text-subtle">Опубликованные блоки меняют главную страницу без выпуска новой версии сайта.</p>
      <div className="mt-4 grid gap-2">
        {blocks.map((block) => {
          ...
          return <article key={block.id} className={`flex items-center gap-3 rounded-[8px] border p-3 ${block.status === 'archived' ? 'border-surface-2 opacity-55' : 'border-surface-3 bg-surface-2'}`}>
            ...
            <div className="flex shrink-0 flex-wrap justify-end gap-1">{block.status === 'draft' && <button type="button" disabled={busy} onClick={() => startEdit(block)} title="Изменить" aria-label="Изменить" className="rounded-[6px] border border-line px-2 py-1.5 text-[11px] text-muted"><Pencil size={13} /></button>}{block.status === 'draft' && <button disabled={busy} onClick={() => run(() => publishStorefrontBlock(block.id, accessToken), 'Блок опубликован')} className="rounded-[6px] bg-coral px-2.5 py-1.5 text-[11px] font-bold text-white">Включить</button>}{block.status === 'scheduled' && <button disabled={busy} onClick={() => run(() => cancelStorefrontBlockSchedule(block.id, accessToken), 'Расписание отменено')} className="rounded-[6px] border border-warn px-2 py-1.5 text-[11px] text-warn"><CalendarClock size={13} /></button>}{block.status === 'published' && <button disabled={busy} onClick={() => run(() => archiveStorefrontBlock(block.id, accessToken), 'Блок выключен и помещён в архив')} className="rounded-[6px] border border-line px-2 py-1.5 text-[11px] text-muted"><Archive size={13} /></button>}</div>
          </article>;
        })}
        {blocks.length === 0 && <div className="rounded-[8px] border border-dashed border-line py-10 text-center text-sm text-subtle">Добавьте первый баннер или подборку</div>}
      </div>
    </section>
  </div>;
}
