...
export function SiteHeader() {
  ...
  return (
    <header className="sticky top-0 z-50 bg-white text-coal shadow-[0_1px_0_#e5e5e7]">
      ...
      <div className="mx-auto grid h-[76px] max-w-[1400px] grid-cols-[auto_1fr_auto] items-center gap-3 px-4 md:h-[88px] md:grid-cols-[auto_auto_1fr_auto] md:gap-6 md:px-5">
        ...
        <form action="/catalog" className="relative hidden h-11 min-w-0 md:block">
          ...
          <input id="header-search" name="q" placeholder="Поиск по товарам" className="h-full w-full rounded-[10px] border border-mist bg-paper px-[18px] pr-14 text-sm outline-none transition focus:border-coal focus:bg-white" />
          <button type="submit" aria-label="Найти" className="absolute bottom-1 right-1 top-1 grid w-10 place-items-center rounded-lg bg-coal text-white"><Search size={17} /></button>
        </form>
        ...
        <div className="ml-auto flex items-center gap-0.5 md:gap-1">
          ...
          <button type="button" onClick={() => setOpen((value) => !value)} className="grid h-11 w-11 place-items-center rounded-lg text-steel md:hidden" aria-label={open ? 'Закрыть меню' : 'Открыть меню'}>
            {open ? <X size={22} /> : <Menu size={22} />}
          </button>
        </div>
      </div>

      <nav className="hidden border-t border-mist md:block" aria-label="Категории товаров">
        <div className="mx-auto flex max-w-[1400px] gap-6 overflow-x-auto px-5">
          {CATEGORY_NAV.map(([label, href], index) => (
            <Link key={label} href={href} className={`relative whitespace-nowrap py-3.5 text-[13px] font-medium text-steel hover:text-coal ${index === 0 && pathname === '/' ? 'font-bold text-coal after:absolute after:inset-x-0 after:bottom-0 after:h-0.5 after:bg-coral' : ''}`}>{label}</Link>
          ))}
          <Link href="/catalog?promo=true" className="whitespace-nowrap py-3.5 text-[13px] font-semibold text-coral">Акции</Link>
        </div>
      </nav>

      {open && (
        <nav className="border-t border-mist bg-white px-4 py-3 md:hidden" aria-label="Мобильная навигация">
          {MOBILE_NAV.map((item) => <Link key={item.href} href={item.href} onClick={() => setOpen(false)} className="block border-b border-mist px-2 py-3 text-sm font-semibold last:border-0">{item.label}</Link>)}
        </nav>
      )}
      <ScrollProgress />
    </header>
...
function HeaderTool({ href, label, icon, count = 0, hideLabel = false }: { href: string; label: string; icon: ReactNode; count?: number; hideLabel?: boolean }) {
  return (
    <Link href={href} aria-label={label} className="relative flex min-w-11 flex-col items-center gap-1 rounded-lg px-2 py-2 text-steel transition-colors hover:bg-paper hover:text-coal lg:min-w-[66px]">
      {icon}
