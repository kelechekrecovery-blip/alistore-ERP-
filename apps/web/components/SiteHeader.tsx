...
export function SiteHeader() {
  ...
  return (
    <header className="sticky top-0 z-50 bg-white text-ink shadow-[0_1px_0_#e5e5e7]">
      ...
      <nav className="hidden border-t border-linen md:block" aria-label="Категории товаров">
        <div className="mx-auto flex max-w-[1400px] gap-6 overflow-x-auto px-5">
          {CATEGORY_NAV.map(([label, href], index) => (
            <Link key={label} href={href} className={`relative whitespace-nowrap py-3.5 text-[13px] font-medium text-faint hover:text-ink ${index === 0 && pathname === '/' ? 'font-bold text-ink after:absolute after:inset-x-0 after:bottom-0 after:h-0.5 after:bg-coral' : ''}`}>{label}</Link>
          ))}
