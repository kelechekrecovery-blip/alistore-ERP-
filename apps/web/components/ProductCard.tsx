...
export function ProductCard({ product }: { product: CatalogProduct }) {
  ...
  return (
    <article className="store-card-enter group relative flex min-w-0 flex-col rounded-[10px] border border-linen bg-white p-3 transition duration-200 hover:-translate-y-0.5 hover:border-linen hover:shadow-[0_8px_24px_rgba(0,0,0,.07)] active:scale-[0.99]">
      <div className="relative aspect-square overflow-hidden rounded-[7px] bg-white">
        ...
        <span className="absolute left-1 top-1 rounded-[4px] bg-tint px-2 py-1 text-[10px] font-bold text-deep">{condition}</span>
        <button type="button" onClick={() => toggle(product.id)} aria-label={has(product.id) ? 'Удалить из избранного' : 'Добавить в избранное'} className={`absolute right-0 top-0 grid h-9 w-9 place-items-center rounded-[8px] bg-white/90 ${has(product.id) ? 'text-coral' : 'text-faint hover:text-ink'}`}>
          <Heart size={17} fill={has(product.id) ? 'currentColor' : 'none'} />
        </button>
      </div>
      <div className="flex flex-1 flex-col pt-2">
        <div className="flex min-h-4 items-center gap-1 text-[11px] text-faint">
        ...
        <div className="mt-auto flex gap-1.5 pt-3">
          ...
          <button type="button" onClick={() => compare.toggle(product.id)} aria-label={compare.has(product.id) ? 'Удалить из сравнения' : 'Добавить к сравнению'} className={`grid h-10 w-10 shrink-0 place-items-center rounded-[8px] bg-sand ${compare.has(product.id) ? 'text-coral' : 'text-faint hover:bg-linen'}`}>
            <Scale size={16} />
          </button>
        </div>
      </div>
