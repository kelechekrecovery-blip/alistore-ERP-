...
export function ProductCard({ product }: { product: CatalogProduct }) {
  ...
  return (
    <article className="store-card-enter group relative flex min-w-0 flex-col rounded-[10px] border border-mist bg-white p-3 transition duration-200 hover:-translate-y-0.5 hover:border-haze hover:shadow-[0_8px_24px_rgba(0,0,0,.07)] active:scale-[0.99]">
      <div className="relative aspect-square overflow-hidden rounded-[7px] bg-white">
        <Link href={href} className="absolute inset-0" aria-label={product.name}>
          {productImage(product) ? <Image src={productImage(product)!} alt={product.name} fill sizes="(max-width: 700px) 50vw, 260px" className="object-contain p-3 transition duration-300 group-hover:scale-[1.04]" /> : <span className="flex h-full flex-col items-center justify-center gap-2 text-xs text-slate"><ImageOff size={28} /><span>Фото готовится</span></span>}
        </Link>
        <span className="absolute left-1 top-1 rounded-[4px] bg-tint px-2 py-1 text-[10px] font-bold text-coral">{condition}</span>
        <button type="button" onClick={() => toggle(product.id)} aria-label={has(product.id) ? 'Удалить из избранного' : 'Добавить в избранное'} className={`absolute right-0 top-0 grid h-9 w-9 place-items-center rounded-[8px] bg-white/90 ${has(product.id) ? 'text-coral' : 'text-slate hover:text-coal'}`}>
          <Heart size={17} fill={has(product.id) ? 'currentColor' : 'none'} />
        </button>
      </div>
      <div className="flex flex-1 flex-col pt-2">
        <div className="flex min-h-4 items-center gap-1 text-[11px] text-slate">
