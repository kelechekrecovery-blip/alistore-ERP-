...
export default function TelegramMiniAppPage() {
  ...
  return (
    <Shell>
      <header className="flex-shrink-0 border-b border-surface-3 px-4 pb-4 pt-5">
        <div className="flex items-start gap-3">
          ...
          <div className="min-w-0">
            ...
            <p className="mt-0.5 text-xs text-subtle">Telegram shell · общий каталог и checkout</p>
          </div>
          <Link href="/" className="ml-auto rounded-chip border border-surface-3 px-3 py-2 text-xs font-semibold text-muted">
            web
          </Link>
        </div>
      </header>
      ...
      {mode === 'catalog' ? (
        <>
            ...
            <div className="mt-3 flex gap-2 overflow-x-auto pb-1">
              <button
                ...
                className={`flex-shrink-0 rounded-[12px] border px-3 py-2 text-xs font-bold ${
                  category === 'all' ? 'border-lime bg-lime text-lime-ink' : 'border-surface-3 bg-surface-2 text-bright'
                }`}
              >
                Все
              </button>
              {categories.map((item) => (
                <button
                  ...
                  className={`flex-shrink-0 rounded-[12px] border px-3 py-2 text-xs font-bold ${
                    category === item ? 'border-lime bg-lime text-lime-ink' : 'border-surface-3 bg-surface-2 text-bright'
                  }`}
                >
                  ...
                  return (
                    <article key={product.id} className="overflow-hidden rounded-[16px] border border-surface-3 bg-surface-2">
                      ...
                      <div className="p-3">
                        ...
                        <div className={`mt-0.5 text-[10px] ${inStock ? 'text-subtle' : 'text-danger-soft'}`}>
                        ...
                        <button
                          ...
                          disabled={!inStock}
                          onClick={() => add(product)}
                          className="mt-2.5 w-full rounded-[10px] bg-lime py-2 text-xs font-bold text-lime-ink disabled:bg-line disabled:text-faint"
                        >
                          Добавить
                        </button>
                      </div>
          ...
          {cart.length > 0 && (
            <div className="absolute bottom-0 left-1/2 w-full max-w-[440px] -translate-x-1/2 border-t border-surface-3 bg-ink-dark/95 p-4 backdrop-blur">
              <button
                type="button"
...
function Shell({ children }: { children: React.ReactNode }) {
  return (
    <div className="fixed inset-0 z-50 flex justify-center bg-night font-sans">
      <div className="relative flex h-full w-full max-w-[440px] flex-col bg-ink-dark text-white">
        {children}
      </div>
    </div>
  );
  ...
  return (
    <section className="min-h-0 flex-1 overflow-y-auto px-4 pb-5 pt-4">
      ...
      <div className="mt-4 space-y-2">
        {cart.map((line) => (
          <div key={line.id} className="flex items-center gap-3 rounded-[14px] border border-surface-3 bg-surface-2 p-3">
            <div className="min-w-0 flex-1">
              <div className="truncate text-sm font-bold">{line.name}</div>
              <div className="mt-1 font-mono text-xs text-subtle">{line.sku}</div>
            </div>
            <div className="flex items-center gap-1">
              <button type="button" onClick={() => onQty(line.id, line.qty - 1)} className="grid h-8 w-8 place-items-center rounded-[9px] bg-ink-dark text-sm font-bold">−</button>
      ...
      <input
        ...
        className="w-full rounded-[13px] border border-surface-3 bg-surface-2 px-4 py-3 text-sm text-white outline-none placeholder:text-faint focus:border-lime"
      />
      <label className="mb-1.5 mt-3 block text-xs font-semibold uppercase tracking-wide text-subtle">Имя</label>
      <input
        value={name}
        ...
        {[
          ...
          <button
            ...
            className={`rounded-[13px] border px-3 py-3 text-sm font-bold ${
              payment === id ? 'border-lime bg-lime text-lime-ink' : 'border-surface-3 bg-surface-2 text-bright'
            }`}
          >
            {label}
          </button>
      ...
      <div className="mt-5 rounded-[16px] border border-surface-3 bg-surface-2 p-4">
