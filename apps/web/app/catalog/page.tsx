      ...
      <div className="grid gap-5 lg:grid-cols-[238px_1fr]">
        ...
        <section>
          <div className="mb-5 flex flex-col gap-3 rounded-[16px] border border-white/10 bg-white/[.04] p-3 shadow-[0_10px_25px_rgba(0,0,0,.25)] sm:flex-row">
            <div className="flex min-w-0 flex-1 items-center rounded-[10px] border border-white/10 bg-white/[.05] px-3 focus-within:border-[#ff7a4d] focus-within:bg-white/[.09]"><input value={query} onChange={(event) => { setQuery(event.target.value); setOffset(0); }} placeholder="Поиск по названию, SKU и категории" className="min-w-0 flex-1 bg-transparent py-3 text-sm text-white outline-none placeholder:text-white/35" />{query && <button type="button" onClick={() => { setQuery(''); setOffset(0); }} aria-label="Очистить поиск" className="text-white/45"><X size={17} /></button>}</div>
