  ...
  return <>
    ...
    <main className="mx-auto max-w-[1400px] px-5 py-8">
      <div className="mb-8 flex flex-col justify-between gap-5 sm:flex-row sm:items-end">
        <div><div className="text-xs text-[#8a8a8a]">Главная / Каталог</div><h1 className="mt-3 text-[34px] font-extrabold">Каталог техники</h1><p className="mt-2 text-[#4a4a4a]">Новое и проверенное Б/У с гарантией AliStore</p></div>
        <div className="text-sm text-[#8a8a8a]">{products === null ? 'Загрузка...' : `${total} товаров`}</div>
      </div>
      ...
      <div className="grid gap-5 lg:grid-cols-[238px_1fr]">
        ...
        <section>
          <div className="mb-5 flex flex-col gap-3 rounded-[12px] border border-[#e5e5e7] bg-white p-3 sm:flex-row">
            <div className="flex min-w-0 flex-1 items-center rounded-[10px] border border-[#e5e5e7] bg-[#f5f5f7] px-3 focus-within:border-[#0f0f0f] focus-within:bg-white"><input value={query} onChange={(event) => { setQuery(event.target.value); setOffset(0); }} placeholder="Поиск по названию, SKU и категории" className="min-w-0 flex-1 bg-transparent py-3 text-sm text-[#0f0f0f] outline-none placeholder:text-[#8a8a8a]" />{query && <button type="button" onClick={() => { setQuery(''); setOffset(0); }} aria-label="Очистить поиск" className="text-[#8a8a8a]"><X size={17} /></button>}</div>
            <select value={sort} onChange={(event) => { setSort(event.target.value as CatalogQuery['sort']); setOffset(0); }} aria-label="Сортировка" className="rounded-[10px] border border-[#e5e5e7] bg-white px-3 py-3 text-sm text-[#0f0f0f] outline-none"><option value="stock_desc">Сначала в наличии</option><option value="price_asc">Сначала дешевле</option><option value="price_desc">Сначала дороже</option><option value="name">По названию</option></select>
          </div>
          {products === null ? <div className="grid grid-cols-2 gap-3 xl:grid-cols-4">{Array.from({ length: 8 }, (_, index) => <div key={index} className="aspect-[.62] animate-pulse rounded-[10px] border border-[#e5e5e7] bg-white" />)}</div> : products.length ? <><div className="grid grid-cols-2 gap-3 xl:grid-cols-4">{products.map((product) => <ProductCard key={product.id} product={product} />)}</div><div className="mt-6 flex items-center justify-center gap-3"><button type="button" disabled={offset === 0} onClick={() => setOffset(Math.max(0, offset - PAGE_SIZE))} className="rounded-[8px] border bg-white px-4 py-2 text-sm disabled:opacity-40">Назад</button><span className="text-sm text-[#8a8a8a]">{Math.floor(offset / PAGE_SIZE) + 1} / {Math.max(1, Math.ceil(total / PAGE_SIZE))}</span><button type="button" disabled={offset + PAGE_SIZE >= total} onClick={() => setOffset(offset + PAGE_SIZE)} className="rounded-[8px] border bg-white px-4 py-2 text-sm disabled:opacity-40">Дальше</button></div></> : <div className="rounded-[12px] border border-[#e5e5e7] bg-white py-20 text-center text-[#4a4a4a]">По выбранным фильтрам ничего не найдено</div>}
        </section>
      </div>
    </main>
    <SiteFooter />
