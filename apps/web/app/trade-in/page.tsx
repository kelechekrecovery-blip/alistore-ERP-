...
export default function TradeInPage() {
    ...
    return (
      <MobileAppFrame title="Trade-in оценка" subtitle="Заявка создана, договор доступен сотруднику." active="home" backHref="/account">
        <div className="rounded-[18px] border border-surface-3 bg-gradient-to-br from-[#2A2A2E] to-surface-2 p-5 text-center">
          ...
          {done.tradeIn.imei && <div className="mt-2 font-mono text-[12px] text-subtle">IMEI {done.tradeIn.imei}</div>}
          <div className="mt-2 text-[12px] leading-relaxed text-subtle">Паспорт сохранён в защищённом виде: {done.tradeIn.sellerPassportMasked}</div>
        ...
        <button type="button" onClick={() => setDone(null)} className="mt-3 w-full text-center text-[13px] text-muted">Оценить другое</button>
      </MobileAppFrame>
    );
  }
  ...
  return (
    <MobileAppFrame title="Trade-in оценка" subtitle="Оцените старое устройство за 30 секунд и зачтите сумму в новую покупку." active="home" backHref="/account">
      ...
      <input value={model} onChange={(e) => setModel(e.target.value)} className="mt-2 w-full rounded-[12px] border border-surface-3 bg-surface-2 p-3 text-sm outline-none focus:border-lime" />

      <div className="mt-3 text-[13px] text-muted">IMEI / серийный номер</div>
      <input value={imei} onChange={(e) => setImei(e.target.value)} placeholder="Можно оставить пустым до диагностики" className="mt-2 w-full rounded-[12px] border border-surface-3 bg-surface-2 p-3 text-sm outline-none placeholder:text-faint focus:border-lime" />

      <div className="mb-2 mt-4 text-[13px] text-muted">Состояние</div>
      {grades.map((g) => (
        <button key={g.id} type="button" onClick={() => setGrade(g.id)} className={`mb-2 flex w-full items-center gap-2.5 rounded-[11px] border bg-surface-2 p-3 text-left ${grade === g.id ? 'border-lime' : 'border-surface-3'}`}>
          <span className={`h-[18px] w-[18px] rounded-full border-2 ${grade === g.id ? 'border-lime bg-lime' : 'border-line'}`} />
          <span>
            <span className="block text-[13px] font-semibold">{g.label}</span>
            <span className="mt-0.5 block text-[11px] text-subtle">{g.desc}</span>
          </span>
        </button>
      ))}

      <textarea value={note} onChange={(e) => setNote(e.target.value)} placeholder="Память, батарея, комплект, дефекты" className="mt-1 min-h-[76px] w-full rounded-[12px] border border-dashed border-line bg-surface-2 p-3 text-sm outline-none placeholder:text-faint focus:border-lime" />
      ...
      <input value={passport} onChange={(e) => setPassport(e.target.value)} placeholder="Паспорт / ID для договора" className="mt-2 w-full rounded-[12px] border border-surface-3 bg-surface-2 p-3 text-sm outline-none placeholder:text-faint focus:border-lime" />
      {error && <p className="mt-2 text-sm text-danger-soft">{error}</p>}
      <button type="button" disabled={busy || !model.trim() || !passport.trim() || (!user && phone.trim().length < 9)} onClick={submit} className="mt-3 w-full rounded-[13px] bg-lime py-3.5 text-[15px] font-bold text-lime-ink disabled:bg-line disabled:text-faint">{busy ? 'Оформляем…' : 'Зафиксировать оценку'}</button>
    </MobileAppFrame>
  );
}
