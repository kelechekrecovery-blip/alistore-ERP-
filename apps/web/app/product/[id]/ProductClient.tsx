...
export default function ProductPage({ params }: { params: { id: string } }) {
  ...
  if (product === 'missing') return <StoreMessage><div className="text-center"><h1 className="font-display text-2xl font-bold text-ink">Товар не найден</h1><Link href="/catalog" className="mt-4 inline-block text-deep">Вернуться в каталог</Link></div></StoreMessage>;
  ...
  return <>
    ...
    <main className="mx-auto max-w-[1400px] px-5 py-8">
      ...
      <section className="grid gap-8 lg:grid-cols-[1.05fr_.95fr] lg:gap-14">
        <div>
          <div className="relative aspect-square max-h-[610px] overflow-hidden rounded-[22px] border border-[#E7DDD3] bg-gradient-to-br from-white to-[#F2ECE5] shadow-soft">
            ...
            <span className="absolute left-5 top-5 rounded-full border border-coral/25 bg-tint px-3 py-1.5 text-xs font-semibold text-deep">{condition}</span>
          </div>
          <div className="mt-3 grid grid-cols-4 gap-3">{[productImage(product), '/products/p-iphone.png', '/products/p-samsung.png', productImage(product)].map((src, index) => <button key={`${src}-${index}`} type="button" className={`relative aspect-square overflow-hidden rounded-[13px] border bg-white ${index === 0 ? 'border-coral' : 'border-[#DED3C8]'}`} aria-label={`Фото товара ${index + 1}`}><Image src={src} alt="" fill sizes="120px" className="object-contain p-3 opacity-80" /></button>)}</div>
        </div>

        <div className="pt-1">
          <div className="flex items-center gap-2"><span className="rounded-full border border-coral/25 bg-tint px-3 py-1 text-xs text-deep">Хит продаж</span><span className="font-mono text-xs text-[#8A7F76]">{product.sku}</span></div>
          <h1 className="mt-5 font-display text-3xl font-bold leading-tight sm:text-4xl">{product.name}</h1>
          <div className="mt-4 flex flex-wrap items-center gap-3 text-sm text-[#6E645C]"><span className="flex items-center gap-1 text-deep"><Star size={16} fill="currentColor" /> {reviewLabel}</span><span>·</span><span>{product.category}</span></div>
          ...
          {Boolean(product.bundleComponents?.length) && <div className="mt-6 rounded-[12px] border border-[#DED3C8] bg-white p-4"><div className="text-xs font-semibold uppercase text-[#8A7F76]">В комплекте</div><div className="mt-2 grid gap-1.5">{product.bundleComponents?.map((component) => <div key={component.productId} className="flex justify-between gap-4 text-sm"><span>{component.name}</span><span className="font-mono text-[#6E645C]">× {component.qty}</span></div>)}</div></div>}
          ...
          <div className="mt-7 grid grid-cols-[auto_1fr] gap-3">
            ...
            <button type="button" onClick={addToCart} disabled={!inStock} className={`flex items-center justify-center gap-2 rounded-[12px] px-5 font-semibold transition disabled:bg-[#E7DDD3] disabled:text-[#8A7F76] ${added ? 'bg-success text-white' : 'bg-coral text-white hover:bg-deep'}`}><ShoppingBag size={18} />{added ? 'Добавлено' : 'В корзину'}</button>
          </div>
          <div className="mt-3 grid grid-cols-2 gap-3">
            <button type="button" onClick={() => favorites.toggle(product.id)} className={`flex items-center justify-center gap-2 rounded-[12px] border py-3 text-sm ${favorites.has(product.id) ? 'border-coral bg-tint text-deep' : 'border-[#DED3C8] bg-white text-[#6E645C]'}`}><Heart size={17} fill={favorites.has(product.id) ? 'currentColor' : 'none'} />Избранное</button>
            <button type="button" onClick={() => compare.toggle(product.id)} className={`flex items-center justify-center gap-2 rounded-[12px] border py-3 text-sm ${compare.has(product.id) ? 'border-coral bg-tint text-deep' : 'border-[#DED3C8] bg-white text-[#6E645C]'}`}><GitCompareArrows size={17} />Сравнить</button>
          </div>
      ...
      <section className="pt-24">
        <div className="text-xs uppercase tracking-[0.16em] text-deep">Технические детали</div><h2 className="mt-2 font-display text-3xl font-bold">Характеристики</h2>
        <div className="mt-7 overflow-hidden rounded-[18px] border border-[#E7DDD3] bg-white shadow-soft">{specs.length ? specs.map(([key, value]) => <div key={key} className="grid gap-2 border-b border-[#E7DDD3] px-5 py-4 text-sm last:border-0 sm:grid-cols-[220px_1fr]"><span className="text-[#8A7F76]">{key}</span><span className="text-ink">{String(value)}</span></div>) : <div className="px-5 py-10 text-center text-[#8A7F76]">Подробные характеристики уточняются</div>}</div>
      </section>

      <section className="grid gap-8 pt-24 lg:grid-cols-[1fr_420px]">
        <div><div className="text-xs uppercase tracking-[0.16em] text-deep">Опыт покупателей</div><h2 className="mt-2 font-display text-3xl font-bold">Отзывы</h2><div className="mt-7 grid gap-3">{reviews?.items.length ? reviews.items.map((review) => <article key={review.id} className="rounded-[18px] border border-[#E7DDD3] bg-white p-5 shadow-soft"><div className="flex items-center gap-3"><strong>{review.customerName}</strong><span className="ml-auto text-deep">{'★'.repeat(review.rating)}</span></div>{review.text && <p className="mt-3 text-sm leading-6 text-[#6E645C]">{review.text}</p>}</article>) : <div className="rounded-[18px] border border-[#E7DDD3] bg-white p-7 text-[#6E645C]">Будьте первым, кто оставит отзыв об этом товаре.</div>}</div></div>
        <div className="h-fit rounded-[18px] border border-[#E7DDD3] bg-white p-6 shadow-soft"><h3 className="font-display text-lg font-semibold">Оставить отзыв</h3>{hydrated && user ? <form onSubmit={submitReview} className="mt-5 grid gap-3"><select value={reviewForm.rating} onChange={(event) => setReviewForm((form) => ({ ...form, rating: Number(event.target.value) }))} className="rounded-[11px] border border-[#DED3C8] bg-white px-3 py-3 text-sm outline-none">{[5,4,3,2,1].map((rating) => <option key={rating} value={rating}>{rating} из 5</option>)}</select><textarea value={reviewForm.text} onChange={(event) => setReviewForm((form) => ({ ...form, text: event.target.value }))} rows={4} maxLength={500} placeholder="Расскажите о покупке" className="resize-none rounded-[11px] border border-[#DED3C8] bg-white p-3 text-sm outline-none focus:border-coral" /><button className="rounded-[12px] bg-coral py-3 text-sm font-bold text-white">Опубликовать</button>{reviewMsg && <p className="text-xs text-[#6E645C]">{reviewMsg}</p>}</form> : <Link href={`/login?next=/product/${product.id}`} className="mt-5 inline-flex rounded-[12px] bg-coral px-5 py-3 text-sm font-bold text-white">Войти и оставить отзыв</Link>}</div>
      </section>

      {similar.length > 0 && <section className="pt-24"><div className="flex items-end justify-between"><div><div className="text-xs uppercase tracking-[0.16em] text-deep">Вам может подойти</div><h2 className="mt-2 font-display text-3xl font-bold">Похожие товары</h2></div><Link href="/catalog" className="text-sm text-deep">Весь каталог</Link></div><div className="mt-7 grid grid-cols-2 gap-3 lg:grid-cols-4">{similar.slice(0,4).map((item) => <ProductCard key={item.id} product={item} />)}</div></section>}
    </main>
    <SiteFooter />
    </div>
  </>;
