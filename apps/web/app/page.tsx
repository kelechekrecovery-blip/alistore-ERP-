      ...
      <div className="hidden min-h-screen bg-paper text-coal [font-family:Manrope,-apple-system,BlinkMacSystemFont,sans-serif] md:block">
        ...
        <main className="mx-auto max-w-[1400px] px-5 py-5">
          {blocks.length > 0 ? <ManagedDesktopBlocks blocks={blocks} /> : <section className="grid grid-cols-[minmax(0,1fr)_340px] gap-4" aria-label="Предложения AliStore">
            <Link href={storefront?.content.heroCtaHref ?? '/catalog'} className="group relative flex min-h-[320px] overflow-hidden rounded-[14px] bg-graphite p-10 text-white">
              <div className="relative z-10 flex max-w-[560px] flex-col justify-center">
                <span className="mb-5 w-fit rounded-md bg-coral px-2.5 py-1.5 text-[11px] font-bold uppercase">{storefront?.content.heroEyebrow ?? 'AliStore'}</span>
                ...
                <span className="mt-7 flex w-fit items-center gap-2 rounded-[9px] bg-white px-5 py-3 text-sm font-bold text-coal transition-transform group-hover:translate-x-1">{storefront?.content.heroCtaLabel ?? 'Открыть каталог'} <ArrowRight size={17} /></span>
              </div>
              <div className="absolute -right-12 -top-20 h-[440px] w-[440px] rounded-full bg-coral/15 blur-3xl" />
            ...
            <div className="grid grid-rows-2 gap-4">
              <Link href="/trade-in" className="group relative overflow-hidden rounded-[14px] bg-tint p-6">
                <span className="text-xs font-bold uppercase text-coral">Trade-in</span>
              ...
              <Link href="/delivery" className="group relative overflow-hidden rounded-[14px] bg-coal p-6 text-white">
          ...
          <section className="mt-6 grid grid-cols-8 gap-3" aria-label="Категории товаров">
            {QUICK_CATEGORIES.map((item) => (
              <Link key={item.name} href={item.href} className="group flex min-h-[112px] min-w-0 flex-col items-center justify-center gap-3 rounded-[12px] border border-mist bg-white px-2 text-center transition hover:-translate-y-0.5 hover:border-haze hover:shadow-sm">
                <span className="grid h-10 w-10 place-items-center text-coal [&_svg]:h-6 [&_svg]:w-6">{item.icon}</span>
                <span className="max-w-full text-[12px] font-semibold leading-4">{item.name}</span>
              </Link>
            ))}
          </section>

          {Boolean(storefront?.content.benefits.length) && <section className="mt-6 grid grid-cols-2 divide-x divide-mist rounded-[12px] border border-mist bg-white py-5 lg:grid-cols-4">{storefront!.content.benefits.slice(0,4).map((benefit) => <Benefit key={benefit.title} icon={<ArrowRight />} title={benefit.title} text={benefit.body} />)}</section>}

          {!blocks.some((block) => block.type === 'collection') && <section className="pb-10 pt-12">
            <div className="mb-6 flex items-end justify-between">
              <div><p className="text-xs font-bold uppercase text-coral">Подборка магазина</p><h2 className="mt-1 text-[28px] font-extrabold">{storefront?.content.featuredTitle ?? 'Товары в каталоге'}</h2></div>
              <Link href="/catalog" className="flex items-center gap-2 text-sm font-bold hover:text-coral">Смотреть все <ArrowRight size={17} /></Link>
            </div>
            {products === null ? <CatalogSkeleton /> : products.length > 0 ? <div className="grid grid-cols-4 gap-4">{products.slice(0, 8).map((product) => <ProductCard key={product.id} product={product} />)}</div> : <div className="rounded-[12px] border border-mist bg-white px-6 py-12 text-center text-steel">Каталог обновляется. <Link href="/catalog" className="font-bold text-coral">Открыть каталог</Link></div>}
          </section>}
        </main>
        <SiteFooter />
      </div>
  ...
  return <section className="grid gap-4" aria-label="Предложения AliStore" data-testid="managed-storefront-blocks">
    {blocks.map((block) => block.type === 'collection'
      ? <div key={block.id} className="py-8" data-storefront-block={block.id}>
          ...
          {block.products?.length ? <div className="grid grid-cols-4 gap-4">{block.products.slice(0, 8).map((product) => <ProductCard key={product.id} product={product} />)}</div> : <div className="rounded-[12px] border border-mist bg-white px-6 py-10 text-center text-[#6e6e73]">Подборка временно недоступна</div>}
        </div>
...
function ManagedBanner({ block }: { block: StorefrontBlock }) {
  ...
  return <Link href={block.ctaHref ?? '/catalog'} data-storefront-block={block.id} className={`group relative flex overflow-hidden rounded-[14px] p-10 ${block.type === 'hero' ? 'min-h-[320px]' : 'min-h-[180px]'} ${desktopTone(block.tone)}`}>
    <div className="relative z-10 flex max-w-[640px] flex-col justify-center">{block.eyebrow && <span className="mb-4 w-fit rounded-md bg-coral px-2.5 py-1.5 text-[11px] font-bold uppercase text-white">{block.eyebrow}</span>}<Heading className={`${block.type === 'hero' ? 'text-[44px]' : 'text-[30px]'} font-extrabold leading-[1.05]`}>{block.title}</Heading>{block.body && <p className="mt-3 max-w-[58ch] text-[15px] opacity-70">{block.body}</p>}{block.ctaLabel && <span className="mt-6 flex w-fit items-center gap-2 rounded-[9px] bg-white px-5 py-3 text-sm font-bold text-coal">{block.ctaLabel} <ArrowRight size={17} /></span>}</div>
...
function desktopTone(tone: StorefrontBlock['tone']) {
  ...
  if (tone === 'light') return 'border border-mist bg-white text-coal';
  if (tone === 'lime') return 'bg-lime text-lime-ink';
  return 'bg-graphite text-white';
}

function Benefit({ icon, title, text }: { icon: ReactNode; title: string; text: string }) {
  return <div className="flex items-center justify-center gap-3 px-5"><span className="text-coral [&_svg]:h-6 [&_svg]:w-6">{icon}</span><span><strong className="block text-[13px] font-bold">{title}</strong><small className="mt-0.5 block text-[11px] text-slate">{text}</small></span></div>;
}

function CatalogSkeleton() {
  return <div className="grid grid-cols-4 gap-4">{Array.from({ length: 8 }, (_, index) => <div key={index} className="aspect-[.76] animate-pulse rounded-[12px] border border-mist bg-white" />)}</div>;
}
