      ...
      <div className="mx-auto flex min-h-screen w-full max-w-[1180px] flex-col lg:grid lg:grid-cols-[240px_1fr]">
        <aside className="hidden border-r border-white/10 bg-[#16130f] p-5 lg:flex lg:flex-col">
          ...
          <div className="mt-auto border-t border-white/10 pt-4">
            ...
            <p className="mt-1 text-xs text-white/48">Курьер · {session.staffId.slice(-6)}</p>
    ...
    <div className="px-4 py-6 lg:px-7 lg:py-8">
      <div className="flex items-end justify-between gap-4">
        <div>
          ...
          <p className="mt-1 text-sm text-white/48">{active.length} активных · {deliveries.filter((delivery) => delivery.status === 'delivered').length} завершено</p>
        </div>
      ...
      <div className="mt-4 border-y border-white/10 py-3">
        {delivery.items.map((item) => (
          <div key={`${item.sku}:${item.imei ?? ''}`} className="flex items-center gap-2 py-1 text-xs">
            ...
            <span className="text-white/48">{item.qty} шт.</span>
          </div>
        ))}
        <div className="mt-2 flex items-center justify-between text-sm">
          <span className="text-white/48">Получить COD</span>
...
function CodWorkspace({
  ...
  return (
    <div className="px-4 py-6 lg:px-7 lg:py-8">
      ...
      <p className="mt-2 max-w-xl text-sm text-white/48">Сдавайте только фактически собранные наличные. Итог и расхождение подтверждает сервер и Event Ledger.</p>
      {error && <ErrorPanel message={error} retry={reload} />}
...
function EmptyRoute() {
  return (
    <div className="mt-6 grid min-h-72 place-items-center rounded-[8px] border border-dashed border-white/15 bg-[#16130f] p-6 text-center">
      <div><MapPinned className="mx-auto h-10 w-10 text-[#c8ff38]" aria-hidden /><h2 className="mt-3 font-display font-bold">Маршрут пока пуст</h2><p className="mt-1 text-sm text-white/48">Новая доставка появится после назначения диспетчером.</p></div>
    </div>
  );
}
...
function EmptyCod() {
  return (
    <div className="mt-6 grid min-h-60 place-items-center rounded-[8px] border border-dashed border-white/15 bg-[#16130f] p-6 text-center">
      <div><Banknote className="mx-auto h-10 w-10 text-[#c8ff38]" aria-hidden /><h2 className="mt-3 font-display font-bold">Нет рейсов для сверки</h2><p className="mt-1 text-sm text-white/48">COD появится после назначения рейса.</p></div>
    </div>
  );
}
