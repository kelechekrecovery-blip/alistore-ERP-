        ...
        {state.kind === 'ready' && (() => {
          ...
          return <>
            <div className="mb-6 flex flex-wrap items-end justify-between gap-4 border-b border-white/10 pb-5">
              <div><p className="text-xs font-semibold uppercase text-white/50">Защищённый гостевой доступ</p><h1 className="mt-1 text-2xl font-bold text-white md:text-3xl">Заказ #{order.id.slice(-8)}</h1><p className="mt-2 text-sm text-white/55">{new Date(order.createdAt).toLocaleString('ru-RU')} · {som(order.total)}</p></div>
