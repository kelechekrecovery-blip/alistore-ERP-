    ...
    <MobileAppFrame title="Бонусы и купоны" subtitle="1 бонус = 1 сом. Баланс и купоны синхронизированы с аккаунтом." backHref="/account">
      ...
      {data && <>
        ...
        {data.coupons.map((coupon) => (
          <div key={coupon.id} className="mb-2 flex items-center gap-3 rounded-[13px] border border-surface-3 bg-surface-2 p-3.5">
            <div className="min-w-0 flex-1">
              <div className="text-[13px] font-semibold">{coupon.title}</div>
              <div className="mt-0.5 font-mono text-[11px] text-subtle">{coupon.code}</div>
            </div>
        ...
        <div className="mb-2 mt-5 text-[13px] text-muted">История</div>
        {data.history.length === 0 && <Empty text="Начислений пока нет" />}
        {data.history.map((entry) => (
          <div key={entry.id} className="flex justify-between border-b border-surface-2 py-2.5 text-[13px]">
            <span className="text-muted">{entry.label}</span>
            <span className={`font-mono ${entry.amount >= 0 ? 'text-lime' : 'text-danger-soft'}`}>{entry.amount > 0 ? '+' : ''}{entry.amount.toLocaleString('ru-RU')}</span>
          </div>
        ))}
      </>}
    </MobileAppFrame>
...
function AccountNotice({ text, href }: { text: string; href?: string }) {
  const body = <div className="rounded-[13px] border border-surface-3 bg-surface-2 p-4 text-sm text-muted">{text}</div>;
  return href ? <Link href={href}>{body}</Link> : body;
}

function Empty({ text }: { text: string }) {
  return <div className="rounded-[13px] border border-surface-3 bg-surface-2 p-4 text-sm text-muted">{text}</div>;
}
