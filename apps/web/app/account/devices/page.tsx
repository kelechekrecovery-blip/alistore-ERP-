        ...
        <div className="flex-1 overflow-y-auto px-4 pb-6">
          ...
          {devices && devices.length === 0 && paidRepairs.length === 0 && !loaners.some((loan) => ['prepared', 'issued', 'overdue'].includes(loan.status)) && (
            <div className="py-12 text-center">
              ...
              <div className="mt-2 text-[13px] text-muted">Купленная техника появится здесь с гарантией</div>
            </div>
          )}
          ...
          {loaners.filter((loan) => ['prepared', 'issued', 'overdue'].includes(loan.status)).map((loan) => <div key={loan.id} data-testid={`customer-loaner-${loan.id}`} className={`mb-3 rounded-[16px] border p-4 ${loan.status === 'overdue' ? 'border-danger/40 bg-danger/5' : 'border-surface-3 bg-surface-2'}`}>
            <div className="flex items-start justify-between gap-3"><div><div className="text-[15px] font-bold">{loan.device?.unit.product.name ?? 'Подменное устройство'}</div><div className="mt-1 font-mono text-[11px] text-subtle">IMEI •••• {loan.device?.unit.imei.slice(-4)}</div></div><span className={`rounded-md px-2 py-1 text-[11px] font-semibold ${loan.status === 'overdue' ? 'bg-danger/15 text-danger' : 'bg-warn/15 text-warn'}`}>{loan.status === 'overdue' ? 'Просрочено' : loan.status === 'prepared' ? 'Оформляется' : 'Выдано'}</span></div>
            <div className="mt-3 border-t border-surface-3 pt-3 text-[12px] text-muted">Вернуть до <strong className="text-white">{new Date(loan.dueAt).toLocaleDateString('ru-RU')}</strong>{loan.agreementRef ? ` · расписка ${loan.agreementRef}` : ''}</div>
            <div className="mt-1 text-[11px] text-subtle">Состояние при выдаче: {loan.issueCondition}</div>
          </div>)}
          {(devices ?? []).map((d) => {
            const w = d.warranty;
            ...
            return (
              <div key={d.imei} className="mb-3 rounded-[16px] border border-surface-3 bg-surface-2 p-4">
                ...
                <div className="mt-3 flex flex-wrap items-center gap-1.5">
                  {w ? (
                    <span className="font-mono text-[11px] text-muted">SLA до {new Date(w.sla).toLocaleDateString('ru-RU')}</span>
                  ) : (
                  ...
                  <div data-testid={`service-estimate-${workOrder.id}`} className="mt-3 rounded-[8px] border border-[#3B342D] bg-ink-dark p-3">
                    <div className="flex items-start justify-between gap-3">
                      <div>
                        ...
                        <div className="mt-1 text-[12px] text-subtle">{workOrder.diagnosticSummary}</div>
                      </div>
                ...
                <Link
                  ...
                  className="mt-3 flex items-center justify-between border-t border-surface-3 pt-3 text-[13px] text-bright"
                >
          ...
          {paidRepairs.map((workOrder) => <div key={workOrder.id} data-testid={`paid-service-account-${workOrder.id}`} className="mb-3 rounded-[16px] border border-surface-3 bg-surface-2 p-4">
            <div className="flex items-start justify-between gap-3">
