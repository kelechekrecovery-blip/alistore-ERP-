    ...
    <MobileAppFrame title="Защита устройства" subtitle="Дополнительное покрытие для техники, купленной в AliStore." active="account" backHref="/account">
      {devices.length === 0 ? (
        ...
        <section className="rounded-[14px] border border-surface-3 bg-surface-2 p-4" data-testid="protection-form">
          <label className="block text-[12px] text-muted">
            Устройство
            <select data-testid="protection-imei" value={imei} onChange={(event) => setImei(event.target.value)} className="mt-1.5 w-full rounded-[10px] border border-surface-3 bg-ink-dark px-3 py-3 text-sm text-white">
              {devices.map((device) => <option key={device.imei} value={device.imei}>{device.product} · {device.imei}</option>)}
            </select>
          </label>
          <div className="mt-4 space-y-2">
            {PLANS.map((plan) => (
              <button key={plan.id} type="button" onClick={() => setPlanType(plan.id)} className={`w-full rounded-[11px] border p-3 text-left ${planType === plan.id ? 'border-lime bg-lime/5' : 'border-surface-3 bg-ink-dark'}`}>
                <span className="block text-sm font-semibold">{plan.label}</span>
                <span className="mt-1 block text-[11px] text-subtle">{plan.detail}</span>
              </button>
            ))}
          </div>
          <div className="mt-3 grid grid-cols-2 gap-2">
            {[12, 24].map((months) => (
              <button key={months} type="button" onClick={() => setCoverageMonths(months as 12 | 24)} className={`rounded-[10px] border py-2.5 text-xs font-semibold ${coverageMonths === months ? 'border-lime text-lime' : 'border-surface-3 text-muted'}`}>{months} месяцев</button>
            ))}
          </div>
      ...
      {policies.length > 0 && (
        <section className="mt-5">
          ...
          {policies.map((policy) => (
            <article key={policy.id} className="mb-2.5 rounded-[14px] border border-surface-3 bg-surface-2 p-4">
              ...
              <div className="mt-1 font-mono text-[11px] text-subtle">{policy.imei}</div>
              <div className="mt-2 text-xs text-muted">{policy.coverageMonths} мес. · {PLANS.find((plan) => plan.id === policy.planType)?.label}</div>
              {policy.premium !== null && <div className="mt-2 font-display text-lg font-extrabold text-lime">{som(policy.premium)}</div>}
              {policy.staffNote && <p className="mt-1 text-xs text-muted">{policy.staffNote}</p>}
              {policy.endsAt && <p className="mt-1 text-[11px] text-subtle">Покрытие до {new Date(policy.endsAt).toLocaleDateString('ru-RU')}</p>}
