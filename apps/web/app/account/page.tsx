...
export default function AccountPage() {
  ...
  return <>
    ...
    <main className="mx-auto max-w-[1400px] px-5 py-10">
      ...
      <section className="pt-14"><div className="flex items-center justify-between"><h2 className="font-display text-2xl font-bold">Мои заказы</h2><span className="text-sm text-[#8A7F76]">{orders?.length ?? 0}</span></div><div className="mt-6 overflow-hidden rounded-[18px] border border-[#E7DDD3] bg-white shadow-soft">{orders === null ? <div className="p-8 text-center text-[#8A7F76]">Загрузка заказов...</div> : orders.length === 0 ? <div className="grid min-h-[230px] place-items-center p-8 text-center"><div><Package className="mx-auto text-[#8A7F76]" size={34} /><h3 className="mt-4 font-display text-lg font-semibold">Заказов пока нет</h3><Link href="/catalog" className="mt-3 inline-block text-sm text-deep">Перейти в каталог</Link></div></div> : orders.map((order) => { const status = STATUS[order.status] ?? { label: order.status, cls: 'border-[#DED3C8] bg-sand text-[#6E645C]' }; return <Link key={order.id} href={`/account/orders/${order.id}`} className="grid gap-3 border-b border-[#E7DDD3] px-5 py-4 last:border-0 hover:bg-sand sm:grid-cols-[150px_130px_1fr_auto] sm:items-center"><strong className="font-mono text-sm">#{order.id.slice(-8)}</strong><span className={`w-fit rounded-full border px-2.5 py-1 text-[11px] ${status.cls}`}>{status.label}</span><span className="text-sm text-[#6E645C]">{order.fulfillmentType ?? order.channel}</span><strong className="font-display text-lg">{som(order.total)}</strong></Link>; })}</div></section>
    </main>
    <SiteFooter />
    </div>
  </>;
