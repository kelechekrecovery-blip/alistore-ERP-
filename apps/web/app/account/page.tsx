...
export default function AccountPage() {
  ...
  // `setOrders([])` показывал покупателю «Заказов пока нет» при упавшем
  // запросе — человек решал, что его заказ не оформился, и заказывал повторно.
  useEffect(() => { if (user) { setOrdersError(''); authed(fetchMyOrders).then(setOrders).catch((cause) => setOrdersError(cause instanceof Error ? cause.message : 'Не удалось загрузить заказы')); } }, [user, authed]);
  ...
  if (!hydrated || !user) return <div className="min-h-screen bg-[#0b0a08] text-white/50"><SiteHeader variant="design3" /><div className="grid min-h-[70vh] place-items-center">Загрузка кабинета...</div></div>;

  const phoneLabel = user.phone ?? 'Телефон не подтверждён';
  const avatarLabel = user.phone?.slice(-2) ?? 'AI';

  return <>
    <div className="md:hidden"><MobileProfile phone={user.phone} orders={orders} ordersError={ordersError} loyalty={loyalty} loyaltyError={loyaltyError} onLogout={async () => { await logout(); router.push('/'); }} /></div>
    ...
    <main className="mx-auto max-w-[1400px] px-5 py-10">
      ...
      <section className="pt-14"><div className="flex items-center justify-between"><h2 className="font-display text-2xl font-bold text-white">Мои заказы</h2><span className="text-sm text-white/40">{ordersError ? '—' : orders?.length ?? 0}</span></div><div className="mt-6 overflow-hidden rounded-[18px] border border-white/10 bg-white/[.04] shadow-[0_12px_30px_rgba(0,0,0,.3)]">{ordersError ? <div className="grid min-h-[230px] place-items-center p-8 text-center"><div><h3 className="font-display text-lg font-semibold text-white">Заказы не загрузились</h3><p className="mt-2 text-sm text-white/50">{ordersError}</p><p className="mt-1 text-sm text-white/40">Это не значит, что заказов нет — мы их просто не увидели.</p></div></div> : orders === null ? <div className="p-8 text-center text-white/40">Загрузка заказов...</div> : orders.length === 0 ? <div className="grid min-h-[230px] place-items-center p-8 text-center"><div><Package className="mx-auto text-white/40" size={34} /><h3 className="mt-4 font-display text-lg font-semibold text-white">Заказов пока нет</h3><Link href="/catalog" className="mt-3 inline-block text-sm text-[#ff9a6e]">Перейти в каталог</Link></div></div> : orders.map((order) => { const status = STATUS[order.status] ?? { label: order.status, cls: 'border-white/10 bg-white/[.04] text-white/50' }; return <Link key={order.id} href={`/account/orders/${order.id}`} className="grid gap-3 border-b border-white/[.08] px-5 py-4 last:border-0 hover:bg-white/[.06] sm:grid-cols-[150px_130px_1fr_auto] sm:items-center"><strong className="font-mono text-sm text-white">#{order.id.slice(-8)}</strong><span className={`w-fit rounded-full border px-2.5 py-1 text-[11px] ${status.cls}`}>{status.label}</span><span className="text-sm text-white/50">{order.fulfillmentType ?? order.channel}</span><strong className="font-display text-lg text-white">{som(order.total)}</strong></Link>; })}</div></section>
    </main>
    <SiteFooter />
    </div>
  </>;
