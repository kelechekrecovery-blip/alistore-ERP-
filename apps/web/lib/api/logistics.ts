...
export function createCourierRun(input: { courierId: string; orderIds: string[]; codTotal: number }, token: string, idempotencyKey: string) { return postAuthJson<CourierRun>('/courier/runs', input, token, { 'idempotency-key': idempotencyKey }); }
