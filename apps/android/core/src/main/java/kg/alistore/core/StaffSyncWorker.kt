...
class StaffSyncWorker(appContext: Context, params: WorkerParameters) : CoroutineWorker(appContext, params) {
  override suspend fun doWork(): Result = withContext(Dispatchers.IO) {
    ...
    val token = SecureTokenStore(applicationContext, "alistore-staff-session").readToken()
      ?: return@withContext Result.failure()
    val queue = OfflineQueueDb(applicationContext, STAFF_QUEUE_DB)
    val client = ApiClient(apiBaseUrl)
    var retryRequired = false
    for (mutation in queue.pending().filter { it.state == "queued" }) {
      try {
