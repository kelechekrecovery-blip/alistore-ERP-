...
class FirebaseCourierPushRegistrar(context: Context, private val apiBaseUrl: String) : StaffPushRegistrar {
  ...
  companion object {
    private const val PREFS = "courier-push"
    ...
    fun onNewToken(context: Context, apiBaseUrl: String, token: String) {
      val registrar = FirebaseCourierPushRegistrar(context, apiBaseUrl)
      registrar.saveToken(token)
      val accessToken = SecureTokenStore(context, "alistore-courier-session").readToken() ?: return
