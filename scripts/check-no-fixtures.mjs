...
if (grown.length === 0) {
  ...
  // Храповик обязан падать только на регрессе (`grown`). Раньше здесь стоял
  // `exit(shrunk ? 1 : 0)` — то есть УЛУЧШЕНИЕ кода (стало меньше фикстур) роняло
  // гейт красным, пока кто-то не обновит базовую линию. Это дало ложные диагнозы:
  // зелёная работа выглядела сломанной. Улучшение — не провал; выходим 0, а
  // напоминание про --update-baseline уже напечатано выше.
  process.exit(0);
}
